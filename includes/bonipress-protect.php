...
/**
 * boniPRESS_Protect class
 * Will be replaced by openssl_encrypt() in 1.8
 * @since 0.1
 * @version 1.3
 */
if ( ! class_exists( 'boniPRESS_Protect' ) ) :
	class boniPRESS_Protect {
		...
		public function __construct( $custom_key = NULL ) {
			...
			if ( $custom_key !== NULL )
				...
				$skey = bonipress_get_option( 'bonipress_key', false );
		...
		public function reset_key() {
			...
			bonipress_update_option( 'bonipress_key', $skey );
			$this->skey = $skey;
...
if ( ! function_exists( 'bonipress_protect' ) ) :
	function bonipress_protect() {
		...
		if ( ! is_object( $bonipress_protect ) )
			$bonipress_protect = new boniPRESS_Protect();

		return $bonipress_protect;
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_create_token' ) ) :
	function bonipress_create_token( $string = '' ) {
		...
		return apply_filters( 'bonipress_create_token', $encoded, $string );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_verify_token' ) ) :
	function bonipress_verify_token( $string = '', $length = 1 ) {
		...
		return apply_filters( 'bonipress_verify_token', $reply, $string, $length );
	}
endif;
