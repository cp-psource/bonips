...
/**
 * boniPRESS_Settings class
 * @see http://codex.bonipress.me/classes/bonipress_settings/
 * @since 0.1
 * @version 1.6
 */
if ( ! class_exists( 'boniPRESS_Settings' ) ) :
	class boniPRESS_Settings {
		...
		public function __construct( $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
			...
			do_action_ref_array( 'bonipress_settings', array( &$this ) );
		...
		public function defaults() {
			...
			return array(
				...
				'name'        => array(
					'singular'    => __( 'Punkt', 'bonipress' ),
					'plural'      => __( 'Punkte', 'bonipress' )
				),
				'before'      => '',
				'after'       => '',
				'caps'        => array(
		...
		public function get_point_type_settings() {
			...
			$settings  = bonipress_get_option( $this->option_id, $defaults );

			return apply_filters( 'bonipress_get_point_type_settings', $settings, $defaults, $this );
		 ...
		 * @param $number (int|float) the initial number
		 * @returns the given number formated either as an integer or float
		 * @filter 'bonipress_format_number'
		 * @since 0.1
		 * @version 1.1
		 */
		public function format_number( $value = NULL ) {
			...
			return apply_filters( 'bonipress_format_number', $creds, $value, $this->core );
		...
		public function format_creds( $value = 0, $before = '', $after = '', $force_in = false ) {
			...
			return apply_filters( 'bonipress_format_creds', $layout, $value, $this );
		...
		public function round_value( $value = NULL, $up_down = false, $precision = 0 ) {
			...
			return apply_filters( 'bonipress_round_value', $value, $original_value, $up_down, $precision );
		...
		public function apply_exchange_rate( $amount = 0, $rate = 1 ) {
			...
			return apply_filters( 'bonipress_apply_exchange_rate', $value, $amount, $rate );
		...
		public function parse_template_tags( $content = '', $log_entry ) {
			...
			$content   = apply_filters( 'bonipress_parse_log_entry',              $content, $log_entry );
			$content   = apply_filters( "bonipress_parse_log_entry_{$reference}", $content, $log_entry );
			...
			if ( isset( $data['ref_type'] ) || isset( $data['post_type'] ) ) {
				...
				$content = apply_filters( "bonipress_parse_tags_{$type}", $content, $log_entry );
		...
		public function template_tags_general( $content = '' ) {
			...
			$content = apply_filters( 'bonipress_parse_tags_general', $content );
			...
			$content = str_replace( '%login_url_here%',  wp_login_url( bonipress_get_permalink() ), $content );

			// Logout URL
			$content = str_replace( '%logout_url%',      wp_logout_url(), $content );
			$content = str_replace( '%logout_url_here%', wp_logout_url( bonipress_get_permalink() ), $content );
		...
		public function template_tags_amount( $content = '', $amount = 0 ) {
			...
			$content = apply_filters( 'bonipress_parse_tags_amount', $content, $amount, $this );
		...
		public function template_tags_post( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			// Get Post Object
			$post     = bonipress_get_post( $ref_id );
			...
			if ( !isset( $post->ID ) ) {
				...
				$post_url = bonipress_get_permalink( $post );

			}

			// Let others play first
			$content = apply_filters( 'bonipress_parse_tags_post', $content, $post, $data );

			// Replace template tags
			$content = str_replace( '%post_title%',      esc_attr( bonipress_get_the_title( $post ) ), $content );
		...
		public function template_tags_user( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			// Let others play first
			$content     = apply_filters( 'bonipress_parse_tags_user', $content, $user, $data );
			...
			$profile_url = bonipress_get_users_profile_url( $user->ID );
		...
		public function template_tags_comment( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			$comment_post_title = __( 'Gelöschter Artikel', 'bonipress' );
			...
			if ( $comment === NULL ) {
				...
				$comment_post       = bonipress_get_post( $comment->comment_post_ID );
				$comment_url        = bonipress_get_permalink( $comment_post );
				$comment_post_title = bonipress_get_permalink( $comment_post );

			}

			// Let others play first
			$content = apply_filters( 'bonipress_parse_tags_comment', $content, $comment, $data );
		 ...
		 * @filter 'bonipress_has_tags'
		 * @filter 'bonipress_has_tags_{$type}'
		...
		public function has_tags( $type = '', $tags = '', $content = '' ) {
			...
			$tags = apply_filters( 'bonipress_has_tags', $tags, $content );
			$tags = apply_filters( 'bonipress_has_tags_' . $type, $tags, $content );
		...
		public function allowed_tags( $data = '', $allow = '' ) {
			...
			return strip_tags( $data, apply_filters( 'bonipress_allowed_tags', '<a><br><em><strong><span>' ) );
		...
		public function allowed_html_tags() {
			...
			return apply_filters( 'bonipress_allowed_html_tags', array(
		...
		public function get_point_admin_capability() {
			...
			// backwards cap.
			$capability = apply_filters( 'bonipress_edit_plugin_cap', $this->caps['plugin'] );
		...
		public function get_point_editor_capability() {
			...
			$capability = apply_filters( 'bonipress_edit_creds_cap', $this->caps['creds'] );
		...
		public function in_exclude_list( $user_id = '' ) {
			...
			return apply_filters( 'bonipress_is_excluded_list', $result, $user_id );
		...
		public function exclude_user( $user_id = NULL ) {
			...
			// Quick override
			if ( apply_filters( 'bonipress_exclude_user', false, $user_id, $this ) === true ) return true;
		...
		public function count_members() {
			...
			$count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( DISTINCT user_id ) FROM {$wpdb->usermeta} WHERE meta_key = %s;", bonipress_get_meta_key( $this->cred_id ) ) );
		...
		public function enforce_max( $user_id = NULL, $amount = NULL ) {
			...
			if ( ( $maxium > $this->zero() && $amount > $maxium ) || ( $maxium < $this->zero() && $amount < $maxium ) ) {
				...
				do_action( 'bonipress_max_enforced', $user_id, $original_amount, $maxium );
		...
		public function get_users_balance( $user_id = NULL, $point_type = NULL ) {
			...
			// Point type
			if ( $point_type === NULL || ! array_key_exists( $point_type, $bonipress_types ) )
				$point_type = $this->get_point_type_key();

			if ( bonipress_is_current_account( $user_id ) && $bonipress_current_account->balance[ $point_type ] !== false )
				$balance = $bonipress_current_account->balance[ $point_type ]->get( 'current' );

			else
				$balance = bonipress_get_user_meta( $user_id, $point_type, '', true );

			if ( $balance == '' ) $balance = $this->zero();

			return $this->number( apply_filters( 'bonipress_get_users_cred', $balance, $this, $user_id, $point_type ) );
		...
		public function get_users_total_balance( $user_id = NULL, $point_type = NULL ) {
			...
			if ( BONIPRESS_ENABLE_TOTAL_BALANCE ) {
				...
				if ( bonipress_is_current_account( $user_id ) && $bonipress_current_account->balance[ $point_type ] !== false )
					$total_balance = $bonipress_current_account->balance[ $point_type ]->get( 'accumulated' );

				else
					$total_balance = bonipress_get_user_meta( $user_id, $point_type, '_total', true );

				if ( $total_balance == '' ) {

					$total_balance = bonipress_query_users_total( $user_id, $point_type );

					bonipress_update_user_meta( $user_id, $point_type, '_total', $total_balance );

				}

			}

			return $this->number( apply_filters( 'bonipress_get_users_total_cred', $total_balance, $this, $user_id, $point_type ) );
		...
		public function update_users_balance( $user_id = NULL, $amount = NULL, $point_type = NULL ) {
			...
			// Save new balance
			bonipress_update_user_meta( $user_id, $point_type, '', $new_balance );

			// Update the current account object
			if ( bonipress_is_current_account( $user_id ) && $bonipress_current_account->balance[ $point_type ] !== false )
				$bonipress_current_account->balance[ $point_type ]->set( 'current', $new_balance );

			// Let others play
			do_action( 'bonipress_update_user_balance', $user_id, $current_balance, $amount, $point_type );
		...
		public function update_users_total_balance( $user_id = NULL, $amount = 0, $point_type = NULL ) {
			...
			if ( bonipress_is_current_account( $user_id ) && $bonipress_current_account->balance[ $point_type ] !== false )
				$bonipress_current_account->balance[ $point_type ]->set( 'accumulated', $total_balance );

			do_action( 'bonipress_update_user_total_balance', $total_balance, $user_id, $point_type, $this );
		...
		public function set_users_balance( $user_id = NULL, $new_balance = NULL ) {
			...
			// Clear caches
			bonipress_delete_option( 'bonipress-cache-total-' . $point_type );

			// Let others play
			do_action( 'bonipress_set_user_balance', $user_id, $new_balance, $this );
		...
		public function set_users_total_balance( $user_id = NULL, $new_balance = NULL ) {
			...
			// Update balance
			bonipress_update_user_meta( $user_id, $this->get_point_type_key(), '_total', $total_balance );

			if ( bonipress_is_current_account( $user_id ) && $bonipress_current_account->balance[ $point_type ] !== false )
				$bonipress_current_account->balance[ $point_type ]->set( 'accumulated', $total_balance );

			// Let others play
			do_action( 'bonipress_set_user_total_balance', $user_id, $total_balance, $this );
		...
		public function add_creds( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $type = NULL ) {
			...
			// Point type
			if ( $type === NULL || ! array_key_exists( $type, $bonipress_types ) )
				$type = $this->get_point_type_key();

			// Execution Override
			// Allows us to stop an execution. 
			// excepts a boolean reply
			$execute            = apply_filters( 'bonipress_add', true, compact( 'ref', 'user_id', 'amount', 'entry', 'ref_id', 'data', 'type' ), $this );
			...
			if ( $execute === true ) {
				...
				$run_this   = apply_filters( 'bonipress_run_this', compact( 'ref', 'user_id', 'amount', 'entry', 'ref_id', 'data', 'type' ), $this );
			...
			return apply_filters( 'bonipress_add_finished', $execute, $run_this, $this );
		...
		public function add_to_log( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $type = NULL ) {
			...
			if ( BONIPRESS_ENABLE_LOGGING ) {
				...
				$time   = apply_filters( 'bonipress_log_time', current_time( 'timestamp' ), $ref, $user_id, $amount, $entry, $ref_id, $data, $type );
				$insert = array(
					'ref'     => $ref,
					'ref_id'  => $ref_id,
					'user_id' => (int) $user_id,
			...
			return apply_filters( 'bonipress_new_log_entry_id', $insert_id, $insert, $this );
		...
		public function update_log_entry( $entry_id = NULL, $data = array(), $prep = array() ) {
			...
			do_action( 'bonipress_log_entry_updated', $entry_id, $data );
		...
		function has_entry( $reference = NULL, $ref_id = NULL, $user_id = NULL, $data = NULL, $type = NULL ) {
			...
			if ( $user_id !== NULL && bonipress_is_current_account( $user_id ) && ! empty( $bonipress_current_account->point_type ) && in_array( $type, $bonipress_current_account->point_type ) ) {

				if ( isset( $bonipress_current_account->balance[ $type ]->history ) && ! empty( $bonipress_current_account->balance[ $type ]->history->data ) ) {

					$data = $bonipress_current_account->balance[ $type ]->history->data;
			...
			return apply_filters( 'bonipress_has_entry', $has_entry, $reference, $ref_id, $user_id, $data, $type );
...
/**
 * boniPRESS Label
 * Returns the boniPRESS Label
 * @since 1.3.3
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_label' ) ) :
	function bonipress_label( $trim = false ) {
		...
		if ( $trim )
			$name = strip_tags( $bonipress_label );
...
/**
 * Get boniPRESS
 * Returns boniPRESS's general settings and core functions.
 * Replaces bonipress_get_settings()
 * @since 1.4
 * @version 1.1
 */
if ( ! function_exists( 'bonipress' ) ) :
	function bonipress( $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		if ( $point_type != BONIPRESS_DEFAULT_TYPE_KEY ) {
			...
			$current_bonipress = new boniPRESS_Settings( $point_type );
 ...
 * Returns boniPRESS's network settings or false if multisite is not enabled.
 * @since 0.1
 * @version 1.2
 */
if ( ! function_exists( 'bonipress_get_settings_network' ) ) :
	function bonipress_get_settings_network() {
		...
		if ( is_array( $bonipress_network ) && ! empty( $bonipress_network ) && array_key_exists( 'master', $bonipress_network ) )
			return $bonipress_network;

		$settings            = ( ( is_multisite() ) ? get_blog_option( get_network()->site_id, 'bonipress_network', $defaults ) : $defaults );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_is_main_site' ) ) :
	function bonipress_is_main_site( $site_id = NULL ) {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_is_site_blocked' ) ) :
	function bonipress_is_site_blocked( $blog_id = NULL ) {
		...
		return apply_filters( 'bonipress_is_site_blocked', $blocked, $blog_id );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_override_settings' ) ) :
	function bonipress_override_settings() {

		// Not a multisite
		if ( ! is_multisite() ) return false;

		$network_setup = bonipress_get_settings_network();

		return apply_filters( 'bonipress_mu_override_settings', (bool) $network_setup['master'], $network_setup );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_centralize_log' ) ) :
	function bonipress_centralize_log() {
		...
		return apply_filters( 'bonipress_mu_centralize_log', (bool) $network_setup['central'], $network_setup );
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_get_module' ) ) :
	function bonipress_get_module( $module = '', $type = 'solo' ) {

		global $bonipress_modules;

		if ( $type == 'solo' ) {

			if ( ! array_key_exists( $module, $bonipress_modules['solo'] ) )
				return false;

			return $bonipress_modules['solo'][ $module ];

		}

		if ( ! array_key_exists( $type, $bonipress_modules['type'] ) )
			return false;

		return $bonipress_modules['type'][ $type ][ $module ];
...
if ( ! function_exists( 'bonipress_get_addon_settings' ) ) :
	function bonipress_get_addon_settings( $addon = '', $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		if ( $addon != '' ) {
			...
			if ( isset( $bonipress->$addon ) )
				$settings = $bonipress->$addon;
		...
		return apply_filters( 'bonipress_get_addon_settings', $settings, $addon, $point_type );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_remote' ) ) :
	function bonipress_get_remote() {

		$defaults = apply_filters( 'bonipress_remote_defaults', array(
			'enabled' => 0,
			'key'     => '',
			'uri'     => 'api-dev',
			'debug'   => 0
		...
		return bonipress_apply_defaults( $defaults, bonipress_get_option( 'bonipress_pref_remote', array() ) );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'is_bonipress_ready' ) ) :
	function is_bonipress_ready() {

		if ( bonipress_is_installed() !== false ) return true;
...
/**
 * Is boniPRESS Installed
 * Returns either false (setup has not been run) or the timestamp when it was completed.
 * @since 1.7
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_is_installed' ) ) :
	function bonipress_is_installed() {

		return bonipress_get_option( 'bonipress_setup_completed', false );

	}
endif;

/**
 * Maybe Install boniPRESS Table
 * Check to see if maybe the boniPRESS table needs to be installed.
 * @since 1.7.6
 * @version 1.0.1
 */
if ( ! function_exists( 'maybe_install_bonipress_table' ) ) :
	function maybe_install_bonipress_table() {
		...
		if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $bonipress_log_table ) ) != $bonipress_log_table ) {

			bonipress_install_log( NULL, true );

			do_action( 'bonipress_reinstalled_table' );
...
if ( ! function_exists( 'bonipress_install_log' ) ) :
	function bonipress_install_log( $decimals = NULL, $force = false ) {
		...
		if ( ! $force ) {
			...
			$db_version = bonipress_get_option( 'bonipress_version_db', false );

			// DB Already installed
			if ( $db_version == boniPRESS_DB_VERSION ) return true;
		...
		dbDelta( "CREATE TABLE IF NOT EXISTS {$bonipress->log_table} ( " . $sql . " ) $collate;" );

		bonipress_update_option( 'bonipress_version_db', boniPRESS_DB_VERSION );
...
/**
 * Get Post Types
 * Returns an array of post types that boniPRESS uses.
 * @since 1.7
 * @version 1.1
 */
if ( ! function_exists( 'get_bonipress_post_types' ) ) :
	function get_bonipress_post_types() {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_get_user_id' ) ) :
	function bonipress_get_user_id( $requested = '' ) {
		...
		return apply_filters( 'bonipress_get_user_id', $user_id, $requested );
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonipress_get_users_profile_url' ) ) :
	function bonipress_get_users_profile_url( $user_id = NULL ) {
		...
		return apply_filters( 'bonipress_users_profile_url', $profile_url, $user );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_get_account' ) ) :
	function bonipress_get_account( $user_id = NULL ) {

		global $bonipress_current_account, $bonipress_account;
		...
		if ( bonipress_is_current_account( $user_id ) )
			return $bonipress_current_account;

		if ( bonipress_is_account( $user_id ) ) {

			return $bonipress_account;

		}

		$bonipress_account = new boniPRESS_Account( $user_id );

		do_action( 'bonipress_get_account' );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_is_account' ) ) :
	function bonipress_is_account( $user_id = NULL ) {

		global $bonipress_account;

		if ( isset( $bonipress_account )
			&& ( $bonipress_account instanceof boniPRESS_Account )
			&& ( $user_id === $bonipress_account->user_id ) 
			&& ( $user_id !== NULL )
			&& ( $user_id !== 0 )
		) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_current_account' ) ) :
	function bonipress_get_current_account() {

		global $bonipress_current_account;

		if ( isset( $bonipress_current_account ) && ( $bonipress_current_account instanceof boniPRESS_Account ) )
			return $bonipress_current_account;
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_set_current_account' ) ) :
	function bonipress_set_current_account( $user_id = NULL ) {

		global $bonipress_current_account;

		if ( isset( $bonipress_current_account )
			&& ( $bonipress_current_account instanceof boniPRESS_Account )
			&& ( $user_id === $bonipress_current_account->user_id ) 
			&& ( $user_id !== NULL )
		) {
		...
		$bonipress_current_account = new boniPRESS_Account( ( ( $user_id === NULL ) ? get_current_user_id() : $user_id ) );

		do_action( 'bonipress_set_current_account' );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_is_current_account' ) ) :
	function bonipress_is_current_account( $user_id = NULL ) {
 ...
 * @param $name_first (bool) option to replace "boniPRESS" with the point type name set.
 * @since 1.4
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_get_types' ) ) :
	function bonipress_get_types( $name_first = false ) {

		global $bonipress_types;

		if ( is_array( $bonipress_types ) && ! empty( $bonipress_types ) )
			$types = $bonipress_types;
		...
		return apply_filters( 'bonipress_types', $types );
...
if ( ! function_exists( 'bonipress_get_point_type' ) ) :
	function bonipress_get_point_type( $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		global $current_bonipress_type;

		if ( isset( $current_bonipress_type )
			&& ( $current_bonipress_type instanceof boniPRESS_Point_Type )
			&& ( $current_bonipress_type->cred_id === $point_type )
		) {

			return $current_bonipress_type;

		}

		$current_bonipress_type = new boniPRESS_Point_Type( $point_type );

		do_action( 'bonipress_get_point_type', $point_type );
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_get_usable_types' ) ) :
	function bonipress_get_usable_types( $user_id = NULL ) {
		...
		if ( is_user_logged_in() || $original_id !== NULL ) {
			...
			global $bonipress, $bonipress_current_account;

			if ( ! isset( $bonipress_current_account->balance ) || empty( $bonipress_current_account->balance ) ) {
			...
			elseif ( ! empty( $bonipress_current_account->balance ) ) {

				foreach ( $bonipress_current_account->balance as $balance ) {
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_point_type_exists' ) ) :
	function bonipress_point_type_exists( $type = NULL ) {
...
if ( ! function_exists( 'bonipress_get_point_type_name' ) ) :
	function bonipress_get_point_type_name( $point_type = BONIPRESS_DEFAULT_TYPE_KEY, $singular = true ) {
		...
		if ( $singular )
			return $bonipress->singular();

		return $bonipress->plural();
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_types_select_from_dropdown' ) ) :
	function bonipress_types_select_from_dropdown( $name = '', $id = '', $selected = '', $return = false, $extra = '' ) {

		$types  = bonipress_get_types();
		$output = '';

		if ( count( $types ) == 1 )
			$output .= '<input type="hidden"' . $extra . ' name="' . $name . '" id="' . $id . '" value="bonipress_default" />';
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_types_select_from_checkboxes' ) ) :
	function bonipress_types_select_from_checkboxes( $name = '', $id = '', $selected_values = array(), $return = false ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_db_column' ) ) :
	function bonipress_get_db_column( $column = '' ) {
		...
		return apply_filters( 'bonipress_get_db_column', $table );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_add_option' ) ) :
	function bonipress_add_option( $option_id, $value = '' ) {

		if ( is_multisite() ) {

			// Master template enabled
			if ( bonipress_override_settings() )
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonipress_get_option' ) ) :
	function bonipress_get_option( $option_id, $default = array() ) {
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonipress_update_option' ) ) :
	function bonipress_update_option( $option_id, $value = '' ) {
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_delete_option' ) ) :
	function bonipress_delete_option( $option_id ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_meta_key' ) ) :
	function bonipress_get_meta_key( $key = '', $end = '' ) {

		if ( is_multisite() ) {

			$blog_id = get_current_blog_id();

			if ( $blog_id > 1 && ! bonipress_centralize_log() && $key != 'bonipress_rank' )
				$key .= '_' . $blog_id;

			elseif ( $blog_id > 1 && ! bonipress_override_settings() && $key == 'bonipress_rank' )
				$key .= '_' . $blog_id;
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_add_user_meta' ) ) :
	function bonipress_add_user_meta( $user_id, $key = '', $end = '', $value = '', $unique = false ) {

		$key = bonipress_get_meta_key( $key, $end );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_get_user_meta' ) ) :
	function bonipress_get_user_meta( $user_id, $key = '', $end = '', $unique = false ) {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_update_user_meta' ) ) :
	function bonipress_update_user_meta( $user_id, $key = '', $end = '', $value = '', $previous = '' ) {
 ...
 * @version 1.1.1
 */
if ( ! function_exists( 'bonipress_delete_user_meta' ) ) :
	function bonipress_delete_user_meta( $user_id, $key = '', $end = '', $value = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_add_post_meta' ) ) :
	function bonipress_add_post_meta( $post_id, $key = '', $value = '', $unique = false ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_post_meta' ) ) :
	function bonipress_get_post_meta( $post_id, $key = '', $unique = false ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_update_post_meta' ) ) :
	function bonipress_update_post_meta( $post_id, $key = '', $value = '', $previous = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_delete_post_meta' ) ) :
	function bonipress_delete_post_meta( $post_id, $key = '', $value = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_post' ) ) :
	function bonipress_get_post( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_permalink' ) ) :
	function bonipress_get_permalink( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_post_type' ) ) :
	function bonipress_get_post_type( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_the_title' ) ) :
	function bonipress_get_the_title( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_page_by_title' ) ) :
	function bonipress_get_page_by_title( $post_id, $type, $post_type ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_trash_post' ) ) :
	function bonipress_trash_post( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_delete_post' ) ) :
	function bonipress_delete_post( $post_id = NULL, $force = false ) {
...
if ( ! function_exists( 'bonipress_is_admin' ) ) :
	function bonipress_is_admin( $user_id = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		if ( $bonipress->user_is_point_admin( $user_id ) || $bonipress->user_is_point_editor( $user_id ) )
			return true;
...
if ( ! function_exists( 'bonipress_exclude_user' ) ) :
	function bonipress_exclude_user( $user_id = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return $bonipress->exclude_user( $user_id );
...
if ( ! function_exists( 'bonipress_get_users_balance' ) ) :
	function bonipress_get_users_balance( $user_id = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		if ( $bonipress->exclude_user( $user_id ) ) return false;

		return $bonipress->get_users_balance( $user_id, $point_type );
...
if ( ! function_exists( 'bonipress_get_users_total_balance' ) ) :
	function bonipress_get_users_total_balance( $user_id = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return $bonipress->get_users_total_balance( $user_id, $point_type );
...
if ( ! function_exists( 'bonipress_display_users_balance' ) ) :
	function bonipress_display_users_balance( $user_id = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return $bonipress->format_creds( $balance );
...
if ( ! function_exists( 'bonipress_display_users_total_balance' ) ) :
	function bonipress_display_users_total_balance( $user_id = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		$balance = $bonipress->get_users_total_balance( $user_id, $point_type );
...
if ( ! function_exists( 'bonipress_format_number' ) ) :
	function bonipress_format_number( $value = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return $bonipress->format_number( $value );
...
if ( ! function_exists( 'bonipress_format_points' ) ) :
	function bonipress_format_points( $value = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return $bonipress->format_creds( $value );
...
if ( ! function_exists( 'bonipress_add' ) ) :
	function bonipress_add( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return $bonipress->add_creds( $ref, $user_id, $amount, $entry, $ref_id, $data, $point_type );
...
if ( ! function_exists( 'bonipress_subtract' ) ) :
	function bonipress_subtract( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		return bonipress_add( $ref, $user_id, $amount, $entry, $ref_id, $data, $point_type );
 ...
 * @version 1.1.1
 */
if ( ! function_exists( 'bonipress_plugin_activation' ) ) :
	function bonipress_plugin_activation() {

		// Load Installer
		require_once boniPRESS_INCLUDES_DIR . 'bonipress-install.php';
		$installer = bonipress_installer();

		// Compatibility check
		$installer::compat();

		// First time activation
		if ( get_option( 'bonipress_version', false ) === false )
			$installer::activate();
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_plugin_deactivation' ) ) :
	function bonipress_plugin_deactivation() {

		// Clear Cron
		wp_clear_scheduled_hook( 'bonipress_reset_key' );
		wp_clear_scheduled_hook( 'bonipress_banking_recurring_payout' );
		wp_clear_scheduled_hook( 'bonipress_banking_interest_compound' );
		wp_clear_scheduled_hook( 'bonipress_banking_interest_payout' );

		do_action( 'bonipress_deactivation' );
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonipress_plugin_uninstall' ) ) :
	function bonipress_plugin_uninstall() {

		// Load Installer
		require_once boniPRESS_INCLUDES_DIR . 'bonipress-install.php';
		$installer = bonipress_installer();

		do_action( 'bonipress_before_deletion', $installer );

		// Run uninstaller
		$installer::uninstall();

		do_action( 'bonipress_after_deletion', $installer );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_apply_defaults' ) ) :
	function bonipress_apply_defaults( &$pref, $set ) {
		...
		foreach ( $pref as $key => $value ) {
			...
			if ( array_key_exists( $key, $set ) ) {
				...
				if ( is_array( $value ) && ! empty( $value ) )
					$return[ $key ] = bonipress_apply_defaults( $value, $set[ $key ] );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_strip_tags' ) ) :
	function bonipress_strip_tags( $string = '', $overwride = '' ) {

		$bonipress = bonipress();

		return $bonipress->allowed_tags( $string, $overwrite );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_flush_widget_cache' ) ) :
	function bonipress_flush_widget_cache( $id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_exchange_rates' ) ) :
	function bonipress_get_exchange_rates( $point_type = '' ) {
		...
		$settings = bonipress_get_option( 'bonipress_pref_exchange_' . $point_type, $default );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_ordinal_suffix' ) ) :
	function bonipress_ordinal_suffix( $num = 0, $depreciated = true ) {
		...
		$result = sprintf( _x( '%d th', 'e.g. 5 th', 'bonipress' ), $value );
		if ( $num < 11 || $num > 13 ) {
			switch ( $num % 10 ) {

				case 1 : $result = sprintf( _x( '%d st', 'e.g. 1 st', 'bonipress' ), $value );
				case 2 : $result = sprintf( _x( '%d nd', 'e.g. 2 nd', 'bonipress' ), $value );
				case 3 : $result = sprintf( _x( '%d rd', 'e.g. 3 rd', 'bonipress' ), $value );

			}
		}

		return apply_filters( 'bonipress_ordinal_suffix', $result, $value );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_date_to_gmt_timestamp' ) ) :
	function bonipress_date_to_gmt_timestamp( $string = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_gmt_timestamp_to_local' ) ) :
	function bonipress_gmt_timestamp_to_local( $string = '' ) {
...
if ( ! function_exists( 'bonipress_force_singular_session' ) ) :
	function bonipress_force_singular_session( $user_id = NULL, $key = NULL, $timelimit = BONIPRESS_MIN_TIME_LIMIT ) {
		...
		return apply_filters( 'bonipress_force_singular_session', $force, $user_id, $key, $timelimit );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_locate_template' ) ) :
	function bonipress_locate_template( $template_name, $template_path = 'bonipress', $default_path = '' ) {
		...
		return apply_filters( 'bonipress_locate_template', $template, $template_name, $template_path );
