 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_render_shortcode_hook_table' ) ) :
	function bonips_render_shortcode_hook_table( $atts ) {
		...
		extract( shortcode_atts( array(
			...
			'comment' => '-comment-',
			'amount'  => '',
			'nothing' => __( 'No instances found for this point type', 'bonips' )
		), $atts, BONIPS_SLUG . '_hook_table' ) );

		if ( ! bonips_point_type_exists( $type ) ) return __( 'Point type not found.', 'bonips' );

		$bonips     = bonips( $type );
		$id         = str_replace( '_', '-', $type );
		$prefs_key  = apply_filters( 'bonips_option_id', 'bonips_pref_hooks' );
...
<div class="table-responsive">
	<table class="table bonips-hook-table hook-table-<?php echo $id; ?>">
		<thead>
			<tr>
				<th class="column-instance" style="width: 60%;"><?php _e( 'Instance', 'bonips' ); ?></th>
				<th class="column-amount" style="width: 20%;"><?php _e( 'Amount', 'bonips' ); ?></th>
				<th class="column-limit" style="width: 20%;"><?php _e( 'Limit', 'bonips' ); ?></th>
			</tr>
		</thead>
		<tbody>
<?php
			...
			foreach ( $applicable as $id => $prefs ) {
				...
				$log = $bonips->template_tags_general( $prefs['log'] );
				...
				$log = apply_filters( 'bonips_hook_table_log', $log, $id, $prefs, $atts );
				...
				if( $id == "approved" ) {
					if ( isset( $hooks["hook_prefs"]["comments"]["limits"] ) ) {
						...
						if( (int) $approved_limits["per_post"] > 0 && (int) $approved_limits["per_day"] > 0 ) {
							...
							$limit = sprintf( __( 'Maximal %s mal pro Beitrag und maximal %s mal pro Tag', 'bonips' ), $approved_limits["per_post"],  $approved_limits["per_day"] );
						}
						elseif( (int) $approved_limits["per_post"] > 0 && (int) $approved_limits["per_day"] < 1 ) {
	
							$limit = sprintf( __( 'Maximal %s Mal pro Beitrag', 'bonips' ), $approved_limits["per_post"] );
						}
						elseif( (int) $approved_limits["per_post"] < 1 && (int) $approved_limits["per_day"] > 0 ) {
	
							$limit = sprintf( __( 'Maximal %s Mal pro Tag', 'bonips' ), $approved_limits["per_day"] );
						}
						else {
							$limit = __('Kein Limit', 'bonips');
						}
					}   
				}
				else {
					$limit = bonips_translate_limit_code( $limit, $id, $bonips );
				}

				$creds = apply_filters( 'bonips_hook_table_creds', $bonips->format_creds( $prefs['creds'] ), $id, $prefs, $atts );
...
<?php
		...
		return apply_filters( 'bonips_render_hook_table', $content, $atts );

	}
endif;
add_shortcode( BONIPS_SLUG . '_hook_table', 'bonips_render_shortcode_hook_table' );
