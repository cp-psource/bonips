 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonipress_render_shortcode_best_user' ) ) :
	function bonipress_render_shortcode_best_user( $attr, $content = '' ) {
		...
		if ( empty( $leaderboard->leaderboard ) )
			return '<p class="bonipress-best-user-no-results text-center">' . $nothing . '</p>';
		...
		if ( empty( $content ) )
			$content = '<div class="bonipress-best-user text-center">%avatar%<h4>%display_name%</h4></div>';

		$content = apply_filters( 'bonipress_best_user_content', $content, $attr );
		...
		return apply_filters( 'bonipress_render_best_user', $content, $leaderboard, $attr );
