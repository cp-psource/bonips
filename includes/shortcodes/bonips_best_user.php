 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_render_shortcode_best_user' ) ) :
	function bonips_render_shortcode_best_user( $attr, $content = '' ) {
		...
		if ( empty( $leaderboard->leaderboard ) )
			return '<p class="bonips-best-user-no-results text-center">' . $nothing . '</p>';
		...
		if ( empty( $content ) )
			$content = '<div class="bonips-best-user text-center">%avatar%<h4>%display_name%</h4></div>';

		$content = apply_filters( 'bonips_best_user_content', $content, $attr );
		...
		return apply_filters( 'bonips_render_best_user', $content, $leaderboard, $attr );

	}
endif;
add_shortcode( BONIPS_SLUG . '_best_user', 'bonips_render_shortcode_best_user' );
