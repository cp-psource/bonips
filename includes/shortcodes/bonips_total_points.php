 ...
 * @version 1.1.2
 */
if ( ! function_exists( 'bonips_render_shortcode_total_points' ) ) :
	function bonips_render_shortcode_total_points( $atts ) {
		...
		if ( $ref == '' && $ref_id == '' && $user_id == '' ) {
			...
			// Add up all balances
			$total = $wpdb->get_var( $wpdb->prepare( "SELECT SUM( meta_value ) FROM {$wpdb->usermeta} WHERE meta_key = %s", bonips_get_meta_key( $type ) ) );
		...
		// Complex
			...
			$wheres   = array();
			$wheres[] = $wpdb->prepare( "ctype = %s", bonips_get_meta_key( $type ) );
			...
			$total   = $wpdb->get_var( "SELECT SUM( creds ) FROM {$bonips_log_table} WHERE {$wheres};" );
		...
		if ( $formatted == 1 )
			return $bonips->format_creds( $total );

		return $total;

	}
endif;
add_shortcode( BONIPS_SLUG . '_total_points', 'bonips_render_shortcode_total_points' );
