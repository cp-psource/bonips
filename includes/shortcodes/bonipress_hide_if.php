 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonipress_render_shortcode_hide_if' ) ) :
	function bonipress_render_shortcode_hide_if( $atts, $content = '' ) {
		...
		if ( $bonipress->exclude_user( $user_id ) ) return do_shortcode( $content );
		...
		$should_hide = apply_filters( 'bonipress_hide_if', $should_hide, $user_id, $atts, $content );

		// Sorry, no show
		if ( $should_hide !== false ) return;

		$content = '<div class="bonipress-hide-this-content">' . $content . '</div>';

		// Return content
		return do_shortcode( apply_filters( 'bonipress_hide_if_render', $content, $user_id, $atts, $content ) );

	}
endif;
add_shortcode( BONIPRESS_SLUG . '_hide_if', 'bonipress_render_shortcode_hide_if' );
