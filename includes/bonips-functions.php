...
/**
 * boniPS_Settings class
 * @see http://codex.bonips.me/classes/bonips_settings/
 * @since 0.1
 * @version 1.6
 */
if ( ! class_exists( 'boniPS_Settings' ) ) :
	class boniPS_Settings {
		...
		public function __construct( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			$this->use_master_template = bonips_override_settings();
			$this->use_central_logging = bonips_centralize_log();

			// Log table
			$this->log_table           = $this->get_log_table();

			// Option ID
			$this->option_id           = 'bonips_pref_core';
			...
			do_action_ref_array( 'bonips_settings', array( &$this ) );
		...
		public function defaults() {
			...
			return array(
				...
				'name'        => array(
					'singular'    => __( 'Punkt', 'bonips' ),
					'plural'      => __( 'Punkte', 'bonips' )
				),
				'before'      => '',
				'after'       => '',
				'caps'        => array(
		...
		public function get_point_type_settings() {
			...
			$settings  = bonips_get_option( $this->option_id, $defaults );

			return apply_filters( 'bonips_get_point_type_settings', $settings, $defaults, $this );
		...
		public function get_log_table() {
			...
			if ( $this->is_multisite && $this->use_central_logging )
				...
				$table_name = wp_cache_get('bonips_log_table_name');
				if( FALSE === $table_name ) {
					$table_name = $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $wp_prefix . 'boniPS_log' ) );
					if( $table_name !== NULL ) {
						wp_cache_set('bonips_log_table_name', $table_name);
					}
				}
				...
				if( $table_name == NULL ) {
					$table_name = $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $wp_prefix . 'bonips_log' ) );
					if( $table_name == NULL ) {
						$table_name = $wp_prefix . 'boniPS_log';
					}
					wp_cache_set('bonips_log_table_name', $table_name);
				}
		...
		public function format_number( $value = NULL ) {
			...
			return apply_filters( 'bonips_format_number', $creds, $value, $this->core );
		...
		public function format_creds( $value = 0, $before = '', $after = '', $force_in = false ) {
			...
			return apply_filters( 'bonips_format_creds', $layout, $value, $this );
		...
		public function round_value( $value = NULL, $up_down = false, $precision = 0 ) {
			...
			return apply_filters( 'bonips_round_value', $value, $original_value, $up_down, $precision );
		...
		public function apply_exchange_rate( $amount = 0, $rate = 1 ) {
			...
			return apply_filters( 'bonips_apply_exchange_rate', $value, $amount, $rate );
		...
		public function parse_template_tags( $log_entry, $content = '' ) {
			...
			$content   = apply_filters( 'bonips_parse_log_entry',              $content, $log_entry );
			$content   = apply_filters( "bonips_parse_log_entry_{$reference}", $content, $log_entry );
			...
			if ( isset( $data['ref_type'] ) || isset( $data['post_type'] ) ) {
				...
				$content = apply_filters( "bonips_parse_tags_{$type}", $content, $log_entry );
		...
		public function template_tags_general( $content = '' ) {
			...
			$content = apply_filters( 'bonips_parse_tags_general', $content );
			...
			$content = str_replace( '%login_url_here%',  wp_login_url( bonips_get_permalink() ), $content );

			// Logout URL
			$content = str_replace( '%logout_url%',      wp_logout_url(), $content );
			$content = str_replace( '%logout_url_here%', wp_logout_url( bonips_get_permalink() ), $content );
		...
		public function template_tags_amount( $content = '', $amount = 0 ) {
			...
			$content = apply_filters( 'bonips_parse_tags_amount', $content, $amount, $this );
		...
		public function template_tags_post( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			if ( !isset( $post->ID ) ) {
				...
				$post_url = bonips_get_permalink( $post );

			}

			// Let others play first
			$content = apply_filters( 'bonips_parse_tags_post', $content, $post, $data );

			// Replace template tags
			$content = str_replace( '%post_title%',      esc_attr( bonips_get_the_title( $post ) ), $content );
		...
		public function template_tags_user( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			// Let others play first
			$content     = apply_filters( 'bonips_parse_tags_user', $content, $user, $data );
			...
			$profile_url = bonips_get_users_profile_url( $user->ID );
		...
		public function template_tags_comment( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			$comment_post_title = __( 'Gelöschter Artikel', 'bonips' );
			...
			if ( $comment === NULL ) {
				...
				$comment_post       = bonips_get_post( $comment->comment_post_ID );
				$comment_url        = bonips_get_permalink( $comment_post );
				$comment_post_title = bonips_get_permalink( $comment_post );

			}

			// Let others play first
			$content = apply_filters( 'bonips_parse_tags_comment', $content, $comment, $data );
		 ...
		 * @filter 'bonips_has_tags'
		 * @filter 'bonips_has_tags_{$type}'
		...
		public function has_tags( $type = '', $tags = '', $content = '' ) {
			...
			$tags = apply_filters( 'bonips_has_tags', $tags, $content );
			$tags = apply_filters( 'bonips_has_tags_' . $type, $tags, $content );
		...
		public function available_template_tags( $available = array(), $custom = '' ) {
			...
			if ( ! empty( $custom ) )
				$custom = ' ' . __( 'und', 'bonips' ) . ': ' . $custom;

			return __( 'Verfügbare Vorlagen-Tags:', 'bonips' ) . ' ' . implode( ', ', $links ) . $custom . '.';
		...
		public function allowed_tags( $data = '', $allow = '' ) {
			...
			return strip_tags( $data, apply_filters( 'bonips_allowed_tags', '<a><br><em><strong><span>' ) );
		...
		public function allowed_html_tags() {
			...
			return apply_filters( 'bonips_allowed_html_tags', array(
		...
		public function get_point_admin_capability() {
			...
			// backwards cap.
			$capability = apply_filters( 'bonips_edit_plugin_cap', $this->caps['plugin'] );
		...
		public function get_point_editor_capability() {
			...
			$capability = apply_filters( 'bonips_edit_creds_cap', $this->caps['creds'] );
		...
		public function in_exclude_list( $user_id = '' ) {
			...
			return apply_filters( 'bonips_is_excluded_list', $result, $user_id );
		...
		public function exclude_user( $user_id = NULL ) {
			...
			// Quick override
			if ( apply_filters( 'bonips_exclude_user', false, $user_id, $this ) === true ) return true;
		...
		public function count_members() {
			...
			$count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( DISTINCT user_id ) FROM {$wpdb->usermeta} WHERE meta_key = %s;", bonips_get_meta_key( $this->cred_id ) ) );
		...
		public function enforce_max( $user_id = NULL, $amount = NULL ) {
			...
			if ( ( $maxium > $this->zero() && $amount > $maxium ) || ( $maxium < $this->zero() && $amount < $maxium ) ) {
				...
				do_action( 'bonips_max_enforced', $user_id, $original_amount, $maxium );
		...
		public function get_users_balance( $user_id = NULL, $point_type = NULL ) {
			...
			global $bonips_types, $bonips_current_account;

			// Point type
			if ( $point_type === NULL || ! array_key_exists( $point_type, $bonips_types ) )
				$point_type = $this->get_point_type_key();

			if ( bonips_is_current_account( $user_id ) && $bonips_current_account->balance[ $point_type ] !== false )
				$balance = $bonips_current_account->balance[ $point_type ]->get( 'current' );

			else
				$balance = bonips_get_user_meta( $user_id, $point_type, '', true );

			if ( $balance == '' ) $balance = $this->zero();

			return $this->number( apply_filters( 'bonips_get_users_cred', $balance, $this, $user_id, $point_type ) );
		...
		public function get_users_total_balance( $user_id = NULL, $point_type = NULL ) {
			...
			if ( BONIPS_ENABLE_TOTAL_BALANCE ) {
				...
				if ( bonips_is_current_account( $user_id ) && $bonips_current_account->balance[ $point_type ] !== false )
					$total_balance = $bonips_current_account->balance[ $point_type ]->get( 'accumulated' );

				else
					$total_balance = bonips_get_user_meta( $user_id, $point_type, '_total', true );

				if ( $total_balance == '' ) {

					$total_balance = bonips_query_users_total( $user_id, $point_type );

					bonips_update_user_meta( $user_id, $point_type, '_total', $total_balance );

				}

			}

			return $this->number( apply_filters( 'bonips_get_users_total_cred', $total_balance, $this, $user_id, $point_type ) );
		...
		public function update_users_balance( $user_id = NULL, $amount = NULL, $point_type = NULL ) {
			...
			// Save new balance
			bonips_update_user_meta( $user_id, $point_type, '', $new_balance );

			// Update the current account object
			if ( bonips_is_current_account( $user_id ) && $bonips_current_account->balance[ $point_type ] !== false )
				$bonips_current_account->balance[ $point_type ]->set( 'current', $new_balance );

			// Let others play
			do_action( 'bonips_update_user_balance', $user_id, $current_balance, $amount, $point_type );
		...
		public function update_users_total_balance( $user_id = NULL, $amount = 0, $point_type = NULL ) {
			...
			if ( bonips_is_current_account( $user_id ) && $bonips_current_account->balance[ $point_type ] !== false )
				$bonips_current_account->balance[ $point_type ]->set( 'accumulated', $total_balance );

			do_action( 'bonips_update_user_total_balance', $total_balance, $user_id, $point_type, $this );
		...
		public function set_users_balance( $user_id = NULL, $new_balance = NULL ) {
			...
			// Clear caches
			bonips_delete_option( 'bonips-cache-total-' . $point_type );

			// Let others play
			do_action( 'bonips_set_user_balance', $user_id, $new_balance, $this );
		...
		public function set_users_total_balance( $user_id = NULL, $new_balance = NULL ) {
			...
			// Update balance
			bonips_update_user_meta( $user_id, $this->get_point_type_key(), '_total', $total_balance );

			if ( bonips_is_current_account( $user_id ) && $bonips_current_account->balance[ $point_type ] !== false )
				$bonips_current_account->balance[ $point_type ]->set( 'accumulated', $total_balance );

			// Let others play
			do_action( 'bonips_set_user_total_balance', $user_id, $total_balance, $this );
		...
		public function add_creds( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $type = NULL ) {
			...
			// Point type
			if ( $type === NULL || ! array_key_exists( $type, $bonips_types ) )
				$type = $this->get_point_type_key();

			// Execution Override
			// Allows us to stop an execution. 
			// excepts a boolean reply
			$execute            = apply_filters( 'bonips_add', true, compact( 'ref', 'user_id', 'amount', 'entry', 'ref_id', 'data', 'type' ), $this );
			...
			if ( $execute === true ) {
				...
				$run_this   = apply_filters( 'bonips_run_this', compact( 'ref', 'user_id', 'amount', 'entry', 'ref_id', 'data', 'type' ), $this );
				...
				if ( BONIPS_ENABLE_TOTAL_BALANCE && BONIPS_ENABLE_LOGGING && ( $run_this['amount'] > 0 || ( $run_this['amount'] < 0 && $run_this['ref'] == 'manual' ) ) ) {
					...
					$is_update_total_balance = apply_filters( 'bonips_update_total_balance', true, $run_this );
			...
			return apply_filters( 'bonips_add_finished', $execute, $run_this, $this );
		...
		public function add_to_log( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $type = NULL ) {
			...
			bonips_update_users_history( $user_id, $type, $ref, $ref_id, $amount );

			// Option to disable logging
			if ( BONIPS_ENABLE_LOGGING ) {

				global $wpdb, $bonips_types;
				...
				$time   = apply_filters( 'bonips_log_time', current_time( 'timestamp' ), $ref, $user_id, $amount, $entry, $ref_id, $data, $type );
				$insert = array(
					'ref'     => $ref,
					'ref_id'  => $ref_id,
					'user_id' => (int) $user_id,
				...
				wp_cache_delete( 'bonips_references' . $type, BONIPS_SLUG );

				delete_transient( 'bonips_log_entries' );

			}

			return apply_filters( 'bonips_new_log_entry_id', $insert_id, $insert, $this );
		...
		public function update_log_entry( $entry_id = NULL, $data = array(), $prep = array() ) {
			...
			do_action( 'bonips_log_entry_updated', $entry_id, $data );
		...
		function has_entry( $reference = NULL, $ref_id = NULL, $user_id = NULL, $data = NULL, $type = NULL ) {
			...
			if ( $user_id !== NULL && bonips_is_current_account( $user_id ) && ! empty( $bonips_current_account->point_type ) && in_array( $type, $bonips_current_account->point_type ) ) {

				if ( isset( $bonips_current_account->balance[ $type ]->history ) && ! empty( $bonips_current_account->balance[ $type ]->history->data ) ) {

					$data = $bonips_current_account->balance[ $type ]->history->data;
			...
			return apply_filters( 'bonips_has_entry', $has_entry, $reference, $ref_id, $user_id, $data, $type );
...
/**
 * boniPS Label
 * Returns the boniPS Label
 * @since 1.3.3
 * @version 1.1
 */
if ( ! function_exists( 'bonips_label' ) ) :
	function bonips_label( $trim = false ) {

		global $bonips_label;

		if ( $bonips_label === NULL )
			$bonips_label = apply_filters( 'bonips_label', BONIPS_DEFAULT_LABEL );

		$name = $bonips_label;
		if ( $trim )
			$name = strip_tags( $bonips_label );
...
/**
 * Get boniPS
 * Returns boniPS's general settings and core functions.
 * Replaces bonips_get_settings()
 * @since 1.4
 * @version 1.1
 */
if ( ! function_exists( 'bonips' ) ) :
	function bonips( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		global $bonips, $current_bonips;

		// Custom point type
		if ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) {

			if ( isset( $current_bonips->cred_id ) && $current_bonips->cred_id === $point_type )
				return $current_bonips;

			$current_bonips = new boniPS_Settings( $point_type );
 ...
 * Returns boniPS's network settings or false if multisite is not enabled.
 * @since 0.1
 * @version 1.2
 */
if ( ! function_exists( 'bonips_get_settings_network' ) ) :
	function bonips_get_settings_network() {
		...
		if ( is_array( $bonips_network ) && ! empty( $bonips_network ) && array_key_exists( 'master', $bonips_network ) )
			return $bonips_network;

		$settings            = ( ( is_multisite() ) ? get_blog_option( get_network()->site_id, 'bonips_network', $defaults ) : $defaults );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_is_main_site' ) ) :
	function bonips_is_main_site( $site_id = NULL ) {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_is_site_blocked' ) ) :
	function bonips_is_site_blocked( $blog_id = NULL ) {
		...
		return apply_filters( 'bonips_is_site_blocked', $blocked, $blog_id );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_override_settings' ) ) :
	function bonips_override_settings() {

		// Not a multisite
		if ( ! is_multisite() ) return false;

		$network_setup = bonips_get_settings_network();

		return apply_filters( 'bonips_mu_override_settings', (bool) $network_setup['master'], $network_setup );
...
if ( ! function_exists( 'bonips_centralize_log' ) ) :
	function bonips_centralize_log() {
		...
		return apply_filters( 'bonips_mu_centralize_log', (bool) $network_setup['central'], $network_setup );
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_get_module' ) ) :
	function bonips_get_module( $module = '', $type = 'solo' ) {

		global $bonips_modules;

		if ( $type == 'solo' ) {

			if ( ! array_key_exists( $module, $bonips_modules['solo'] ) )
				return false;

			return $bonips_modules['solo'][ $module ];

		}

		if ( ! array_key_exists( $type, $bonips_modules['type'] ) )
			return false;

		return $bonips_modules['type'][ $type ][ $module ];
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_get_addon_settings' ) ) :
	function bonips_get_addon_settings( $addon = '', $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		if ( $addon != '' ) {
			...
			if ( isset( $bonips->$addon ) )
				$settings = $bonips->$addon;
		...
		return apply_filters( 'bonips_get_addon_settings', $settings, $addon, $point_type );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_remote' ) ) :
	function bonips_get_remote() {

		$defaults = apply_filters( 'bonips_remote_defaults', array(
			'enabled' => 0,
			'key'     => '',
			'uri'     => 'api-dev',
			'debug'   => 0
		...
		return bonips_apply_defaults( $defaults, bonips_get_option( 'bonips_pref_remote', array() ) );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'is_bonips_ready' ) ) :
	function is_bonips_ready() {

		if ( bonips_is_installed() !== false ) return true;
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_is_installed' ) ) :
	function bonips_is_installed() {

		return bonips_get_option( 'bonips_setup_completed', false );

	}
endif;

/**
 * Maybe Install boniPS Table
 * Check to see if maybe the boniPS table needs to be installed.
 * @since 1.7.6
 * @version 1.0.1
 */
if ( ! function_exists( 'maybe_install_bonips_table' ) ) :
	function maybe_install_bonips_table() {

		// No need to check this if we have disabled logging. Prevent this from being used using AJAX
		if ( ! BONIPS_ENABLE_LOGGING || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) || apply_filters( 'bonips_maybe_install_db', true ) === false ) return;

		global $wpdb, $bonips_log_table;

		// Check if the table exists
		if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $bonips_log_table ) ) != $bonips_log_table ) {

			bonips_install_log( NULL, true );

			do_action( 'bonips_reinstalled_table' );
...
if ( ! function_exists( 'bonips_install_log' ) ) :
	function bonips_install_log( $decimals = NULL, $force = false ) {
		...
		if ( ! $force ) {
			...
			$db_version = bonips_get_option( 'bonips_version_db', false );

			// DB Already installed
			if ( $db_version == boniPS_DB_VERSION ) return true;
		...
		dbDelta( "CREATE TABLE IF NOT EXISTS {$bonips->log_table} ( " . $sql . " ) $collate;" );

		bonips_update_option( 'bonips_version_db', boniPS_DB_VERSION );
...
/**
 * Get Post Types
 * Returns an array of post types that boniPS uses.
 * @since 1.7
 * @version 1.1
 */
if ( ! function_exists( 'get_bonips_post_types' ) ) :
	function get_bonips_post_types() {

		$post_type_keys   = array( 'bonips_badge', 'buycred_payment' );

		// Badges
		$post_type_keys[] = ( defined( 'BONIPS_BADGE_KEY' ) ) ? BONIPS_BADGE_KEY : 'bonips_badge';

		// Coupons
		$post_type_keys[] = ( defined( 'BONIPS_COUPON_KEY' ) ) ? BONIPS_COUPON_KEY : 'bonips_coupon';

		// Ranks
		$post_type_keys[] = ( defined( 'BONIPS_RANK_KEY' ) ) ? BONIPS_RANK_KEY : 'bonips_rank';

		return apply_filters( 'bonips_post_types', $post_type_keys );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_get_user_id' ) ) :
	function bonips_get_user_id( $requested = '' ) {
		...
		return apply_filters( 'bonips_get_user_id', $user_id, $requested );
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonips_get_users_profile_url' ) ) :
	function bonips_get_users_profile_url( $user_id = NULL ) {
		...
		return apply_filters( 'bonips_users_profile_url', $profile_url, $user );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_get_account' ) ) :
	function bonips_get_account( $user_id = NULL ) {

		global $bonips_current_account, $bonips_account;
		...
		if ( bonips_is_current_account( $user_id ) )
			return $bonips_current_account;

		if ( bonips_is_account( $user_id ) ) {

			return $bonips_account;

		}

		$bonips_account = new boniPS_Account( $user_id );

		do_action( 'bonips_get_account' );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_is_account' ) ) :
	function bonips_is_account( $user_id = NULL ) {

		global $bonips_account;

		if ( isset( $bonips_account )
			&& ( $bonips_account instanceof boniPS_Account )
			&& ( $user_id === $bonips_account->user_id ) 
			&& ( $user_id !== NULL )
			&& ( $user_id !== 0 )
		) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_current_account' ) ) :
	function bonips_get_current_account() {

		global $bonips_current_account;

		if ( isset( $bonips_current_account ) && ( $bonips_current_account instanceof boniPS_Account ) )
			return $bonips_current_account;
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_set_current_account' ) ) :
	function bonips_set_current_account( $user_id = NULL ) {

		global $bonips_current_account;

		if ( isset( $bonips_current_account )
			&& ( $bonips_current_account instanceof boniPS_Account )
			&& ( $user_id === $bonips_current_account->user_id ) 
			&& ( $user_id !== NULL )
		) {
		...
		$bonips_current_account = new boniPS_Account( ( ( $user_id === NULL ) ? get_current_user_id() : $user_id ) );

		do_action( 'bonips_set_current_account' );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_is_current_account' ) ) :
	function bonips_is_current_account( $user_id = NULL ) {
 ...
 * @param $name_first (bool) option to replace "boniPS" with the point type name set.
 * @since 1.4
 * @version 1.1
 */
if ( ! function_exists( 'bonips_get_types' ) ) :
	function bonips_get_types( $name_first = false ) {

		global $bonips_types;

		if ( is_array( $bonips_types ) && ! empty( $bonips_types ) )
			$types = $bonips_types;
			...
			$available_types = bonips_get_option( 'bonips_types', array( BONIPS_DEFAULT_TYPE_KEY => bonips_label() ) );
			if ( count( $available_types ) > 1 ) {

				foreach ( $available_types as $type => $label ) {

					if ( $type == BONIPS_DEFAULT_TYPE_KEY )
						$label   = bonips_get_point_type_name( BONIPS_DEFAULT_TYPE_KEY, false );
				...
				if ( $name_first )
					$available_types[ BONIPS_DEFAULT_TYPE_KEY ] = bonips_get_point_type_name( BONIPS_DEFAULT_TYPE_KEY, false );
		...
		return apply_filters( 'bonips_types', $types );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_point_type' ) ) :
	function bonips_get_point_type( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$point_type = sanitize_key( $point_type );

		global $current_bonips_type;

		if ( isset( $current_bonips_type )
			&& ( $current_bonips_type instanceof boniPS_Point_Type )
			&& ( $current_bonips_type->cred_id === $point_type )
		) {

			return $current_bonips_type;

		}

		$current_bonips_type = new boniPS_Point_Type( $point_type );

		do_action( 'bonips_get_point_type', $point_type );
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_get_usable_types' ) ) :
	function bonips_get_usable_types( $user_id = NULL ) {
		...
		if ( is_user_logged_in() || $original_id !== NULL ) {
			...
			global $bonips, $bonips_current_account;

			if ( ! isset( $bonips_current_account->balance ) || empty( $bonips_current_account->balance ) ) {

				$types = bonips_get_types();

				if ( count( $types ) == 1 && ! $bonips->exclude_user( $user_id ) )
					...
					foreach ( $types as $type_id => $type ) {
						...
						if ( $type_id == BONIPS_DEFAULT_TYPE_KEY && ! $bonips->exclude_user( $user_id ) )
							$usable[] = BONIPS_DEFAULT_TYPE_KEY;

						else {

							$custom_type = bonips( $type_id );
			...
			elseif ( ! empty( $bonips_current_account->balance ) ) {

				foreach ( $bonips_current_account->balance as $balance ) {
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_point_type_exists' ) ) :
	function bonips_point_type_exists( $type = NULL ) {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_get_point_type_name' ) ) :
	function bonips_get_point_type_name( $point_type = BONIPS_DEFAULT_TYPE_KEY, $singular = true ) {

		$bonips = bonips( $point_type );

		if ( $singular )
			return $bonips->singular();

		return $bonips->plural();
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_types_select_from_dropdown' ) ) :
	function bonips_types_select_from_dropdown( $name = '', $id = '', $selected = '', $return = false, $extra = '' ) {

		$types  = bonips_get_types();
		$output = '';

		if ( count( $types ) == 1 )
			$output .= '<input type="hidden"' . $extra . ' name="' . $name . '" id="' . $id . '" value="bonips_default" />';
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_types_select_from_checkboxes' ) ) :
	function bonips_types_select_from_checkboxes( $name = '', $id = '', $selected_values = array(), $return = false ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_db_column' ) ) :
	function bonips_get_db_column( $column = '' ) {
		...
		if ( bonips_override_settings() && ! bonips_is_main_site() )
			$table = $wpdb->get_blog_prefix( get_network()->site_id ) . $column;

		return apply_filters( 'bonips_get_db_column', $table );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_add_option' ) ) :
	function bonips_add_option( $option_id, $value = '' ) {

		if ( is_multisite() ) {

			// Master template enabled
			if ( bonips_override_settings() )
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonips_get_option' ) ) :
	function bonips_get_option( $option_id, $default = array() ) {
		...
		$get_option_id = apply_filters( 'bonips_get_option_id', $option_id );
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonips_update_option' ) ) :
	function bonips_update_option( $option_id, $value = '' ) {
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_delete_option' ) ) :
	function bonips_delete_option( $option_id ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_meta_key' ) ) :
	function bonips_get_meta_key( $key = '', $end = '' ) {

		if ( is_multisite() ) {

			$blog_id = get_current_blog_id();

			if ( $blog_id > 1 && ! bonips_centralize_log() && $key != 'bonips_rank' )
				$key .= '_' . $blog_id;

			elseif ( $blog_id > 1 && ! bonips_override_settings() && $key == 'bonips_rank' )
				$key .= '_' . $blog_id;
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_add_user_meta' ) ) :
	function bonips_add_user_meta( $user_id, $key = '', $end = '', $value = '', $unique = false ) {

		$key = bonips_get_meta_key( $key, $end );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_get_user_meta' ) ) :
	function bonips_get_user_meta( $user_id, $key = '', $end = '', $unique = false ) {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_update_user_meta' ) ) :
	function bonips_update_user_meta( $user_id, $key = '', $end = '', $value = '', $previous = '' ) {
 ...
 * @version 1.1.1
 */
if ( ! function_exists( 'bonips_delete_user_meta' ) ) :
	function bonips_delete_user_meta( $user_id, $key = '', $end = '', $value = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_add_post_meta' ) ) :
	function bonips_add_post_meta( $post_id, $key = '', $value = '', $unique = false ) {

		$override = ( bonips_override_settings() && ! bonips_is_main_site() );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_post_meta' ) ) :
	function bonips_get_post_meta( $post_id, $key = '', $unique = false ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_update_post_meta' ) ) :
	function bonips_update_post_meta( $post_id, $key = '', $value = '', $previous = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_delete_post_meta' ) ) :
	function bonips_delete_post_meta( $post_id, $key = '', $value = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_post' ) ) :
	function bonips_get_post( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_permalink' ) ) :
	function bonips_get_permalink( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_post_type' ) ) :
	function bonips_get_post_type( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_the_title' ) ) :
	function bonips_get_the_title( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_page_by_title' ) ) :
	function bonips_get_page_by_title( $post_id, $type, $post_type ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_trash_post' ) ) :
	function bonips_trash_post( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_attachment_url' ) ) :
	function bonips_get_attachment_url( $post_id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_delete_post' ) ) :
	function bonips_delete_post( $post_id = NULL, $force = false ) {
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_is_admin' ) ) :
	function bonips_is_admin( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$bonips = bonips( $point_type );

		if ( $bonips->user_is_point_admin( $user_id ) || $bonips->user_is_point_editor( $user_id ) )
			return true;
...
/**
 * Exclude User
 * Checks if a given user is excluded from using boniPS.
 * @param $user_id (int), optional user to check, defaults to current user
 * @since 0.1
 * @version 1.2
 */
if ( ! function_exists( 'bonips_exclude_user' ) ) :
	function bonips_exclude_user( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$bonips = bonips( $point_type );

		return $bonips->exclude_user( $user_id );
 ...
 * @version 1.1
 */ 
if ( ! function_exists( 'bonips_get_users_balance' ) ) :
	function bonips_get_users_balance( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$bonips = bonips( $point_type );

		if ( $bonips->exclude_user( $user_id ) ) return false;

		return $bonips->get_users_balance( $user_id, $point_type );

	}
endif;
// Depreciated
if ( ! function_exists( 'bonips_get_users_cred' ) ) :
	function bonips_get_users_cred( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		return bonips_get_users_balance( $user_id, $point_type );
 ...
 * @version 1.0
 */ 
if ( ! function_exists( 'bonips_get_users_total_balance' ) ) :
	function bonips_get_users_total_balance( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$bonips = bonips( $point_type );

		if ( $bonips->exclude_user( $user_id ) ) return false;

		return $bonips->get_users_total_balance( $user_id, $point_type );
 ...
 * @version 1.3
 */
if ( ! function_exists( 'bonips_display_users_balance' ) ) :
	function bonips_display_users_balance( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		return $bonips->format_creds( $balance );

	}
endif;
// Depreciated
if ( ! function_exists( 'bonips_get_users_fcred' ) ) :
	function bonips_get_users_fcred( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		return bonips_display_users_balance( $user_id, $point_type );
 ...
 * @version 1.1
 */ 
if ( ! function_exists( 'bonips_display_users_total_balance' ) ) :
	function bonips_display_users_total_balance( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$bonips  = bonips( $point_type );

		if ( $bonips->exclude_user( $user_id ) ) return '';

		$balance = $bonips->get_users_total_balance( $user_id, $point_type );
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_format_number' ) ) :
	function bonips_format_number( $value = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		if ( $value === NULL || ! is_numeric( $value ) ) return $value;

		$bonips = bonips( $point_type );

		return $bonips->format_number( $value );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_format_points' ) ) :
	function bonips_format_points( $value = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		if ( $value === NULL || ! is_numeric( $value ) ) return $value;

		$bonips = bonips( $point_type );

		return $bonips->format_creds( $value );

	}
endif;
// Depreciated
if ( ! function_exists( 'bonips_format_creds' ) ) :
	function bonips_format_creds( $value = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		return bonips_format_points( $value, $point_type );
 ...
 * @version 1.3
 */
if ( ! function_exists( 'bonips_add' ) ) :
	function bonips_add( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		return $bonips->add_creds( $ref, $user_id, $amount, $entry, $ref_id, $data, $point_type );
...
/**
 * Subtract Creds
 * Subtracts creds from a given user. Works just as bonips_add() but the creds are converted into a negative value.
 * @uses bonips_add()
 * @since 0.1
 * @version 1.1.1
 */
if ( ! function_exists( 'bonips_subtract' ) ) :
	function bonips_subtract( $ref = '', $user_id = '', $amount = '', $entry = '', $ref_id = '', $data = '', $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		if ( $ref == '' || $user_id == '' || $amount == '' ) return false;
		if ( $amount > 0 ) $amount = 0 - $amount;

		return bonips_add( $ref, $user_id, $amount, $entry, $ref_id, $data, $point_type );
 ...
 * @version 1.1.1
 */
if ( ! function_exists( 'bonips_plugin_activation' ) ) :
	function bonips_plugin_activation() {

		// Load Installer
		require_once boniPS_INCLUDES_DIR . 'bonips-install.php';
		$installer = bonips_installer();

		// Compatibility check
		$installer::compat();

		// First time activation
		if ( get_option( 'bonips_version', false ) === false )
			$installer::activate();
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_plugin_deactivation' ) ) :
	function bonips_plugin_deactivation() {

		// Clear Cron
		wp_clear_scheduled_hook( 'bonips_reset_key' );
		wp_clear_scheduled_hook( 'bonips_banking_recurring_payout' );
		wp_clear_scheduled_hook( 'bonips_banking_interest_compound' );
		wp_clear_scheduled_hook( 'bonips_banking_interest_payout' );

		do_action( 'bonips_deactivation' );
 ...
 * @version 1.0.2
 */
if ( ! function_exists( 'bonips_plugin_uninstall' ) ) :
	function bonips_plugin_uninstall() {

		// Load Installer
		require_once boniPS_INCLUDES_DIR . 'bonips-install.php';
		$installer = bonips_installer();

		do_action( 'bonips_before_deletion', $installer );

		// Run uninstaller
		$installer::uninstall();

		do_action( 'bonips_after_deletion', $installer );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_apply_defaults' ) ) :
	function bonips_apply_defaults( &$pref, $set ) {
		...
		foreach ( $pref as $key => $value ) {
			...
			if ( array_key_exists( $key, $set ) ) {
				...
				if ( is_array( $value ) && ! empty( $value ) )
					$return[ $key ] = bonips_apply_defaults( $value, $set[ $key ] );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_strip_tags' ) ) :
	function bonips_strip_tags( $string = '', $overwride = '' ) {

		$bonips = bonips();

		return $bonips->allowed_tags( $string, $overwrite );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_flush_widget_cache' ) ) :
	function bonips_flush_widget_cache( $id = NULL ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_exchange_rates' ) ) :
	function bonips_get_exchange_rates( $point_type = '' ) {
		...
		$settings = bonips_get_option( 'bonips_pref_exchange_' . $point_type, $default );
...
if ( ! function_exists( 'bonips_translate_limit_code' ) ) :
	function bonips_translate_limit_code( $id, $bonips, $code = '' ) {
		...
		if ( $code == '0/x' || $code == 0 )
			return __( 'Kein Limit', 'bonips' );

		$result = '-';
		$check  = explode( '/', $code );
		if ( count( $check ) == 2 ) {

			$per    = __( 'in Summe', 'bonips' );
			if ( $check[1] == 'd' )
				$per = __( 'pro Tag', 'bonips' );

			elseif ( $check[1] == 'w' )
				$per = __( 'pro Woche', 'bonips' );

			elseif ( $check[1] == 'm' )
				$per = __( 'pro Monat', 'bonips' );

			$result = sprintf( _n( 'Maximal einmal', 'Maximal %d mal', $check[0], 'bonips' ), $check[0] ) . ' ' . $per;

		}

		elseif ( is_numeric( $code ) ) {

			$result = sprintf( _n( 'Maximal einmal', 'Maximal %d mal', $code, 'bonips' ), $code );

		}

		return apply_filters( 'bonips_translate_limit_code', $result, $code, $id, $bonips );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_ordinal_suffix' ) ) :
	function bonips_ordinal_suffix( $num = 0, $depreciated = true ) {
		...
		$result = sprintf( _x( '%d th', 'e.g. 5 th', 'bonips' ), $value );
		if ( $num < 11 || $num > 13 ) {
			switch ( $num % 10 ) {

				case 1 : $result = sprintf( _x( '%d st', 'e.g. 1 st', 'bonips' ), $value );
				case 2 : $result = sprintf( _x( '%d nd', 'e.g. 2 nd', 'bonips' ), $value );
				case 3 : $result = sprintf( _x( '%d rd', 'e.g. 3 rd', 'bonips' ), $value );

			}
		}

		return apply_filters( 'bonips_ordinal_suffix', $result, $value );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_date_to_gmt_timestamp' ) ) :
	function bonips_date_to_gmt_timestamp( $string = '' ) {
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_gmt_timestamp_to_local' ) ) :
	function bonips_gmt_timestamp_to_local( $string = '' ) {
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_force_singular_session' ) ) :
	function bonips_force_singular_session( $user_id = NULL, $key = NULL, $timelimit = BONIPS_MIN_TIME_LIMIT ) {
		...
		return apply_filters( 'bonips_force_singular_session', $force, $user_id, $key, $timelimit );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_locate_template' ) ) :
	function bonips_locate_template( $template_name, $template_path = 'bonips', $default_path = '' ) {
		...
		return apply_filters( 'bonips_locate_template', $template, $template_name, $template_path );
