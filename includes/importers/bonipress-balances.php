 ...
 * @version 1.3
 */
if ( ! class_exists( 'boniPRESS_Importer_Balances' ) ) :
	class boniPRESS_Importer_Balances extends WP_Importer {
		...
		public function handle_upload() {
			...
			if ( empty( $_POST['file_url'] ) ) {
				...
				if ( file_exists( ABSPATH . $_POST['file_url'] ) ) {
					...
					echo '<div class="error notice notice-error is-dismissible"><p>' . __( 'The file does not exist or could not be read.', 'bonipress' ) . '</p></div>';
					return false;
			...
			if ( ( $handle = fopen( $file, "r" ) ) !== false ) {
				...
				if ( $no_of_columns == 3 || $no_of_columns == 4 ) {
					...
					while ( ( $row = fgetcsv( $handle, 0, $this->delimiter ) ) !== false ) {
						...
						$user_id = bonipress_get_user_id( $identification );
						...
						if ( ! empty( $log_entry ) )
							$bonipress->add_to_log( 'import', $user_id, $balance, $log_entry );

						// Add to the balance
						if ( $method == 'add' )
							$bonipress->update_users_balance( $user_id, $balance );
					...
					echo '<div class="error notice notice-error is-dismissible"><p>' . __( 'Invalid CSV file. Please consult the documentation for further assistance.', 'bonipress' ) . '</p></div>';
...
<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr( wp_nonce_url( $action_url, 'import-upload' ) ); ?>">
	<table class="form-table">
		<tbody>
			<tr>
				<th>
					<label for="upload"><?php _e( 'Choose a file from your computer:', 'bonipress' ); ?></label>
				</th>
				<td>
					<input type="file" id="upload" name="import" size="25" />
					<input type="hidden" name="action" value="save" />
					<input type="hidden" name="max_file_size" value="<?php echo $bytes; ?>" />
					<small><?php printf( __( 'Maximum size: %s', 'bonipress' ), $size ); ?></small>
				</td>
			</tr>
			<tr>
				<th>
					<label for="file_url"><?php _e( 'OR enter path to file:', 'bonipress' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="delimiter"><?php _e( 'Delimiter', 'bonipress' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label><?php _e( 'Method', 'bonipress' ); ?></label>
				</th>
				<td>
					<select name="method">
						<option value=""><?php _e( 'Replace current balances with the amount in this CSV file', 'bonipress' ); ?></option>
						<option value="add"><?php _e( 'Adjust current balances according to the amount in this CSV file', 'bonipress' ); ?></option>
					</select>
				</td>
			</tr>
		</tbody>
