 ...
 * @version 1.3
 */
if ( ! class_exists( 'boniPS_Importer_Balances' ) ) :
	class boniPS_Importer_Balances extends WP_Importer {
		...
		public function __construct() {
			...
			$this->documentation = 'http://codex.bonips.me/chapter-ii/import-data/importing-balances/';
			...
			if ( ( $handle = fopen( $file, "r" ) ) !== false ) {
				...
				if ( $no_of_columns == 3 || $no_of_columns == 4 ) {
					...
					while ( ( $row = fgetcsv( $handle, 0, $this->delimiter ) ) !== false ) {
						...
						if ( ! empty( $log_entry ) )
							$bonips->add_to_log( 'import', $user_id, $balance, $log_entry );

						// Add to the balance
						if ( $method == 'add' )
							$bonips->update_users_balance( $user_id, $balance );
		...
		public function header() {
			...
			$label = __( 'Import Balances', 'bonips' );
...
<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr( wp_nonce_url( $action_url, 'import-upload' ) ); ?>">
	<table class="form-table">
		<tbody>
			...
			<tr>
				<th>
					<label><?php _e( 'Method', 'bonips' ); ?></label>
				</th>
				<td>
					<select name="method">
						<option value=""><?php _e( 'Replace current balances with the amount in this CSV file', 'bonips' ); ?></option>
						<option value="add"><?php _e( 'Adjust current balances according to the amount in this CSV file', 'bonips' ); ?></option>
					</select>
				</td>
			</tr>
		</tbody>
