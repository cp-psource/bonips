...
/**
 * boniPS_Account class
 * @see http://codex.bonips.me/objects/bonips_account/
 * @since 1.7
 * @version 1.1
 */
if ( ! class_exists( 'boniPS_Account' ) ) :
	class boniPS_Account extends boniPS_Object {
		...
		protected function populate( $fresh = false ) {
			...
			if ( ! empty( $point_types ) ) {
				foreach ( $point_types as $type_id => $label ) {
					...
					if ( $bonips->exclude_user( $this->user_id ) ) {
		...
		public function get_balance( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			if ( ! empty( $this->balance ) && array_key_exists( $point_type, $this->balance ) )
				...
				$balance = new boniPS_Balance( $this->user_id, $point_type );
...
/**
 * boniPS_Balance class
 * @see http://codex.bonips.me/objects/bonips_balance/
 * @since 1.7
 * @version 1.1
 */
if ( ! class_exists( 'boniPS_Balance' ) ) :
	class boniPS_Balance extends boniPS_Object {
		...
		protected function populate( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			$this->set( 'current',     $bonips->get_users_balance( $user_id ) );
			$this->set( 'accumulated', $bonips->get_users_total_balance( $user_id ) );

			$this->set( 'point_type',  new boniPS_Point_Type( $point_type ) );
...
/**
 * boniPS_Point_Type class
 * @see http://codex.bonips.me/objects/bonips_point_type/
 * @since 1.7
 * @version 1.1
 */
if ( ! class_exists( 'boniPS_Point_Type' ) ) :
	class boniPS_Point_Type extends boniPS_Object {
		...
		protected function populate( $type_id = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			$this->set( 'singular',     $bonips->singular() );
			$this->set( 'plural',       $bonips->plural() );
			$this->set( 'prefix',       $bonips->before );
			$this->set( 'suffix',       $bonips->after );
			$this->set( 'format',       $bonips->format );
		...
		public function number( $number = '' ) {
			...
			return apply_filters( 'bonips_type_number', $result, $number, $this );
		...
		public function format( $number = '' ) {
			...
			return apply_filters( 'bonips_type_format', $prefix . $number . $suffix, $number, $this );
...
/**
 * boniPS_History class
 * @see http://codex.bonips.me/objects/bonips_history-2/
 * @since 1.8
 * @version 1.0
 */
if ( ! class_exists( 'boniPS_History' ) ) :
	class boniPS_History extends boniPS_Object {
		...
		protected function populate( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			$this->data = bonips_get_users_history( $user_id, $point_type );
			if ( ! empty( $this->data ) ) {
