...
if ( ! class_exists( 'boniPS_Query_Log' ) ) :
	class boniPS_Query_Log {
		...
		public function __construct( $args = array(), $array = false ) {
			...
			$this->args = bonips_apply_defaults( $defaults, $args );
			...
			if ( $this->args['cache'] != '' ) {
				...
				if ( is_multisite() )
					$data = get_site_transient( 'bonips_log_query_' . $cache_id );
				else
					$data = get_transient( 'bonips_log_query_' . $cache_id );
			}
			if ( $data === false ) {
				...
				if ( $this->args['orderby'] != '' ) {
					...
					$allowed = apply_filters( 'bonips_allowed_sortby', $sortbys );
				...
				// Filter
				$select = apply_filters( 'bonips_query_log_select', $select, $this->args, $this->core );
				$sortby = apply_filters( 'bonips_query_log_sortby', $sortby, $this->args, $this->core );
				$limits = apply_filters( 'bonips_query_log_limits', $limits, $this->args, $this->core );
				$wheres = apply_filters( 'bonips_query_log_wheres', $wheres, $this->args, $this->core );

				$prep = apply_filters( 'bonips_query_log_prep', $prep, $this->args, $this->core );
				...
				if ( $this->args['cache'] != '' ) {
					if ( is_multisite() )
						set_site_transient( 'bonips_log_query_' . $cache_id, $this->results, DAY_IN_SECONDS * 1 );
					else
						set_transient( 'bonips_log_query_' . $cache_id, $this->results, DAY_IN_SECONDS * 1 );
				}
...
<?php
		...
		public function pagination( $location = 'top', $id = '' ) {
			...
			if ( $this->have_entries() ) {
				...
				$output = '<span class="displaying-num">' . sprintf( __( '%d %s werden angezeigt', 'bonips' ), $total_number, _n( 'Eintrag', 'Einträge', $total_number, 'bonips' ) ) . '</span>';
			}
		...
		public function get_refs( $req = array() ) {
			$refs = bonips_get_used_references( $this->args['ctype'] );
			...
			return apply_filters( 'bonips_log_get_refs', $refs );
		}
		...
		protected function get_users() {
			$users = wp_cache_get( 'bonips_users' );
			...
			if ( false === $users ) {
				...
				wp_cache_set( 'bonips_users', $users );
			}

			return apply_filters( 'bonips_log_get_users', $users );
		}
		...
		public function filter_options( $is_profile = false, $refs = array() ) {
			...
			if ( $this->core->can_edit_creds() && ! $is_profile && $this->num_rows > 0 ) {
				echo '<input type="text" class="form-control" name="user_id" id="boniPS-user-filter" size="12" placeholder="' . __( 'Benutzer ID', 'bonips' ) . '" value="' . ( ( isset( $_GET['user_id'] ) ) ? $_GET['user_id'] : '' ) . '" /> ';
				$show = true;
			}
			...
			if ( $this->num_rows > 0 ) {
				...
				$options = array( 'ASC' => __( 'Aufsteigend', 'bonips' ), 'DESC' => __( 'Absteigend', 'bonips' ) );
			...
			if ( $show === true )
				echo '<input type="submit" class="btn btn-default button button-secondary button-large" value="' . __( 'Filter', 'bonips' ) . '" />';
		...
		public function exporter( $title = '', $is_profile = false ) {
			...
			if ( ! apply_filters( 'bonips_user_can_export', false ) && ! $this->core->can_edit_creds() ) return;

			// Check if we allow export from front end. Disallowed by default
			if ( ! apply_filters( 'bonips_allow_front_export', false ) && ! is_admin() ) return;
...
<div style="display:none;" class="clear" id="export-log-history">
	...
	<form action="<?php echo add_query_arg( array( 'bonips-export' => 'do' ) ); ?>" method="post">
		<input type="hidden" name="token" value="<?php echo wp_create_nonce( 'bonips-run-log-export' ); ?>" />
<?php
			if ( ! empty( $exports ) ) {
...
<?php
		...
		public function table_headers() {
			...
			return apply_filters( 'bonips_log_column_headers', array(
				'column-username' => __( 'Benutzer', 'bonips' ),
				'column-time'     => __( 'Datum', 'bonips' ),
				'column-creds'    => $this->core->plural(),
				'column-entry'    => __( 'Eintrag', 'bonips' )
			), $this );
		}
		...
		public function get_display() {
			$output = '
<table class="table bonips-table widefat log-entries table-striped" cellspacing="0">
	<thead>
		<tr>';
	...
	<tbody id="the-list">';
			...
			if ( $this->have_entries() ) {
				...
				foreach ( $this->results as $log_entry ) {
					$row_class = apply_filters( 'bonips_log_row_classes', array( 'boniPS-log-row' ), $log_entry );
			...
			foreach ( $this->headers as $column_id => $column_name ) {
				switch ( $column_id ) {
					...
					case 'column-username' :
						...
						if ( $user === false )
							$content = '<span>' . __( 'Benutzer fehlt', 'bonips' ) . ' (ID: ' . $log_entry->user_id . ')</span>';
						else
					...
					case 'column-entry' :
						...
						$content = '<div class="bonips-mobile-log" style="display:none;">' . $time . '<div>' . $creds . '</div></div>';
					...
					default :
						...
						$content = apply_filters( 'bonips_log_' . $column_id, '', $log_entry );
		...
		public function mobile_support() {
			echo '<style type="text/css">' . apply_filters( 'bonips_log_mobile_support', '
@media all and (max-width: 480px) {
	.column-time, .column-creds { display: none; }
	.bonips-mobile-log { display: block !important; }
	.bonips-mobile-log div { float: right; font-weight: bold; }
}
' ) . '</style>';
		}
		...
		public function search() {
			...
			<p class="search-box">
				<label class="screen-reader-text" for=""><?php _e( 'Suchprotokoll', 'bonips' ); ?>:</label>
				<input type="search" name="s" value="<?php echo $serarch_string; ?>" placeholder="<?php _e( 'Logeinträge suchen', 'bonips' ); ?>" />
				<input type="submit" name="bonips-search-log" id="search-submit" class="button button-medium button-secondary" value="<?php _e( 'Suche Log', 'bonips' ); ?>" />
			</p>
<?php
		}
