...
if ( ! class_exists( 'boniPRESS_Query_Log' ) ) :
	class boniPRESS_Query_Log {
		...
		public function __construct( $args = array(), $array = false ) {
			...
			$this->args = bonipress_apply_defaults( $defaults, $args );
			...
			if ( $this->args['cache'] != '' ) {
				...
				if ( is_multisite() )
					$data = get_site_transient( 'bonipress_log_query_' . $cache_id );
				else
					$data = get_transient( 'bonipress_log_query_' . $cache_id );
			}
			if ( $data === false ) {
				...
				if ( $this->args['orderby'] != '' ) {
					...
					$allowed = apply_filters( 'bonipress_allowed_sortby', $sortbys );
				...
				// Filter
				$select = apply_filters( 'bonipress_query_log_select', $select, $this->args, $this->core );
				$sortby = apply_filters( 'bonipress_query_log_sortby', $sortby, $this->args, $this->core );
				$limits = apply_filters( 'bonipress_query_log_limits', $limits, $this->args, $this->core );
				$wheres = apply_filters( 'bonipress_query_log_wheres', $wheres, $this->args, $this->core );

				$prep = apply_filters( 'bonipress_query_log_prep', $prep, $this->args, $this->core );
				...
				if ( $this->args['cache'] != '' ) {
					if ( is_multisite() )
						set_site_transient( 'bonipress_log_query_' . $cache_id, $this->results, DAY_IN_SECONDS * 1 );
					else
						set_transient( 'bonipress_log_query_' . $cache_id, $this->results, DAY_IN_SECONDS * 1 );
				}
...
<?php
		...
		public function get_refs( $req = array() ) {
			$refs = bonipress_get_used_references( $this->args['ctype'] );
			...
			return apply_filters( 'bonipress_log_get_refs', $refs );
		}
		...
		protected function get_users() {
			$users = wp_cache_get( 'bonipress_users' );
			...
			if ( false === $users ) {
				...
				wp_cache_set( 'bonipress_users', $users );
			}

			return apply_filters( 'bonipress_log_get_users', $users );
		}
		...
		public function filter_options( $is_profile = false, $refs = array() ) {
			...
			if ( $show === true )
				echo '<input type="submit" class="btn btn-default button button-secondary button-large" value="' . __( 'Filter', 'bonipress' ) . '" />';
		...
		public function exporter( $title = '', $is_profile = false ) {
			...
			if ( ! apply_filters( 'bonipress_user_can_export', false ) && ! $this->core->can_edit_creds() ) return;

			// Check if we allow export from front end. Disallowed by default
			if ( ! apply_filters( 'bonipress_allow_front_export', false ) && ! is_admin() ) return;
...
<div style="display:none;" class="clear" id="export-log-history">
	...
	<form action="<?php echo add_query_arg( array( 'bonipress-export' => 'do' ) ); ?>" method="post">
		<input type="hidden" name="token" value="<?php echo wp_create_nonce( 'bonipress-run-log-export' ); ?>" />
<?php
			if ( ! empty( $exports ) ) {
...
<?php
		...
		public function get_display() {
			$output = '
<table class="table bonipress-table widefat log-entries table-striped" cellspacing="0">
	<thead>
		<tr>';
	...
	<tbody id="the-list">';
			...
			if ( $this->have_entries() ) {
				...
				foreach ( $this->results as $log_entry ) {
					$row_class = apply_filters( 'bonipress_log_row_classes', array( 'boniPRESS-log-row' ), $log_entry );
			...
			foreach ( $this->headers as $column_id => $column_name ) {
				switch ( $column_id ) {
					...
					case 'column-entry' :
						...
						$content = '<div class="bonipress-mobile-log" style="display:none;">' . $time . '<div>' . $creds . '</div></div>';
					...
					default :
						...
						$content = apply_filters( 'bonipress_log_' . $column_id, '', $log_entry );
		...
		public function mobile_support() {
			echo '<style type="text/css">' . apply_filters( 'bonipress_log_mobile_support', '
@media all and (max-width: 480px) {
	.column-time, .column-creds { display: none; }
	.bonipress-mobile-log { display: block !important; }
	.bonipress-mobile-log div { float: right; font-weight: bold; }
}
' ) . '</style>';
		}
