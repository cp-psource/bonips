...
/**
 * Get boniPS Leaderboard
 * Returns the boniPS_Leaderboard object containing results.
 *
...
if ( ! function_exists( 'bonips_get_leaderboard' ) ) :
	function bonips_get_leaderboard( $args = array(), $reset = false, $raw = false )
	{
		$default = array(
			'number'      => '-1',
			'order'       => 'DESC',
		...
		return new boniPS_Query_Leaderboard( $args );
	}
endif;
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_leaderboard_position' ) ) :
	function bonips_leaderboard_position( $user_id = '', $type = 'bonips_default' )
	{
		$args = array( 'type' => $type );
		$rankings = bonips_get_leaderboard( $args);
		$leaderboard = new boniPS_Leaderboard( $args, $rankings->get_results() );
		return apply_filters( 'bonips_get_leaderboard_position', $leaderboard->users_position( $user_id ), $user_id, $type );
	}
endif;

/**
 * boniPS Query Leaderboard Class
 ...
 * @see http://codex.bonips.me/classes/bonips_leaderboard/
 * @since 1.4
 * @version 1.0
 */
if ( ! class_exists( 'boniPS_Leaderboard' ) ) :
	class boniPS_Leaderboard {
		...
		public function __construct( $args = array(), $results = array() ) {
			if ( ! isset( $args['type'] ) || empty( $args['type'] ) )
				$args['type'] = 'bonips_default';
			
			// Get settings
			$bonips = bonips( $args['type'] );
			$this->core = $bonips;
			$this->args = $args;
			$this->result = $results;
		}
