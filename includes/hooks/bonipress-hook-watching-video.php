 ...
 * @version 1.1
 */
if ( ! class_exists( 'boniPRESS_Hook_Video_Views' ) ) :
	class boniPRESS_Hook_Video_Views extends boniPRESS_Hook {
		...
		public function run() {
			...
			add_action( 'wp_ajax_bonipress-viewing-videos', array( $this, 'ajax_call_video_points' ) );

			add_filter( 'bonipress_video_js', array( $this, 'adjust_js' ) );
			add_filter( 'bonipress_video_view_' . $this->bonipress_type, array( $this, 'video_view' ), 10, 2 );
			...
			switch ( $logic ) {
				...
				case 'play' :
					...
					if ( $state == 1 ) {
						...
						if ( ! $this->has_entry( 'watching_video', '', $user_id, $video_id, $this->bonipress_type ) ) {
				...
				case 'interval' :
					...
					if ( $state == 1 && $users_log === NULL ) {
						...
						// Add points without using bonipress_add to prevent
					...
					elseif ( $state == 0 && isset( $users_log->creds ) && $users_log->creds+$amount <= $max ) {
						...
						if ( function_exists( 'bonipress_add_new_notice' ) ) {
							...
							if ( ! empty( $message ) )
								bonipress_add_new_notice( array( 'user_id' => $user_id, 'message' => $message, 'color' => $color ) );
		...
		public function get_users_video_log( $video_id, $user_id ) {
			...
			return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bonipress_log_table} WHERE user_id = %d AND data = %s AND ctype = %s;", $user_id, $video_id, $this->bonipress_type ) );
...
<div class="hook-instance">
	...
	<div class="row">
		<div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"><?php _e( 'Award Logic', 'bonipress' ); ?></label>
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"<?php checked( $prefs['logic'], 'play' ); ?> value="play" class="toggle-hook-option" /> <?php _e( 'Play - As soon as video starts playing.', 'bonipress' ); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'logic' => 'full' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'full' ) ); ?>"<?php checked( $prefs['logic'], 'full' ); ?> value="full" class="toggle-hook-option" /> <?php _e( 'Full - First when the entire video has played.', 'bonipress' ); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'logic' => 'interval' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'interval' ) ); ?>"<?php checked( $prefs['logic'], 'interval' ); ?> value="interval" class="toggle-hook-option" /> <?php echo $this->core->template_tags_general( __( 'Interval - For each x number of seconds watched.', 'bonipress' ) ); ?></label>
				</div>
			</div>
		</div>
		<div class="col-lg-5 col-md-6 col-sm-12 col-xs-12">
			<div id="<?php echo $this->field_id( array( 'logic-option-interval' ) ); ?>"<?php if ( $prefs['logic'] != 'interval' ) echo ' style="display: none;"';?>>
				<div class="form-group">
					<label for="<?php echo $this->field_id( 'interval' ); ?>"><?php _e( 'Intervals', 'bonipress' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( 'interval' ); ?>" id="<?php echo $this->field_id( 'interval' ); ?>" placeholder="<?php _e( 'required', 'bonipress' ); ?>" value="<?php echo esc_attr( $prefs['interval'] ); ?>" class="form-control" />
					<span class="description"><?php printf( __( 'The number of seconds a user must watch in order to get %s.', 'bonipress' ), $this->core->plural() ); ?></span>
				</div>
			</div>
			<div id="<?php echo $this->field_id( array( 'logic-option-full' ) ); ?>"<?php if ( $prefs['logic'] != 'full' ) echo ' style="display: none;"';?>>
				<div class="form-group">
					<label for="<?php echo $this->field_id( 'leniency' ); ?>"><?php _e( 'Leniency', 'bonipress' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( 'leniency' ); ?>" id="<?php echo $this->field_id( 'leniency' ); ?>" placeholder="<?php _e( 'required', 'bonipress' ); ?>" value="<?php echo esc_attr( $prefs['leniency'] ); ?>" class="form-control" />
					<span class="description"><?php _e( 'Do not set this value to zero! A lot of thing can happen while a user watches a movie and sometimes a few seconds can drop of the counter due to buffering or play back errors.', 'bonipress' ); ?></span>
				</div>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label><?php _e( 'Available Shortcode', 'bonipress' ); ?></label>
				<p class="form-control-static"><a href="https://n3rds.work/docs/bonipress-shortcode-bonipress_video/" target="_blank">[bonipress_video]</a></p>
			</div>
		</div>
	</div>
</div>
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_load_video_view_program' ) ) :
	function bonipress_load_video_view_program() {
		...
		add_action( 'bonipress_front_enqueue',        'bonipress_video_register_scripts' );
		add_action( 'template_redirect',           'bonipress_video_detect_views' );
		add_action( 'bonipress_front_enqueue_footer', 'bonipress_video_enqueue_footer' );

	}
endif;
add_action( 'bonipress_init', 'bonipress_load_video_view_program', 91 );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_video_register_scripts' ) ) :
	function bonipress_video_register_scripts() {

		wp_register_script(
			'bonipress-video-points',
			plugins_url( 'assets/js/video.js', boniPRESS_THIS ),
			array( 'jquery' ),
			boniPRESS_VERSION . '.1',
			true
		);
		...
		wp_localize_script(
			'bonipress-video-points',
			'boniPRESS_Video',
			apply_filters( 'bonipress_video_js', array(
				'ajaxurl' => esc_url( ( isset( $post->ID ) ) ? bonipress_get_permalink( $post->ID ) : home_url( '/' ) ),
				'token'   => wp_create_nonce( 'bonipress-video-points' )
			) )
		);
		wp_enqueue_script( 'bonipress-video-points' );

		wp_register_script(
			'bonipress-video-youtube',
			plugins_url( 'assets/js/youtube.js', boniPRESS_THIS ),
			array( 'jquery' ),
			boniPRESS_VERSION . '.1',
			true
		);
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_video_detect_views' ) ) :
	function bonipress_video_detect_views() {

		if ( is_user_logged_in() ) {

			if ( isset( $_POST['action'] ) && $_POST['action'] == 'bonipress-viewing-videos' && isset( $_POST['setup'] ) && isset( $_POST['type'] ) && isset( $_POST['token'] ) && wp_verify_nonce( $_POST['token'], 'bonipress-video-points' ) ) {

				$key        = sanitize_text_field( $_POST['setup'] );
				$point_type = sanitize_text_field( $_POST['type'] );
				$setup      = bonipress_verify_token( $key, 6 );
				...
				foreach ( explode( ',', $setup[5] ) as $type_key ) {
					...
					if ( bonipress_point_type_exists( $type_key ) && ! in_array( $type_id, $types ) )
						$types[] = $type_key;
				...
				if ( ! empty( $types ) ) {
					...
					do_action( 'bonipress_video_view', $point_type, $setup );

					foreach ( $types as $point_type )
						do_action( 'bonipress_video_view_' . $point_type, $setup );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_video_enqueue_footer' ) ) :
	function bonipress_video_enqueue_footer() {

		global $bonipress_video_points;

		// If youtube videos are used
		if ( in_array( 'youtube', (array) $bonipress_video_points ) )
			wp_enqueue_script( 'bonipress-video-youtube' );
