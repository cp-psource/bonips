 ...
 * @version 1.1
 */
if ( ! class_exists( 'boniPS_Hook_Video_Views' ) ) :
	class boniPS_Hook_Video_Views extends boniPS_Hook {
		...
		public function run() {
			...
			add_action( 'wp_ajax_bonips-viewing-videos', array( $this, 'ajax_call_video_points' ) );

			add_filter( 'bonips_video_js', array( $this, 'adjust_js' ) );
			add_filter( 'bonips_video_view_' . $this->bonips_type, array( $this, 'video_view' ), 10, 2 );
			...
			switch ( $logic ) {
				...
				case 'play' :
					...
					if ( $state == 1 ) {
						...
						if ( ! $this->has_entry( 'watching_video', '', $user_id, $video_id, $this->bonips_type ) ) {
				...
				case 'interval' :
					...
					if ( $state == 1 && $users_log === NULL ) {
						...
						// Add points without using bonips_add to prevent
					...
					elseif ( $state == 0 && isset( $users_log->creds ) && $users_log->creds+$amount <= $max ) {
						...
						if ( function_exists( 'bonips_add_new_notice' ) ) {
							...
							if ( ! empty( $message ) )
								bonips_add_new_notice( array( 'user_id' => $user_id, 'message' => $message, 'color' => $color ) );
		...
		public function get_users_video_log( $video_id, $user_id ) {
			...
			return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bonips_log_table} WHERE user_id = %d AND data = %s AND ctype = %s;", $user_id, $video_id, $this->bonips_type ) );
...
<div class="hook-instance">
	...
	<div class="row">
		<div class="col-lg-7 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"><?php _e( 'Award Logic', 'bonips' ); ?></label>
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"<?php checked( $prefs['logic'], 'play' ); ?> value="play" class="toggle-hook-option" /> <?php _e( 'Play - As soon as video starts playing.', 'bonips' ); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'logic' => 'full' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'full' ) ); ?>"<?php checked( $prefs['logic'], 'full' ); ?> value="full" class="toggle-hook-option" /> <?php _e( 'Full - First when the entire video has played.', 'bonips' ); ?></label>
				</div>
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'logic' => 'interval' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'interval' ) ); ?>"<?php checked( $prefs['logic'], 'interval' ); ?> value="interval" class="toggle-hook-option" /> <?php echo $this->core->template_tags_general( __( 'Interval - For each x number of seconds watched.', 'bonips' ) ); ?></label>
				</div>
			</div>
		</div>
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_load_video_view_program' ) ) :
	function bonips_load_video_view_program() {
		...
		add_action( 'bonips_front_enqueue',        'bonips_video_register_scripts' );
		add_action( 'template_redirect',           'bonips_video_detect_views' );
		add_action( 'bonips_front_enqueue_footer', 'bonips_video_enqueue_footer' );

	}
endif;
add_action( 'bonips_init', 'bonips_load_video_view_program', 91 );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_video_register_scripts' ) ) :
	function bonips_video_register_scripts() {

		wp_register_script(
			'bonips-video-points',
			plugins_url( 'assets/js/video.js', boniPS_THIS ),
			array( 'jquery' ),
			boniPS_VERSION . '.1',
			true
		);
		...
		wp_localize_script(
			'bonips-video-points',
			'boniPS_Video',
			apply_filters( 'bonips_video_js', array(
				'ajaxurl' => esc_url( ( isset( $post->ID ) ) ? bonips_get_permalink( $post->ID ) : home_url( '/' ) ),
				'token'   => wp_create_nonce( 'bonips-video-points' )
			) )
		);
		wp_enqueue_script( 'bonips-video-points' );

		wp_register_script(
			'bonips-video-youtube',
			plugins_url( 'assets/js/youtube.js', boniPS_THIS ),
			array( 'jquery' ),
			boniPS_VERSION . '.1',
			true
		);
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_video_detect_views' ) ) :
	function bonips_video_detect_views() {

		if ( is_user_logged_in() ) {

			if ( isset( $_POST['action'] ) && $_POST['action'] == 'bonips-viewing-videos' && isset( $_POST['setup'] ) && isset( $_POST['type'] ) && isset( $_POST['token'] ) && wp_verify_nonce( $_POST['token'], 'bonips-video-points' ) ) {

				$key        = sanitize_text_field( $_POST['setup'] );
				$point_type = sanitize_text_field( $_POST['type'] );
				$setup      = bonips_verify_token( $key, 6 );
				...
				foreach ( explode( ',', $setup[5] ) as $type_key ) {
					...
					if ( bonips_point_type_exists( $type_key ) && ! in_array( $type_id, $types ) )
						$types[] = $type_key;
				...
				if ( ! empty( $types ) ) {
					...
					do_action( 'bonips_video_view', $point_type, $setup );

					foreach ( $types as $point_type )
						do_action( 'bonips_video_view_' . $point_type, $setup );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_video_enqueue_footer' ) ) :
	function bonips_video_enqueue_footer() {

		global $bonips_video_points;

		// If youtube videos are used
		if ( in_array( 'youtube', (array) $bonips_video_points ) )
			wp_enqueue_script( 'bonips-video-youtube' );
