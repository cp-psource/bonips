 ...
 * @version 1.3.1
 */
if ( ! class_exists( 'boniPRESS_Hook_Affiliate' ) ) :
	class boniPRESS_Hook_Affiliate extends boniPRESS_Hook {
		...
		function __construct( $hook_prefs, $type = BONIPRESS_DEFAULT_TYPE_KEY ) {
			...
			parent::__construct( array(
				...
				'defaults' => array(
					...
					'buddypress' => array(
						'profile'  => 0,
						'priority' => 10,
						'title'    => __( 'Affiliate Program', 'bonipress' ),
						'desc'     => ''
					)
				)
			), $hook_prefs, $type );
			...
			$this->limit_by = apply_filters( 'bonipress_affiliate_limit_by', array(
				'total' => __( 'Total', 'bonipress' ),
				'daily' => __( 'Per Day', 'bonipress' )
			), $this );

			// Let others play with the ref key
			$this->ref_key = apply_filters( 'bonipress_affiliate_key', 'mref', $this );

			add_filter( 'bonipress_parse_log_entry_signup_referral', array( $this, 'parse_log_entry' ), 10, 2 );
		...
		public function run() {
			...
			if ( function_exists( 'buddypress' ) && apply_filters( 'bp_core_signup_send_activation_key', true ) )
				add_action( 'bonipress_bp_user_activated', array( $this, 'verified_signup' ) );

			// Register Shortcodes
			add_filter( 'bonipress_affiliate_link_' . $this->bonipress_type, array( $this, 'shortcode_affiliate_link' ), 10, 2 );
			add_filter( 'bonipress_affiliate_id_' . $this->bonipress_type,   array( $this, 'shortcode_affiliate_id' ), 10, 2 );

			add_filter( 'bonipress_referral_keys', array( $this, 'add_key' ) );
			...
			if ( $this->prefs['visit']['creds'] != 0 || $this->prefs['signup']['creds'] != 0 )
				add_action( 'bonipress_referred_visit', array( $this, 'site_visits' ) );

			// Points for signups
			if ( $this->prefs['signup']['creds'] != 0 )
				add_action( 'bonipress_referred_signup', array( $this, 'site_signup' ) );
		...
		public function add_key( $keys ) {
			...
			if ( ! isset( $_GET[ $this->ref_key ] ) || isset( $_COOKIE[ 'bonipress_ref' . $this->bonipress_type ] ) ) return $keys;
		...
		public function shortcode_affiliate_link( $content = '', $atts ) {
			...
			if ( $post_id != '' )
				$url = bonipress_get_permalink( $post_id );
		...
		public function shortcode_affiliate_id( $content = '', $atts ) {
			...
			return apply_filters( 'bonipress_affiliate_id', $ref_id, $atts, $this );
		...
		public function buddypress_profile() {
			...
			if ( bp_is_my_profile() || bonipress_is_admin() ) {

				$users_ref_link = $this->get_ref_link( $user_id, home_url( '/' ) );

				$output .= '<div class="bp-widget bonipress">';
				...
				$output .= sprintf( '<tr class="field_1 field_ref_link"><td class="label">%s</td><td><input type="text" value="%s" id="mref-link-buddypress-profile" readonly><button onclick="copy_to_clipBoard()">Copy</button></td></tr>', __( 'Link', 'bonipress' ), $users_ref_link );

				// Show Visitor referral count
				if ( $this->prefs['visit']['creds'] != 0 )
					$output .= sprintf( '<tr class="field_2 field_ref_count_visit"><td class="label">%s</td><td>%s</td></tr>', __( 'Visitors Referred', 'bonipress' ), bonipress_count_ref_instances( 'visitor_referral', $user_id, $this->bonipress_type ) );

				// Show Signup referral count
				if ( $this->prefs['signup']['creds'] != 0 )
					$output .= sprintf( '<tr class="field_3 field_ref_count_signup"><td class="label">%s</td><td>%s</td></tr>', __( 'Signups Referred', 'bonipress' ), bonipress_count_ref_instances( 'signup_referral', $user_id, $this->bonipress_type ) );
			...
			echo apply_filters( 'bonipress_affiliate_bp_profile', $output, $user_id, $users_ref_link, $this );
		...
		public function site_visits() {
			...
			// Required
			if ( ! isset( $_GET[ $this->ref_key ] ) || empty( $_GET[ $this->ref_key ] ) || isset( $_COOKIE[ 'bonipress_ref' . $this->bonipress_type ] ) ) return;
			...
			if ( $user_id !== NULL && ! is_user_logged_in() ) {
				...
				$IP = apply_filters( 'bonipress_affiliate_IP', $_SERVER['REMOTE_ADDR'], 'visit', $this );
				if ( $IP != '' && $IP != '0.0.0.0' ) {
					...
					if ( $this->ref_counts( $user_id, $IP ) ) {
						...
						do_action( 'bonipress_visitor_referral', $user_id, $IP, $this );
					...
					if ( ! headers_sent() ) {
						...
						setcookie( 'bonipress_ref' . $this->bonipress_type, $_GET[ $this->ref_key ], apply_filters( 'bonipress_affiliate_cookie', ( time()+3600*24 ), false, $this ), COOKIEPATH, COOKIE_DOMAIN );

						if ( get_option( 'users_can_register' ) && $this->prefs['signup']['creds'] > 0 )
							setcookie( 'signup_ref' . $this->bonipress_type, $_GET[ $this->ref_key ], apply_filters( 'bonipress_affiliate_cookie', ( time()+3600*24 ), true, $this ), COOKIEPATH, COOKIE_DOMAIN );
		...
		public function site_signup( $new_user_id ) {
			...
			$ref = false;
			$key = '';
			if ( isset( $_COOKIE[ 'signup_ref' . $this->bonipress_type ] ) ) {
				$ref = $_COOKIE[ 'signup_ref' . $this->bonipress_type ];
				$key = 'signup_ref' . $this->bonipress_type;
			}
			elseif ( isset( $_COOKIE[ 'bonipress_ref' . $this->bonipress_type ] ) ) {
				$ref = $_COOKIE[ 'bonipress_ref' . $this->bonipress_type ];
				$key = 'bonipress_ref' . $this->bonipress_type;
			}
			...
			// Attempt to get the users IP
			$IP = apply_filters( 'bonipress_affiliate_IP', $_SERVER['REMOTE_ADDR'], 'signup', $this );
			if ( $IP != '' && $IP != '0.0.0.0' ) {
				...
				if ( $this->ref_counts( $user_id, $IP, 'signup' ) ) {
					...
					if ( function_exists( 'buddypress' ) && apply_filters( 'bp_core_signup_send_activation_key', true ) === true ) {
						bonipress_add_user_meta( $new_user_id, 'referred_by', '', $user_id, true );
						bonipress_add_user_meta( $new_user_id, 'referred_by_IP', '', $IP, true );
						bonipress_add_user_meta( $new_user_id, 'referred_by_type', '', $this->bonipress_type, true );
					}
					...
					// Award now
						...
						do_action( 'bonipress_signup_referral', $user_id, $IP, $new_user_id, $this );
		...
		public function verified_signup( $user_id ) {
			...
			$referred_by    = bonipress_get_user_meta( $user_id, 'referred_by', '', true );
			$referred_by_IP = bonipress_get_user_meta( $user_id, 'referred_by_IP', '', true );
			$referred_type  = bonipress_get_user_meta( $user_id, 'referred_by_type', '', true );

			if ( $referred_by == '' || $referred_by_IP == '' || $this->bonipress_type != $referred_type ) return;
			...
			do_action( 'bonipress_signup_referral', $referred_by, $referred_by_IP, $user_id, $this );

			// Clean up
			bonipress_delete_user_meta( $user_id, 'referred_by' );
			bonipress_delete_user_meta( $user_id, 'referred_by_IP' );
			bonipress_delete_user_meta( $user_id, 'referred_by_type' );
		...
		public function get_ref_link( $user_id = '', $url = '' ) {
			...
			return apply_filters( 'bonipress_affiliate_get_ref_link', esc_url( $link ), $user_id, $url, $this );
		...
		public function get_ref_id( $user_id ) {
			...
			switch ( $this->prefs['setup']['links'] ) {
				...
				case 'numeric' :
					...
					$id = bonipress_get_user_meta( $user_id, 'bonipress_affiliate_link', '', true );
					if ( ! is_numeric( $id ) ) {

						$counter = absint( get_option( 'bonipress_affiliate_counter', 0 ) );
						$number  = $counter+1;

						bonipress_update_option( 'bonipress_affiliate_counter', $number );
						bonipress_update_user_meta( $user_id, 'bonipress_affiliate_link', '', $number );
			...
			return apply_filters( 'bonipress_affiliate_get_ref_id', $ref_id, $user_id, $this );
		...
		public function get_user_id_from_ref_id( $string = '' ) {
			...
			switch ( $this->prefs['setup']['links'] ) {
				...
				case 'numeric' :
					...
					$referral_id_key = bonipress_get_meta_key( 'bonipress_affiliate_link' );
			...
			return apply_filters( 'bonipress_affiliate_get_user_id', $user_id, $string, $this );
		...
		public function ref_counts( $user_id, $IP = '', $instance = 'visit' ) {
			...
			if ( $this->prefs['setup']['IP'] > 0 ) {
				...
				$count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$bonipress_log_table} WHERE ref = %s AND data = %s AND ctype = %s;", $ref, $IP, $this->bonipress_type ) );
			...
			return apply_filters( 'bonipress_affiliate_ref_counts', $reply, $this );
...
<div class="hook-instance">
	...
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'visit', 'limit' ) ); ?>"><?php _e( 'Limit', 'bonipress' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'visit' => 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
...
<div class="hook-instance">
	<h3><?php _e( 'Referring Signups', 'bonipress' ); ?></h3>
	...
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'signup', 'limit' ) ); ?>"><?php _e( 'Limit', 'bonipress' ); ?></label>
				<?php echo $this->hook_limit_setting( $this->field_name( array( 'signup', 'limit' ) ), $this->field_id( array( 'signup', 'limit' ) ), $prefs['signup']['limit'] ); ?>
			</div>
		</div>
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'signup' => 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
...
<div class="hook-instance">
	<h3><?php _e( 'Referral Links', 'bonipress' ); ?></h3>
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'setup' => 'links' ) ); ?>-numeric"><input type="radio" name="<?php echo $this->field_name( array( 'setup' => 'links' ) ); ?>" id="<?php echo $this->field_id( array( 'setup' => 'links' ) ); ?>-numeric" <?php checked( $prefs['setup']['links'], 'numeric' ); ?> value="numeric" /> <?php _e( 'Assign numeric referral IDs to each user.', 'bonipress' ); ?></label>
				<span class="description"><?php printf( '%s: %s', __( 'Example', 'bonipress' ), esc_url( add_query_arg( array( $this->ref_key => 1 ), home_url( '/' ) ) ) ); ?></span>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'setup' => 'links' ) ); ?>-username"><input type="radio" name="<?php echo $this->field_name( array( 'setup' => 'links' ) ); ?>" id="<?php echo $this->field_id( array( 'setup' => 'links' ) ); ?>-username" <?php checked( $prefs['setup']['links'], 'username' ); ?> value="username" /> <?php _e( 'Assign usernames as IDs for each user.', 'bonipress' ); ?></label>
				<span class="description"><?php printf( '%s: %s', __( 'Example', 'bonipress' ), esc_url( add_query_arg( array( $this->ref_key => 'john+doe' ), home_url( '/' ) ) ) ); ?></span>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'setup' => 'IP' ) ); ?>"><?php _e( 'IP Limit', 'bonipress' ); ?></label>
				...
				<span class="description"><?php echo $this->core->template_tags_general( __( 'The number of times each IP address grants %_plural%. Use zero for unlimited.', 'bonipress' ) ); ?></span>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label><?php _e( 'Available Shortcodes', 'bonipress' ); ?></label>
				<p class="form-control-static"><a href="http://codex.bonipress.me/shortcodes/bonipress_affiliate_link/" target="_blank">[bonipress_affiliate_link]</a>, <a href="http://codex.bonipress.me/shortcodes/bonipress_affiliate_id/" target="_blank">[bonipress_affiliate_id]</a></p>
			</div>
		</div>
	</div>
</div>
...
<div class="hook-instance">
	<h3><?php _e( 'BuddyPress Profile', 'bonipress' ); ?></h3>
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="checkbox">
					<label for="<?php echo $this->field_id( array( 'buddypress' => 'profile' ) ); ?>"><input type="checkbox" name="<?php echo $this->field_name( array( 'buddypress' => 'profile' ) ); ?>" id="<?php echo $this->field_id( array( 'buddypress' => 'profile' ) ); ?>"<?php checked( $prefs['buddypress']['profile'], 1 ); ?> value="1" /> <?php _e( 'Insert referral link in users profiles', 'bonipress' ); ?></label>
				</div>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-8 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label><?php _e( 'Title', 'bonipress' ); ?></label><br />
				...
				<span class="description"><?php _e( 'Leave empty to hide.', 'bonipress' ); ?></span>
			</div>
		</div>
		<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label><?php _e( 'Profile Positioning', 'bonipress' ); ?></label><br />
				...
				<span class="description"><?php _e( 'You can move around the referral link on your users profile by changing the position. Increase to move up, decrease to move down.', 'bonipress' ); ?></span>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'buddypress' => 'desc' ) ); ?>"><?php _e( 'Description', 'bonipress' ); ?></label>
				<span class="description"><?php _e( 'Optional description to insert under the link.', 'bonipress' ); ?></span>
...
<?php
			...
			do_action( 'bonipress_affiliate_prefs', $prefs, $this );
		...
		function sanitise_preferences( $data ) {
			...
			return apply_filters( 'bonipress_affiliate_save_pref', $data );
...
add_action( 'bonipress_init', 'bonipress_load_referral_program', 90 );
