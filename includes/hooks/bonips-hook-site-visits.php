 ...
 * @version 1.1
 */
if ( ! class_exists( 'boniPS_Hook_Site_Visits' ) ) :
	class boniPS_Hook_Site_Visits extends boniPS_Hook {
		...
		public function run() {
			...
			if ( is_user_logged_in() && ! isset( $_COOKIE['bonips_site_visit'] ) )
		...
		public function site_visit() {
			...
			if ( ! headers_sent() ) setcookie( 'bonips_site_visit', 1, $lifespan, COOKIEPATH, COOKIE_DOMAIN );

			// Make sure user is not excluded
			if ( $this->core->exclude_user( $user_id ) ) return;

			// Store todays date as an integer
			$today = (int) apply_filters( 'bonips_site_visit_id', date( 'Ymd', $now ) );
			$data = '';

			// Make sure this is unique
			if ( $this->core->has_entry( 'site_visit', $today, $user_id, $data, $this->bonips_type ) ) return;
