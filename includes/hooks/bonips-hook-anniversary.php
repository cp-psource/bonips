...
if ( ! class_exists( 'boniPS_Hook_Anniversary' ) ) :
	class boniPS_Hook_Anniversary extends boniPS_Hook {
		...
		public function page_load() {
			...
			$last_run = bonips_get_user_meta( $user_id, 'anniversary-' . $this->bonips_type, '', true );
			...
			if ( isset( $result->user_registered ) && $result->difference >= 1 ) {
				...
				for ( $i = 0; $i < $result->difference; $i++ ) {
					...
					$year_joined++;
					if ( $this->core->has_entry( 'anniversary', $year_joined, $user_id, $date_joined, $this->bonips_type ) ) continue;
			...
			bonips_update_user_meta( $user_id, 'anniversary-' . $this->bonips_type, '', $today );
