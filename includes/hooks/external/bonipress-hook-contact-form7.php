 ...
 * @version 1.1
 */
add_filter( 'bonipress_setup_hooks', 'bonipress_register_contact_form_seven_hook', 50 );
function bonipress_register_contact_form_seven_hook( $installed ) {

	if ( ! function_exists( 'wpcf7' ) ) return $installed;

	$installed['contact_form7'] = array(
		'title'         => __( 'Contact Form 7 Form Submissions', 'bonipress' ),
		'description'   => __( 'Awards %_plural% for successful form submissions (by logged in users).', 'bonipress' ),
		'documentation' => 'http://codex.bonipress.me/hooks/submitting-contact-form-7-forms/',
		'callback'      => array( 'boniPRESS_Contact_Form7' )
	);
 ...
 * @version 1.1
 */
add_action( 'bonipress_load_hooks', 'bonipress_load_contact_form_seven_hook', 50 );
function bonipress_load_contact_form_seven_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPRESS_Contact_Form7' ) || ! function_exists( 'wpcf7' ) ) return;

	class boniPRESS_Contact_Form7 extends boniPRESS_Hook {
...
<div class="hook-instance">
	<h3><?php printf( __( 'Form: %s', 'bonipress' ), $form_title ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $form_id, 'limit' ) ); ?>"><?php _e( 'Limit', 'bonipress' ); ?></label>
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $form_id, 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( $form_id, 'log' ) ); ?>" id="<?php echo $this->field_id( array( $form_id, 'log' ) ); ?>" placeholder="<?php _e( 'required', 'bonipress' ); ?>" value="<?php echo esc_attr( $prefs[ $form_id ]['log'] ); ?>" class="form-control" />
