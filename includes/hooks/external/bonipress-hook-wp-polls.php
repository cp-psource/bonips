 ...
 * @version 1.1
 */
add_filter( 'bonipress_setup_hooks', 'bonipress_register_wp_polls_hook', 105 );
function bonipress_register_wp_polls_hook( $installed ) {

	if ( ! function_exists( 'vote_poll' ) ) return $installed;

	$installed['wppolls'] = array(
		'title'         => __( 'WP-Polls', 'bonipress' ),
		'description'   => __( 'Awards %_plural% for users voting in polls.', 'bonipress' ),
		'documentation' => '',
		'callback'      => array( 'boniPRESS_Hook_WPPolls' )
	);
 ...
 * @version 1.1
 */
add_action( 'bonipress_load_hooks', 'bonipress_load_wp_polls_hook', 105 );
function bonipress_load_wp_polls_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPRESS_Hook_WPPolls' ) || ! function_exists( 'vote_poll' ) ) return;

	class boniPRESS_Hook_WPPolls extends boniPRESS_Hook {
		...
		public function run() {
			...
			add_filter( 'bonipress_parse_tags_poll', array( $this, 'parse_custom_tags' ), 10, 2 );
