...
function bonips_load_jetpack_hook() {
	...
	class boniPS_Hook_Jetpack extends boniPS_Hook {
		...
		public function run() {
			...
			add_action( 'bonips_admin_init', array( $this, 'admin_init' ) );
		...
		public function admin_init() {
			...
			foreach ( $types as $type ) {
				...
				if ( false === ( $pending = get_option( 'bonips_jetpack_' . $type . '_pendings' ) ) )
					continue;

				// Make sure list is not empty
				if ( empty( $pending ) ) {
					// Clean up before exit
					delete_option( 'bonips_jetpack_' . $type . '_pendings' );
					continue;
				}
				...
				if ( ! empty( $new ) )
					update_option( 'bonips_jetpack_' . $type . '_pendings', $new );
		...
		protected function comment_subscribe( $email = '', $post_ids = '', $user_id = 0, $comment_id = 0 ) {
			...
			if ( $subscription == 'active' ) {
				...
				do_action( 'bonips_jetpack_comment', $user_id, $comment_id );
		...
		protected function site_subscribe( $email = '', $user_id = 0 ) {
			...
			if ( $subscription == 'active' ) {
				...
				// Let others share our success
				do_action( 'bonips_jetpack_site', $user_id, $GLOBALS['blog_id'] );
		...
		protected function is_pending( $email = NULL, $section = 'site' ) {
			...
			if ( false === ( $pending = get_option( 'bonips_jetpack_' . $name . '_pendings' ) ) ) {
				if ( $name == 'site' )
					$pending = array( $email );
				else
					$pending = array( $section => $email );

				update_option( 'bonips_jetpack_' . $name . '_pendings', $pending );
			}
...
<div class="hook-instance">
	...
	<div class="row">
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'subscribe_site' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'subscribe_site' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'subscribe_site' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['subscribe_site']['log'] ); ?>" class="form-control" />
...
<div class="hook-instance">
	...
	<div class="row">
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'subscribe_comment' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'subscribe_comment' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'subscribe_comment' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['subscribe_comment']['log'] ); ?>" class="form-control" />
				<span class="description"><?php echo $this->available_template_tags( array( 'general' ) ); ?></span>
			</div>
		</div>
	</div>
