 ...
 * @version 1.1
 */
add_filter( 'bonipress_setup_hooks', 'bonipress_register_jetpack_hook', 75 );
function bonipress_register_jetpack_hook( $installed ) {

	if ( ! defined( 'JETPACK__PLUGIN_DIR' ) ) return $installed;

	$installed['jetpack'] = array(
		'title'         => __( 'Jetpack Subscriptions', 'bonipress' ),
		'description'   => __( 'Awards %_plural% for users signing up for site or comment updates using Jetpack.', 'bonipress' ),
		'documentation' => 'http://codex.bonipress.me/hooks/jetpack-subscriptions/',
		'callback'      => array( 'boniPRESS_Hook_Jetpack' )
	);
 ...
 * @version 1.1.1
 */
add_action( 'bonipress_load_hooks', 'bonipress_load_jetpack_hook', 75 );
function bonipress_load_jetpack_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPRESS_Hook_Jetpack' ) || ! defined( 'JETPACK__PLUGIN_DIR' ) ) return;

	class boniPRESS_Hook_Jetpack extends boniPRESS_Hook {
		...
		public function run() {
			...
			add_action( 'bonipress_admin_init', array( $this, 'admin_init' ) );
		...
		public function admin_init() {
			...
			foreach ( $types as $type ) {
				...
				if ( false === ( $pending = get_option( 'bonipress_jetpack_' . $type . '_pendings' ) ) )
					continue;

				// Make sure list is not empty
				if ( empty( $pending ) ) {
					// Clean up before exit
					delete_option( 'bonipress_jetpack_' . $type . '_pendings' );
					continue;
				}
				...
				if ( ! empty( $new ) )
					update_option( 'bonipress_jetpack_' . $type . '_pendings', $new );
		...
		protected function comment_subscribe( $email = '', $post_ids = '', $user_id = 0, $comment_id = 0 ) {
			...
			if ( $subscription == 'active' ) {
				...
				do_action( 'bonipress_jetpack_comment', $user_id, $comment_id );
		...
		protected function site_subscribe( $email = '', $user_id = 0 ) {
			...
			if ( $subscription == 'active' ) {
				...
				// Let others share our success
				do_action( 'bonipress_jetpack_site', $user_id, $GLOBALS['blog_id'] );
		...
		protected function is_pending( $email = NULL, $section = 'site' ) {
			...
			if ( false === ( $pending = get_option( 'bonipress_jetpack_' . $name . '_pendings' ) ) ) {
				if ( $name == 'site' )
					$pending = array( $email );
				else
					$pending = array( $section => $email );

				update_option( 'bonipress_jetpack_' . $name . '_pendings', $pending );
			}
...
<div class="hook-instance">
	<h3><?php _e( 'Site Subscriptions', 'bonipress' ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'subscribe_site' => 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'subscribe_site' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'subscribe_site' => 'log' ) ); ?>" placeholder="<?php _e( 'required', 'bonipress' ); ?>" value="<?php echo esc_attr( $prefs['subscribe_site']['log'] ); ?>" class="form-control" />
...
<div class="hook-instance">
	<h3><?php _e( 'Comment Subscriptions', 'bonipress' ); ?></h3>
	<div class="row">
		<div class="col-lg-2 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'subscribe_comment' => 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'subscribe_comment' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'subscribe_comment' => 'log' ) ); ?>" placeholder="<?php _e( 'required', 'bonipress' ); ?>" value="<?php echo esc_attr( $prefs['subscribe_comment']['log'] ); ?>" class="form-control" />
				<span class="description"><?php echo $this->available_template_tags( array( 'general' ) ); ?></span>
			</div>
		</div>
	</div>
