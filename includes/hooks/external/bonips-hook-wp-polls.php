 ...
 * @version 1.1
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_wp_polls_hook', 105 );
function bonips_register_wp_polls_hook( $installed ) {

	if ( ! function_exists( 'vote_poll' ) ) return $installed;

	$installed['wppolls'] = array(
		'title'         => __( 'WP-Polls', 'bonips' ),
		'description'   => __( 'Awards %_plural% for users voting in polls.', 'bonips' ),
		'documentation' => '',
		'callback'      => array( 'boniPS_Hook_WPPolls' )
	);
 ...
 * @version 1.1
 */
add_action( 'bonips_load_hooks', 'bonips_load_wp_polls_hook', 105 );
function bonips_load_wp_polls_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_Hook_WPPolls' ) || ! function_exists( 'vote_poll' ) ) return;

	class boniPS_Hook_WPPolls extends boniPS_Hook {
		...
		public function run() {
			...
			add_filter( 'bonips_parse_tags_poll', array( $this, 'parse_custom_tags' ), 10, 2 );
