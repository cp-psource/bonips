...
if ( ! function_exists( 'bonipress_woo_remove_review_from_comments' ) ) :
	function bonipress_woo_remove_review_from_comments( $reply, $comment ) {
		...
		if ( bonipress_get_post_type( $comment->comment_post_ID ) == 'product' ) return false;
...
if ( ! function_exists( 'bonipress_woo_product_metabox' ) ) :
	function bonipress_woo_product_metabox( $post ) {
		...
		$prefs = (array) bonipress_get_post_meta( $post->ID, 'bonipress_reward', true );
		...
		foreach ( $types as $point_type => $point_type_label ) {
			...
			if ( ! $bonipress->user_is_point_admin( $cui ) ) continue;
...
if ( ! function_exists( 'bonipress_woo_add_product_variation_detail' ) ) :
	function bonipress_woo_add_product_variation_detail( $loop, $variation_data, $variation ) {
		...
		$prefs   = (array) bonipress_get_post_meta( $variation->ID, '_bonipress_reward', true );
...
<?php
		...
		foreach ( $types as $point_type => $point_type_label ) {
			...
			if ( ! $bonipress->user_is_point_admin( $user_id ) ) continue;
...
if ( ! function_exists( 'bonipress_woo_save_reward_settings' ) ) :
	function bonipress_woo_save_reward_settings( $post_id ) {
		...
		if ( ! isset( $_POST['bonipress_reward'] ) || empty( $_POST['bonipress_reward'] ) || bonipress_get_post_type( $post_id ) != 'product' ) return;
		...
		if ( empty( $new_setup ) )
			bonipress_delete_post_meta( $post_id, 'bonipress_reward' );
		else
			bonipress_update_post_meta( $post_id, 'bonipress_reward', $new_setup );
...
if ( ! function_exists( 'bonipress_woo_save_product_variation_detail' ) ) :
	function bonipress_woo_save_product_variation_detail( $post_id ) {
		...
		if ( empty( $new_setup ) )
			bonipress_delete_post_meta( $post_id, '_bonipress_reward' );
		else
			bonipress_update_post_meta( $post_id, '_bonipress_reward', $new_setup );
	...
	function bonipress_get_woo_product_reward( $product_id = NULL, $variation_id = NULL, $requested_type = false ) {
		...
		if ( function_exists( 'wc_get_product' ) ) {
			...
			if ( $product->is_type( 'variable' ) && $variation_id !== NULL && $variation_id > 0 ) {
				...
				$reward_setup        = (array) bonipress_get_post_meta( $variation_id, '_bonipress_reward', true );
				$parent_reward_setup = (array) bonipress_get_post_meta( $parent_product_id, 'bonipress_reward', true );
			}
			else {
				$reward_setup        = (array) bonipress_get_post_meta( $product_id, 'bonipress_reward', true );
...
function bonipress_load_woocommerce_hook() {
	...
	class boniPRESS_Hook_WooCommerce_Reviews extends boniPRESS_Hook {
		...
		public function review_transitions( $new_status, $old_status, $comment ) {
			...
			if ( bonipress_get_post_type( $comment->comment_post_ID ) != 'product' ) return;
