 ...
 * @version 1.1
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_contact_form_seven_hook', 50 );
function bonips_register_contact_form_seven_hook( $installed ) {

	if ( ! function_exists( 'wpcf7' ) ) return $installed;

	$installed['contact_form7'] = array(
		'title'         => __( 'Contact Form 7 Form Submissions', 'bonips' ),
		'description'   => __( 'Awards %_plural% for successful form submissions (by logged in users).', 'bonips' ),
		'documentation' => 'http://codex.bonips.me/hooks/submitting-contact-form-7-forms/',
		'callback'      => array( 'boniPS_Contact_Form7' )
	);
 ...
 * @version 1.1
 */
add_action( 'bonips_load_hooks', 'bonips_load_contact_form_seven_hook', 50 );
function bonips_load_contact_form_seven_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_Contact_Form7' ) || ! function_exists( 'wpcf7' ) ) return;

	class boniPS_Contact_Form7 extends boniPS_Hook {
...
<div class="hook-instance">
	<h3><?php printf( __( 'Form: %s', 'bonips' ), $form_title ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $form_id, 'limit' ) ); ?>"><?php _e( 'Limit', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $form_id, 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( $form_id, 'log' ) ); ?>" id="<?php echo $this->field_id( array( $form_id, 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs[ $form_id ]['log'] ); ?>" class="form-control" />
