 ...
 * @version 1.3
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_badgeos_hook', 15 );
function bonips_register_badgeos_hook( $installed ) {

	if ( ! class_exists( 'BadgeOS' ) ) return $installed;

	$installed['badgeos'] = array(
		'title'         => __( 'BadgeOS', 'bonips' ),
		'description'   => __( 'Default settings for each BadgeOS Achievement type. These settings may be overridden for individual achievement type.', 'bonips' ),
		'documentation' => 'http://codex.bonips.me/hooks/badgeos-achievements/',
		'callback'      => array( 'boniPS_Hook_BadgeOS' )
	);
 ...
 * @version 1.1.2
 */
add_action( 'bonips_load_hooks', 'bonips_load_badgeos_hook', 15 );
function bonips_load_badgeos_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_Hook_BadgeOS' ) || ! class_exists( 'BadgeOS' ) ) return;

	class boniPS_Hook_BadgeOS extends boniPS_Hook {
		...
		public function add_metaboxes() {
			...
			foreach ( $badge_post_types as $post_type ) {
				...
				add_meta_box(
					'bonips_badgeos_' . $post_type . '_' . $this->bonips_type,
					$this->core->plural(),
		...
		public function render_meta_box( $post ) {
			...
			if ( ! isset( $this->prefs[ $post->post_type ] ) ) {
				...
				if ( ! $this->is_main_type )
					$page = BONIPS_SLUG . '_' . $this->bonips_type . '-hooks';

				echo '<p>' . sprintf( __( 'Please setup your <a href="%s">default settings</a> before using this feature.', 'bonips' ), admin_url( 'admin.php?page=' . $page ) ) . '</p>';
				return;

			}

			$post_key         = 'bonips_values' . $this->bonips_type;

			// Prep Achievement Data
			$prefs            = $this->prefs;
			$achievement_data = bonips_get_post_meta( $post->ID, $this->metakey, true );
			if ( $achievement_data == '' )
				$achievement_data = $prefs[ $post->post_type ];

?>
<p><strong><?php echo $this->core->template_tags_general( __( '%plural% to Award', 'bonips' ) ); ?></strong></p>
<p>
	<label class="screen-reader-text" for="bonips-values-<?php echo $this->bonips_type; ?>-creds"><?php echo $this->core->template_tags_general( __( '%plural% to Award', 'bonips' ) ); ?></label>
	<input type="text" name="<?php echo $post_key; ?>[creds]" id="bonips-values-<?php echo $this->bonips_type; ?>-creds" value="<?php echo $this->core->number( $achievement_data['creds'] ); ?>" size="8" />
	<span class="description"><?php _e( 'Use zero to disable', 'bonips' ); ?></span>
</p>
<p><strong><?php _e( 'Protokollvorlage', 'bonips' ); ?></strong></p>
<p>
	<label class="screen-reader-text" for="bonips-values-<?php echo $this->bonips_type; ?>-log"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
	<input type="text" name="<?php echo $post_key; ?>[log]" id="bonips-values-<?php echo $this->bonips_type; ?>-log" value="<?php echo esc_attr( $achievement_data['log'] ); ?>" style="width:99%;" />
</p>
<?php
...
?>
<p><strong><?php _e( 'Deduction Protokollvorlage', 'bonips' ); ?></strong></p>
<p>
	<label class="screen-reader-text" for="bonips-values-<?php echo $this->bonips_type; ?>-log"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
	<input type="text" name="<?php echo $post_key; ?>[deduct_log]" id="bonips-values-deduct-<?php echo $this->bonips_type; ?>-log" value="<?php echo esc_attr( $achievement_data['deduct_log'] ); ?>" style="width:99%;" />
</p>
<?php
		...
		public function save_achivement_data( $post_id ) {
			...
			if ( $_POST[ $post_key ]['creds'] == $this->prefs[ $post_type ]['creds'] && $_POST[ $post_key ]['log'] == $this->prefs[ $post_type ]['log'] ) {
				...
				bonips_delete_post_meta( $post_id, $this->metakey );
				return;
			...
			// Update sales values
			bonips_update_post_meta( $post_id, $this->metakey, $data );
		...
		public function award_achievent( $user_id, $achievement_id ) {
			...
			$post_type        = bonips_get_post_type( $achievement_id );

			// Settings are not set
			if ( ! isset( $this->prefs[ $post_type ]['creds'] ) ) return;

			// Get achievemen data
			$achievement_data = bonips_get_post_meta( $achievement_id, $this->metakey, true );
			if ( $achievement_data == '' )
		...
		public function preferences() {
			...
			foreach ( $badge_post_types as $post_type ) {
				...
				if ( in_array( $post_type, apply_filters( 'bonips_badgeos_excludes', array( 'step' ) ) ) ) continue;
...
<div class="hook-instance">
	<h3><?php printf( __( 'Earning: %s', 'bonips' ), $post_type_object->labels->singular_name ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $post_type, 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( $post_type, 'log' ) ); ?>" id="<?php echo $this->field_id( array( $post_type, 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs[ $post_type ]['log'] ); ?>" class="form-control" />
...
<div class="hook-instance">
	<h3><?php printf( __( 'Revoked: %s', 'bonips' ), $post_type_object->labels->singular_name ); ?></h3>
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
		...
		<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $post_type, 'deduct_log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( $post_type, 'deduct_log' ) ); ?>" id="<?php echo $this->field_id( array( $post_type, 'deduct_log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs[ $post_type ]['deduct_log'] ); ?>" class="form-control" />
				<span class="description"><?php echo $this->available_template_tags( array( 'general', 'post' ) ); ?></span>
			</div>
		</div>
	</div>
