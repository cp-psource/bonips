 ...
 * @version 1.3
 */
if ( ! class_exists( 'boniPRESS_Hook_Click_Links' ) ) :
	class boniPRESS_Hook_Click_Links extends boniPRESS_Hook {
		...
		public function run() {
			...
			add_action( 'bonipress_register_assets',      array( $this, 'register_script' ) );
			add_action( 'bonipress_front_enqueue_footer', array( $this, 'enqueue_footer' ) );
			add_filter( 'bonipress_parse_tags_link',      array( $this, 'parse_custom_tags' ), 10, 2 );

			if ( isset( $_POST['action'] ) && $_POST['action'] == 'bonipress-click-points' && isset( $_POST['token'] ) && wp_verify_nonce( $_POST['token'], 'bonipress-link-points' ) )
		...
		public function custom_limit() {
			...
			return array(
				'none' => __( 'No limit', 'bonipress' ),
				'url'  => __( 'Once for each unique URL', 'bonipress' ),
				'id'   => __( 'Once for each unique link id', 'bonipress' )
			);
		...
		public function register_script() {
			...
			$bonipress_link_points = false;

			wp_register_script(
				'bonipress-link-points',
				plugins_url( 'assets/js/links.js', boniPRESS_THIS ),
				array( 'jquery' ),
				boniPRESS_VERSION . '.1',
				true
			);
			...
			if ( $bonipress_link_points === true ) {
				...
				wp_localize_script(
					...
					array(
						...
						'token'   => wp_create_nonce( 'bonipress-link-points' )
					)
				);
				wp_enqueue_script( 'bonipress-link-points' );
		...
		public function has_entry( $action = '', $reference = '', $user_id = '', $data = '', $type = '' ) {
			...
			$sql = "SELECT id FROM {$bonipress_log_table} WHERE ref = %s AND user_id = %d AND data LIKE %s AND ctype = %s;";
			$wpdb->get_results( $wpdb->prepare( $sql, $action, $user_id, $string, $this->bonipress_type ) );
		...
		public function ajax_call_link_points() {
			...
			if ( ! isset( $_POST['ctype'] ) || $_POST['ctype'] != $this->bonipress_type || ! isset( $_POST['url'] ) ) return;

			// Security
			check_ajax_referer( 'bonipress-link-points', 'token' );

			// Current User
			$user_id = get_current_user_id();

			if ( bonipress_force_singular_session( $user_id, 'bonipress-last-linkclick' ) )
				wp_send_json( 101 );
			...
			$token = bonipress_verify_token( $_POST['key'], 4 );
		...
		public function has_clicked( $user_id = NULL, $by = '', $check = '' ) {
			...
			$rows  = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bonipress_log_table} WHERE ref = %s AND user_id = %d AND ctype = %s", 'link_click', $user_id, $this->bonipress_type ) );
...
<div class="hook-instance">
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for=""><?php _e( 'Limits', 'bonipress' ); ?></label>
<?php 

			add_filter( 'bonipress_hook_impose_limits', array( $this, 'custom_limit' ) );
...
?>
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				...
				<p class="form-control-static"><a href="https://n3rds.work/docs/bonipress-shortcodes-bonipress_link/" target="_blank">[bonipress_link]</a></p>
			</div>
		</div>
	</div>
</div>
