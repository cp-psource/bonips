 ...
 * @version 1.0
 */
if ( ! class_exists( 'boniPRESS_Hook_Anniversary' ) ) :
	class boniPRESS_Hook_Anniversary extends boniPRESS_Hook {
		...
		public function page_load() {
			...
			$last_run = bonipress_get_user_meta( $user_id, 'anniversary-' . $this->bonipress_type, '', true );
			...
			if ( isset( $result->user_registered ) && $result->difference >= 1 ) {
				...
				for ( $i = 0; $i < $result->difference; $i++ ) {
					...
					$year_joined++;
					if ( $this->core->has_entry( 'anniversary', $year_joined, $user_id, $date_joined, $this->bonipress_type ) ) continue;
			...
			bonipress_update_user_meta( $user_id, 'anniversary-' . $this->bonipress_type, '', $today );
