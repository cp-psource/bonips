...
if ( ! class_exists( 'boniPS_Hook_Comments' ) ) :
	class boniPS_Hook_Comments extends boniPS_Hook {
		...
		public function comment_transitions( $new_status, $old_status, $comment ) {
			...
			if ( apply_filters( 'bonips_comment_gets_cred', true, $comment, $new_status, $old_status ) === false ) return;
			...
			if ( isset( $comment->comment_post_ID ) || $comment->comment_post_ID != '' ) {
				$post = bonips_get_post( (int) $comment->comment_post_ID );
		...
		public function user_exceeds_limit( $user_id = NULL, $post_id = NULL ) {
			...
			if ( $this->prefs['limits']['per_post'] > 0 ) {
				...
				if ( ! $this->is_main_type )
					$limit = bonips_get_user_meta( $user_id, 'bonips_comment_limit_post_' . $this->bonips_type, '', true );
				else
					$limit = bonips_get_user_meta( $user_id, 'bonips_comment_limit_post', '', true );
				...
				// Save
				if ( ! $this->is_main_type )
					bonips_update_user_meta( $user_id, 'bonips_comment_limit_post_' . $this->bonips_type, '', $limit );
				else
					bonips_update_user_meta( $user_id, 'bonips_comment_limit_post', '', $limit );
			...
			if ( $this->prefs['limits']['per_day'] > 0 ) {
				...
				// Grab limit
				if ( ! $this->is_main_type )
					$limit = bonips_get_user_meta( $user_id, 'bonips_comment_limit_day_' . $this->bonips_type, '', true );
				else
					$limit = bonips_get_user_meta( $user_id, 'bonips_comment_limit_day', '', true );
				...
				// Save
				if ( ! $this->is_main_type )
					bonips_update_user_meta( $user_id, 'bonips_comment_limit_day_' . $this->bonips_type, '', $limit );
				else
					bonips_update_user_meta( $user_id, 'bonips_comment_limit_day', '', $limit );
		...
		public function preferences() {
			...
			if ( function_exists( 'dsq_is_installed' ) )
				echo '<p>' . $this->core->template_tags_general( __( '%plural% are only awarded when your website has been synced with the Disqus server!', 'bonips' ) ) . '</p>';

?>
<div class="hook-instance">
	<h3><?php _e( 'Approved Comments', 'bonips' ); ?></h3>
	<div class="row">
		<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'approved' => 'creds' ) ); ?>"><?php _e( 'Member', 'bonips' ); ?></label>
		...
		<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'approved' => 'author' ) ); ?>"><?php _e( 'Content Author', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'approved' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'approved' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'approved' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['approved']['log'] ); ?>" class="form-control" />
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'limits' => 'per_post' ) ); ?>"><?php _e( 'Limit per post', 'bonips' ); ?></label>
				...
				<span class="description"><?php echo $this->core->template_tags_general( __( 'The number of comments per post that grants %_plural% to the comment author. Use zero for unlimited.', 'bonips' ) ); ?></span>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'limits' => 'per_day' ) ); ?>"><?php _e( 'Limit per day', 'bonips' ); ?></label>
				...
				<span class="description"><?php echo $this->core->template_tags_general( __( 'Number of comments per day that grants %_plural%. Use zero for unlimited.', 'bonips' ) ); ?></span>
			</div>
		</div>
	</div>
</div>
<div class="hook-instance">
	<h3><?php _e( 'SPAM Comments', 'bonips' ); ?></h3>
	<div class="row">
		<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'spam' => 'creds' ) ); ?>"><?php _e( 'Member', 'bonips' ); ?></label>
		...
		<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'spam' => 'author' ) ); ?>"><?php _e( 'Content Author', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'spam' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'spam' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'spam' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['spam']['log'] ); ?>" class="form-control" />
				<span class="description"><?php echo $this->available_template_tags( array( 'general', 'comment' ) ); ?></span>
			</div>
		</div>
	</div>
...
<div class="hook-instance">
	<h3><?php _e( 'Trashed Comments', 'bonips' ); ?></h3>
	<div class="row">
		<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'trash' => 'creds' ) ); ?>"><?php _e( 'Member', 'bonips' ); ?></label>
		...
		<div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'trash' => 'author' ) ); ?>"><?php _e( 'Content Author', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'trash' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'trash' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'trash' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['trash']['log'] ); ?>" class="form-control" />
				<span class="description"><?php echo $this->available_template_tags( array( 'general', 'comment' ) ); ?></span>
			</div>
		</div>
	</div>
