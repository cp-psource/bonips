...
/**
 * Get Settings
 * Returns boniPRESS's general settings.
 * @since 0.1
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_settings' ) ) :
	function bonipress_get_settings()
	{
		_deprecated_function( 'bonipress_get_settings()', '1.4', 'bonipress()' );
...
/**
 * boniPRESS Query Rankings Class
 * @see http://codex.bonipress.me/classes/bonipress_query_rankings/
 * @since 1.1.2
 * @version 1.1
 */
if ( ! class_exists( 'boniPRESS_Query_Rankings' ) ) {
	class boniPRESS_Query_Rankings {
		...
		public function get_rankings() {
			...
			$SQL = apply_filters( 'bonipress_ranking_sql', "
SELECT {$select} 
FROM {$wpdb->users} 
...
/**
 * boniPRESS_Rankings class
 * @see http://codex.bonipress.me/classes/bonipress_rankings/
 * @since 0.1
 * @version 2.0
 */
if ( !class_exists( 'boniPRESS_Rankings' ) ) {
	class boniPRESS_Rankings {
		...
		public function get_leaderboard() {
			return '<ol class="boniPRESS-leaderboard">' . $this->loop( 'li' ) . '</ol>';
		}
		...
		public function loop( $wrap = '' ) {
			...
			foreach ( $this->result as $position => $row ) {
				...
				$layout = apply_filters( 'bonipress_ranking_row', $layout, $this->args['template'], $row, $position+1 );
...
/**
 * Get boniPRESS Rankings
 * Returns the boniPRESS_Rankings object containing results.
 *
 ...
 * for each user
 * @uses boniPRESS_Query_Rankings()
 * @uses boniPRESS_Rankings()
 * @returns class object
 * @since 0.1
 * @version 2.0
 */
if ( !function_exists( 'bonipress_rankings' ) ) {
	function bonipress_rankings( $args = array(), $reset = false )
	{
		$default = array(
			'number'      => '-1',
			'order'       => 'DESC',
		...
		elseif ( !empty( $diff ) ) {
			$ranking = new boniPRESS_Query_Rankings( $args );
			$ranking->get_rankings();
		
			$_rankings = $ranking->result;
		}
		$bonipress_rankings = new boniPRESS_Rankings( $args, $_rankings );

		return $bonipress_rankings;
	}
}
 ...
 * @version 1.1
 */
if ( !function_exists( 'bonipress_rankings_position' ) ) {
	function bonipress_rankings_position( $user_id = '', $type = 'bonipress_default' )
	{
		$rankings = bonipress_rankings( array( 'type' => $type ) );
