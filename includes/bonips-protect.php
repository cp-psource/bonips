...
/**
 * boniPS_Protect class
 * Will be replaced by openssl_encrypt() in 1.8
 * @since 0.1
 * @version 1.3
 */
if ( ! class_exists( 'boniPS_Protect' ) ) :
	class boniPS_Protect {
		...
		public function __construct( $custom_key = NULL ) {
			...
			if ( $custom_key !== NULL )
				...
				$skey = bonips_get_option( 'bonips_key', false );
		...
		public function reset_key() {
			...
			bonips_update_option( 'bonips_key', $skey );
			$this->skey = $skey;
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_protect' ) ) :
	function bonips_protect() {

		if ( ! class_exists( 'boniPS_Protect' ) || BONIPS_DISABLE_PROTECTION ) return false;

		global $bonips_protect;

		if ( ! is_object( $bonips_protect ) )
			$bonips_protect = new boniPS_Protect();

		return $bonips_protect;
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_create_token' ) ) :
	function bonips_create_token( $string = '' ) {
		...
		return apply_filters( 'bonips_create_token', $encoded, $string );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_verify_token' ) ) :
	function bonips_verify_token( $string = '', $length = 1 ) {
		...
		return apply_filters( 'bonips_verify_token', $reply, $string, $length );
	}
endif;
