...
/**
 * Get boniPRESS Leaderboard
 * Returns the boniPRESS_Leaderboard object containing results.
 *
...
if ( ! function_exists( 'bonipress_get_leaderboard' ) ) :
	function bonipress_get_leaderboard( $args = array(), $reset = false, $raw = false )
	{
		$default = array(
			'number'      => '-1',
			'order'       => 'DESC',
		...
		return new boniPRESS_Query_Leaderboard( $args );
	}
endif;
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_leaderboard_position' ) ) :
	function bonipress_leaderboard_position( $user_id = '', $type = 'bonipress_default' )
	{
		$args = array( 'type' => $type );
		$rankings = bonipress_get_leaderboard( $args);
		$leaderboard = new boniPRESS_Leaderboard( $args, $rankings->get_results() );
		return apply_filters( 'bonipress_get_leaderboard_position', $leaderboard->users_position( $user_id ), $user_id, $type );
	}
endif;

/**
 * boniPRESS Query Leaderboard Class
 ...
 * @see http://codex.bonipress.me/classes/bonipress_leaderboard/
 * @since 1.4
 * @version 1.0
 */
if ( ! class_exists( 'boniPRESS_Leaderboard' ) ) :
	class boniPRESS_Leaderboard {
		...
		public function __construct( $args = array(), $results = array() ) {
			if ( ! isset( $args['type'] ) || empty( $args['type'] ) )
				$args['type'] = 'bonipress_default';
			
			// Get settings
			$bonipress = bonipress( $args['type'] );
			$this->core = $bonipress;
			$this->args = $args;
			$this->result = $results;
		}
