...
/**
 * boniPRESS_BuddyPress_Module class
 * @since 0.1
 * @version 1.3.2
 */
if ( ! class_exists( 'boniPRESS_BuddyPress_Module' ) ) :
	class boniPRESS_BuddyPress_Module extends boniPRESS_Module {
			...
			parent::__construct( 'boniPRESS_BuddyPress', array(
				...
				'defaults'    => array(
					...
					'history_menu_title' => array(
						'me'      => __( "Wallet", 'bonipress' ),
						'others'  => __( "%s's Wallet", 'bonipress' )
					),
					'history_menu_pos'   => 99,
					'history_url'        => 'bonipress-history',
					'history_num'        => 10
				),
				'register'    => false,
				'add_to_core' => true
		...
		public function adjust_logout( $logouturl, $redirect ) {
			...
			return apply_filters( 'bonipress_bp_logout_url', esc_url( $logouturl ), $this );
		...
		public function show_balance_profile() {
			...
			if ( ! $this->buddypress['visibility']['balance'] && ! bp_is_my_profile() && ! bonipress_is_admin() ) return;

			// Loop though all post types
			$bonipress_types = bonipress_get_types();
			if ( ! empty( $bonipress_types ) ) {

				$template = $this->buddypress['balance_template'];
				foreach ( $bonipress_types as $type => $label ) {

					// Load boniPRESS with this points type
					...
					$output  .= sprintf( '<div class="bp-widget bonipress"><h4>%s</h4><table class="profile-fields"><tr class="field_1 field_current_balance_' . $type . '"><td class="label">%s</td><td class="data">%s</td></tr></table></div>', $bonipress->plural(), __( 'Aktuelles Guthaben', 'bonipress' ), $bonipress->format_creds( $balance ) );

				}

			}

			echo apply_filters( 'bonipress_bp_profile_details', $output, $balance, $this );
		...
		public function show_balance( $dump = NULL, $context = 'header' ) {
			...
			if ( ! empty( $bonipress_types ) ) {
				...
				foreach ( $bonipress_types as $type => $label ) {
					...
					$output  .= '<div class="bonipress-balance bonipress-' . $type . '">' . $template . ' ' . $bonipress->format_creds( $balance ) . '</div>';

				}
			
			}

			echo apply_filters( 'bonipress_bp_profile_header', $output, $this->buddypress['balance_template'], $this );
		...
		public function setup_nav() {
			...
			$date_sorting = apply_filters( 'bonipress_sort_by_time', array(
				...
				'thismonth' => __( 'Diesen Monat', 'bonipress' )
			) );
		...
		public function my_history() {
			...
			add_filter( 'bonipress_log_paginate_class', array( $this, 'paginate_class' ) );
		...
		public function my_history_title() {
			...
			echo apply_filters( 'bonipress_br_history_page_title', $title, $this );
		...
		public function my_history_screen() {
			...
			$args = array(
				'user_id' => bp_displayed_user_id(),
				'number'  => apply_filters( 'bonipress_bp_history_num_to_show', $this->buddypress['history_num'] ),
				'ctype'   => $type
			);
...
?>
<form action="" id="bonipress-sort-cred-history-form" method="get"><label><?php _e( 'Show:', 'bonipress' ); ?></label> <?php bonipress_types_select_from_dropdown( 'show-ctype', 'bonipress-select-type', $type ); ?> <input type="submit" class="btn btn-large btn-primary button button-large button-primary" value="<?php _e( 'Los', 'bonipress' ); ?>" /></form>
<?php
...
<?php do_action( 'bonipress_bp_profile_before_history', $args['user_id'] ); ?>
...
<?php
			...
			do_action( 'bonipress_bp_profile_after_history', $args['user_id'] );

			$output = ob_get_contents();
			ob_end_clean();

			echo apply_filters( 'bonipress_bp_history_page', $output, $this );
		...
		public function after_general_settings( $bonipress = NULL ) {
			...
			$balance_locations = array(
				...
				'top'         => __( 'In Profilheader aufnehmen', 'bonipress' ),
				'profile_tab' => __( 'Als Profil-Registerkarte', 'bonipress' ),
				'both'        => __( 'Als Profil-Registerkarte und im Profilheader', 'bonipress' )
			);

			$history_locations = array(
				''    => __( 'Nicht anzeigen', 'bonipress' ),
				'top' => __( 'Im Profil anzeigen', 'bonipress' )
			);
...
<div class="body" style="display:none;">
	...
	<?php do_action( 'bonipress_bp_before_settings', $this ); ?>

	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'balance_location' ); ?>"><?php echo $this->core->template_tags_general( __( '%singular% Guthaben', 'bonipress' ) ); ?></label>
...
?>
	...
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'history_location' ); ?>"><?php echo $this->core->template_tags_general( __( '%plural% Verläufe', 'bonipress' ) ); ?></label>
...
?>
		...
		<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
			<div class="form-group">
				...
				<p><span class="description"><?php _e( 'Titel mir gezeigt', 'bonipress' ); ?></span></p>
			</div>
		</div>
		<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
			<div class="form-group">
				...
				<p><span class="description"><?php _e( 'Titel anderen gezeigt. Verwende %s, um den Vornamen anzuzeigen.', 'bonipress' ); ?></span></p>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'history_menu_pos' ); ?>"><?php _e( 'Menüposition', 'bonipress' ); ?></label>
				...
				<p><span class="description"><?php printf( '%s %s', __( 'Aktuelle Menüpositionen:', 'bonipress' ), implode( ', ', $bp_nav_positions ) ); ?></span></p>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'history_url' ); ?>"><?php _e( 'Verlaufs-URL-Slug', 'bonipress' ); ?></label>
				...
				<p><span class="description"><?php _e( 'Der Verlaufsseiten-Slug. Muss URL-freundlich sein.', 'bonipress' ); ?></span></p>
			</div>
		</div>
		<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'history_num' ); ?>"><?php _e( 'Anzahl der anzuzeigenden Verlaufseinträge', 'bonipress' ); ?></label>
	...
	<?php do_action( 'bonipress_bp_after_settings', $this ); ?>
...
<?php
		...
		public function sanitize_extra_settings( $new_data, $data, $core ) {
			...
			return apply_filters( 'bonipress_bp_sanitize_settings', $new_data, $data, $core );
