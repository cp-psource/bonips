...
if ( defined( 'boniPRESS_VERSION' ) ) {
	 ...
	 * @version 1.0
	 */
	add_filter( 'bonipress_setup_hooks', 'BuddyPress_Gallery_boniPRESS_Hook' );
	function BuddyPress_Gallery_boniPRESS_Hook( $installed ) {

		$installed['hook_bp_gallery'] = array(
			'title'       => __( 'BuddyPress: Gallery Actions', 'bonipress' ),
			'description' => __( 'Awards %_plural% for creating a new gallery either using BP Album+ or BP Gallery.', 'bonipress' ),
			'callback'    => array( 'boniPRESS_BuddyPress_Gallery' )
		);
	...
	/**
	 * boniPRESS_BuddyPress_Gallery class
	 *
	 * Creds for creating a gallery or deleting gallery
	 * @since 0.1
	 * @version 1.0
	 */
	if ( ! class_exists( 'boniPRESS_BuddyPress_Gallery' ) && class_exists( 'boniPRESS_Hook' ) ) {
		class boniPRESS_BuddyPress_Gallery extends boniPRESS_Hook {
...
<label for="<?php echo $this->field_id( array( 'new_gallery', 'creds' ) ); ?>" class="subheader"><?php echo $this->core->template_tags_general( __( '%plural% for New Gallery', 'bonipress' ) ); ?></label>
<ol>
	<li>
	...
	<li class="empty">&nbsp;</li>
	<li>
		<label for="<?php echo $this->field_id( array( 'new_gallery', 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
