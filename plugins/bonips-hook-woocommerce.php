...
if ( ! function_exists( 'bonips_woo_add_product_variation_detail' ) ) :
	function bonips_woo_add_product_variation_detail( $loop, $variation_data, $variation ) {
		...
		$prefs   = (array) get_post_meta( $variation->ID, '_bonips_reward', true );
...
<?php
		...
		foreach ( $types as $point_type => $point_type_label ) {
			...
			if ( ! $bonips->can_edit_creds( $user_id ) ) continue;
...
if ( ! function_exists( 'bonips_woo_save_reward_settings' ) ) :
	function bonips_woo_save_reward_settings( $post_id ) {
		...
		if ( ! isset( $_POST['bonips_reward'] ) || empty( $_POST['bonips_reward'] ) || get_post_type( $post_id ) != 'product' ) return;
		...
		if ( empty( $new_setup ) )
			delete_post_meta( $post_id, 'bonips_reward' );
		else
			update_post_meta( $post_id, 'bonips_reward', $new_setup );
...
if ( ! function_exists( 'bonips_woo_save_product_variation_detail' ) ) :
	function bonips_woo_save_product_variation_detail( $post_id ) {
		...
		if ( empty( $new_setup ) )
			delete_post_meta( $post_id, '_bonips_reward' );
		else
			update_post_meta( $post_id, '_bonips_reward', $new_setup );
	...
	function bonips_get_woo_product_reward( $product_id = NULL, $variation_id = NULL, $requested_type = false ) {
		...
		if ( function_exists( 'wc_get_product' ) ) {
			...
			if ( $product->is_type( 'variable' ) && $variation_id !== NULL && $variation_id > 0 ) {
				...
				$reward_setup        = (array) get_post_meta( $variation_id, '_bonips_reward', true );
				$parent_reward_setup = (array) get_post_meta( $parent_product_id, 'bonips_reward', true );
			}
			else {
				$reward_setup        = (array) get_post_meta( $product_id, 'bonips_reward', true );
