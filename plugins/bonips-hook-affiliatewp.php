 ...
 * @version 1.1
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_affiliatewp_hook', 10 );
function bonips_register_affiliatewp_hook( $installed ) {
 ...
 * @version 1.1
 */
add_action( 'bonips_load_hooks', 'bonips_load_affiliatewp_hook', 10 );
function bonips_load_affiliatewp_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_AffiliateWP' ) || ! class_exists( 'Affiliate_WP' ) ) return;

	class boniPS_AffiliateWP extends boniPS_Hook {
		...
		public function before( $formatted, $currency, $amount ) {
			...
			return apply_filters( 'bonips_format_creds', $formatted, $amount, $this->core );
		...
		public function referral_payouts( $referral_id, $new_status, $old_status ) {
			...
			$amount = apply_filters( 'bonips_affiliatewp_payout', $amount, $referral, $new_status, $old_status, $this );
			...
			elseif ( 'paid' === $old_status ) {
				...
				if ( $this->core->has_entry( 'affiliate_referral', $referral_id, $user_id, array( 'ref_type' => 'post' ), $this->bonips_type ) )
					$this->core->add_creds(
...
<div class="hook-instance">
	<h3><?php _e( 'Affiliate-Anmeldung', 'bonips' ); ?></h3>
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'signup', 'creds' ) ); ?>"><?php _e( 'Betrag', 'bonips' ); ?></label>
				...
				<span class="description"><?php _e( 'Verwende Null zum Deaktivieren.', 'bonips' ); ?></span>
			</div>
		</div>
...
<div class="hook-instance">
	...
	<div class="row">
		<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'visits', 'creds' ) ); ?>"><?php _e( 'Betrag', 'bonips' ); ?></label>
		...
		<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'visits', 'limit' ) ); ?>"><?php _e( 'Limit', 'bonips' ); ?></label>
...
<div class="hook-instance">
	<h3><?php _e( 'Referring Sales', 'bonips' ); ?></h3>
	<div class="row">
		<div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="radio">
					<label for="<?php echo $this->field_id( array( 'referrals', 'pay-amount' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( array( 'referrals', 'pay' ) ); ?>" id="<?php echo $this->field_id( array( 'referrals', 'pay-amount' ) ); ?>"<?php checked( $this->prefs['referrals']['pay'], 'creds' ); ?> value="creds" /> <?php _e( 'Zahle einen festgelegten Betrag', 'bonips' ); ?></label>
				</div>
				<label for="<?php echo $this->field_id( array( 'referrals', 'creds' ) ); ?>"><?php _e( 'Betrag', 'bonips' ); ?></label>
				...
				<span class="description"><?php _e( 'Alle Empfehlungen zahlen den gleichen Betrag.', 'bonips' ); ?></span>
			</div>
		</div>
		<div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="radio">
					<label for="<?php echo $this->field_id( array( 'referrals', 'pay-store' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( array( 'referrals', 'pay' ) ); ?>" id="<?php echo $this->field_id( array( 'referrals', 'pay-store' ) ); ?>"<?php checked( $this->prefs['referrals']['pay'], 'currency' ); ?> value="currency" /> <?php _e( 'Zahle den Empfehlungsbetrag', 'bonips' ); ?></label>
				</div>
				<label for="<?php echo $this->field_id( array( 'referrals', 'currency' ) ); ?>"><?php _e( 'Punkte-Währungscode', 'bonips' ); ?></label>
				...
				<span class="description"><?php _e( 'Erfordert, dass AffiliateWP und Dein Geschäft Punkte als Währung verwenden.', 'bonips' ); ?></span>
			</div>
		</div>
		<div class="col-lg-4 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<div class="radio">
					<label for="<?php echo $this->field_id( array( 'referrals', 'pay-ex' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( array( 'referrals', 'pay' ) ); ?>"<?php if ( array_key_exists( $this->currency, $this->point_types ) ) echo ' readonly="readonly"'; ?> id="<?php echo $this->field_id( array( 'referrals', 'pay-ex' ) ); ?>"<?php checked( $this->prefs['referrals']['pay'], 'exchange' ); ?> value="exchange" /> <?php _e( 'Wende einen Wechselkurs an', 'bonips' ); ?></label>
				</div>
				<label for="<?php echo $this->field_id( array( 'referrals', 'exchange' ) ); ?>"><?php _e( 'Wechselkurs', 'bonips' ); ?></label>
				...
				<span class="description"><?php if ( ! array_key_exists( $this->currency, $this->point_types ) ) printf( __( 'Wie viel ist 1 %s in %s wert', 'bonips' ), $this->core->plural(), $this->currency ); else _e( 'Deaktiviert', 'bonips' ); ?></span>
			</div>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'referrals', 'log' ) ); ?>"><?php _e( 'Protokollvorlage – Auszahlung', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'referrals', 'remove_log' ) ); ?>"><?php _e( 'Protokollvorlage – Rückerstattung', 'bonips' ); ?></label>
