 ...
 * @version 1.1
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_invite_anyone_hook', 70 );
function bonips_register_invite_anyone_hook( $installed ) {

	if ( ! function_exists( 'invite_anyone_init' ) ) return $installed;

	$installed['invite_anyone'] = array(
		'title'         => __( 'Invite Anyone Plugin', 'bonips' ),
		'description'   => __( 'Awards %_plural% for sending invitations and/or %_plural% if the invite is accepted.', 'bonips' ),
		'documentation' => 'http://codex.bonips.me/hooks/inviting-users/',
		'callback'      => array( 'boniPS_Invite_Anyone' )
	);
 ...
 * @version 1.4.1
 */
add_action( 'bonips_load_hooks', 'bonips_load_invite_anyone_hook', 70 );
function bonips_load_invite_anyone_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_Invite_Anyone' ) || ! function_exists( 'invite_anyone_init' ) ) return;

	class boniPS_Invite_Anyone extends boniPS_Hook {
		...
		public function verified_signup( $user_id ) {
			...
			// Get Pending List
			$pending = get_transient( 'bonips-pending-bp-signups' );
			...
			// Update pending list
			delete_transient( 'bonips-pending-bp-signups' );
			set_transient( 'bonips-pending-bp-signups', $pending, 7 * DAY_IN_SECONDS );
...
<div class="hook-instance">
	<h3><?php _e( 'Sending Invites', 'bonips' ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'send_invite' => 'limit' ) ); ?>"><?php _e( 'Limit', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'send_invite' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'send_invite' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'send_invite' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['send_invite']['log'] ); ?>" class="form-control" />
...
<div class="hook-instance">
	<h3><?php _e( 'Accepted Invites', 'bonips' ); ?></h3>
	<div class="row">
		<div class="col-lg-2 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
		...
		<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'accept_invite' => 'limit' ) ); ?>"><?php _e( 'Limit', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'accept_invite' => 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( 'accept_invite' => 'log' ) ); ?>" id="<?php echo $this->field_id( array( 'accept_invite' => 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs['accept_invite']['log'] ); ?>" class="form-control" />
				<span class="description"><?php echo $this->available_template_tags( array( 'general' ) ); ?></span>
			</div>
		</div>
	</div>
