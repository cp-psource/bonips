 ...
 * @version 1.1
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_gravity_forms_hook', 65 );
function bonips_register_gravity_forms_hook( $installed ) {

	if ( ! class_exists( 'GFForms' ) ) return $installed;

	$installed['gravityform'] = array(
		'title'         => __( 'Gravityform Submissions', 'bonips' ),
		'description'   => __( 'Awards %_plural% for successful form submissions.', 'bonips' ),
		'documentation' => 'http://codex.bonips.me/hooks/submitting-gravity-forms/',
		'callback'      => array( 'boniPS_Gravity_Forms' )
	);
 ...
 * @version 1.1.1
 */
add_action( 'bonips_load_hooks', 'bonips_load_gravity_forms_hook', 65 );
function bonips_load_gravity_forms_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_Gravity_Forms' ) || ! class_exists( 'GFForms' ) ) return;

	class boniPS_Gravity_Forms extends boniPS_Hook {
		...
		public function form_submission( $lead, $form ) {
			...
			// See if the form contains boniPS fields that override these defaults
			if ( isset( $form['fields'] ) && ! empty( $form['fields'] ) ) {
				foreach ( $form['fields'] as $field ) {

					// Amount override
					if ( $field->label == 'bonips_amount' ) {
						$amount = $this->core->number( $field->defaultValue );
					}

					// Entry override
					if ( $field->label == 'bonips_entry' ) {
		...
		public function preferences() {
			...
			// No forms found
			if ( empty( $forms ) ) {
				echo '<p>' . __( 'No forms found.', 'bonips' ) . '</p>';
				return;
			}
...
<div class="hook-instance">
	<h3><?php printf( __( 'Form: %s', 'bonips' ), $form->title ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $form->id, 'limit' ) ); ?>"><?php _e( 'Limit', 'bonips' ); ?></label>
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $form->id, 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( $form->id, 'log' ) ); ?>" id="<?php echo $this->field_id( array( $form->id, 'log' ) ); ?>" placeholder="<?php _e( 'erforderlich', 'bonips' ); ?>" value="<?php echo esc_attr( $prefs[ $form->id ]['log'] ); ?>" class="form-control" />
