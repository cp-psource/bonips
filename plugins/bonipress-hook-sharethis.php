 ...
 * @version 1.1
 */
add_filter( 'bonipress_setup_hooks', 'bonipress_register_sharethis_hook', 80 );
function bonipress_register_sharethis_hook( $installed ) {

	if ( ! function_exists( 'install_ShareThis' ) ) return $installed;

	$installed['sharethis'] = array(
		'title'         => __( '%plural% for Sharing', 'bonipress' ),
		'description'   => __( 'Awards %_plural% for users sharing / liking your website content to popular social media sites.', 'bonipress' ),
		'documentation' => 'http://codex.bonipress.me/hooks/sharethis-actions/',
		'callback'      => array( 'boniPRESS_ShareThis' )
	);
 ...
 * @version 1.0.1
 */
add_action( 'bonipress_load_hooks', 'bonipress_load_sharethis_hook', 80 );
function bonipress_load_sharethis_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPRESS_ShareThis' ) || ! function_exists( 'install_ShareThis' ) ) return;

	class boniPRESS_ShareThis extends boniPRESS_Hook {
		...
		public function run() {
			...
			add_filter( 'bonipress_parse_log_entry',                          array( $this, 'parse_tags' ), 10, 2 );
			add_action( 'wp_footer',                                       array( $this, 'detect_shares' ), 80 );
			add_action( 'wp_ajax_bonipress-share-this-' . $this->bonipress_type, array( $this, 'ajax' ) );
		...
		public function parse_tags( $content, $log ) {
			...
			$names = bonipress_get_share_service_names();
		...
		public function detect_shares() {
			...
			if ( is_singular() && apply_filters( 'bonipress_load_share_this', true, $this ) ) {
				...
				$post_id = apply_filters( 'bonipress_shared_post_id', $post_id, $this );

?>
<script type="text/javascript">
jQuery(function($) {

	function bonipress_detect_share_<?php echo sanitize_key( $this->bonipress_type ); ?>( event,service ) {
		...
		$.ajax({
			type     : "POST",
			data     : {
				action   : 'bonipress-share-this-<?php echo $this->bonipress_type; ?>',
				token    : '<?php echo wp_create_nonce( 'bonipress-share-this' . $this->bonipress_type ); ?>',
	...
	stLight.subscribe( 'click', bonipress_detect_share_<?php echo sanitize_key( $this->bonipress_type ); ?> );
...
<?php
		...
		public function ajax() {
			...
			check_ajax_referer( 'bonipress-share-this' . $this->bonipress_type, 'token' );
			...
			if ( $this->core->has_entry( 'share', $post_id, $user_id, $data, $this->bonipress_type ) ) wp_send_json( 'HAS ENTRY' );
		...
		public function preferences() {
			...
			if ( $st_public_key === false ) :
				...
				echo '<p>' . __( 'Your ShareThis public key is not set.', 'bonipress' ) . '</p>';

			// Services is not yet setup
			elseif ( $st_services === false ) :

				echo '<p>' . __( 'No ShareThis services detected. Please check your installation.', 'bonipress' ) . '</p>';
...
<div class="hook-instance">
	<h3><?php _e( 'Publishing Posts', 'bonipress' ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $service => 'limit' ) ); ?>"><?php _e( 'Limit', 'bonipress' ); ?></label>
		...
		<div class="col-lg-6 col-md-12 col-sm-12 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $service => 'log' ) ); ?>"><?php _e( 'Log template', 'bonipress' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( array( $service => 'log' ) ); ?>" id="<?php echo $this->field_id( array( $service => 'log' ) ); ?>" placeholder="<?php _e( 'required', 'bonipress' ); ?>" value="<?php echo esc_attr( $this->prefs[ $service ]['log'] ); ?>" class="form-control" />
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_share_service_names' ) ) :
	function bonipress_get_share_service_names() {

		return apply_filters( 'bonipress_get_sharethis_service_name', array(
			'facebook'         => 'Facebook',
