 ...
 * @version 1.1
 */
add_filter( 'bonips_setup_hooks', 'bonips_register_sharethis_hook', 80 );
function bonips_register_sharethis_hook( $installed ) {

	if ( ! function_exists( 'install_ShareThis' ) ) return $installed;

	$installed['sharethis'] = array(
		'title'         => __( '%plural% for Sharing', 'bonips' ),
		'description'   => __( 'Awards %_plural% for users sharing / liking your website content to popular social media sites.', 'bonips' ),
		'documentation' => 'http://codex.bonips.me/hooks/sharethis-actions/',
		'callback'      => array( 'boniPS_ShareThis' )
	);
 ...
 * @version 1.0.1
 */
add_action( 'bonips_load_hooks', 'bonips_load_sharethis_hook', 80 );
function bonips_load_sharethis_hook() {

	// If the hook has been replaced or if plugin is not installed, exit now
	if ( class_exists( 'boniPS_ShareThis' ) || ! function_exists( 'install_ShareThis' ) ) return;

	class boniPS_ShareThis extends boniPS_Hook {
		...
		public function run() {
			...
			add_filter( 'bonips_parse_log_entry',                          array( $this, 'parse_tags' ), 10, 2 );
			add_action( 'wp_footer',                                       array( $this, 'detect_shares' ), 80 );
			add_action( 'wp_ajax_bonips-share-this-' . $this->bonips_type, array( $this, 'ajax' ) );
		...
		public function parse_tags( $content, $log ) {
			...
			$names = bonips_get_share_service_names();
		...
		public function detect_shares() {
			...
			if ( is_singular() && apply_filters( 'bonips_load_share_this', true, $this ) ) {
				...
				$post_id = apply_filters( 'bonips_shared_post_id', $post_id, $this );

?>
<script type="text/javascript">
jQuery(function($) {

	function bonips_detect_share_<?php echo sanitize_key( $this->bonips_type ); ?>( event,service ) {
		...
		$.ajax({
			type     : "POST",
			data     : {
				action   : 'bonips-share-this-<?php echo $this->bonips_type; ?>',
				token    : '<?php echo wp_create_nonce( 'bonips-share-this' . $this->bonips_type ); ?>',
	...
	stLight.subscribe( 'click', bonips_detect_share_<?php echo sanitize_key( $this->bonips_type ); ?> );
...
<?php
		...
		public function ajax() {
			...
			check_ajax_referer( 'bonips-share-this' . $this->bonips_type, 'token' );
			...
			if ( $this->core->has_entry( 'share', $post_id, $user_id, $data, $this->bonips_type ) ) wp_send_json( 'HAS ENTRY' );
		...
		public function preferences() {
			...
			if ( $st_public_key === false ) :
				...
				echo '<p>' . __( 'Your ShareThis public key is not set.', 'bonips' ) . '</p>';

			// Services is not yet setup
			elseif ( $st_services === false ) :

				echo '<p>' . __( 'No ShareThis services detected. Please check your installation.', 'bonips' ) . '</p>';
...
<div class="hook-instance">
	<h3><?php _e( 'Publishing Posts', 'bonips' ); ?></h3>
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( $service => 'limit' ) ); ?>"><?php _e( 'Limit', 'bonips' ); ?></label>
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_get_share_service_names' ) ) :
	function bonips_get_share_service_names() {

		return apply_filters( 'bonips_get_sharethis_service_name', array(
			'facebook'         => 'Facebook',
