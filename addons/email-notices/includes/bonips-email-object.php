...
/**
 * boniPS_Email class
 * @see http://codex.bonips.me/classes/bonips_email/
 * @since 1.8
 * @version 1.0
 */
if ( ! class_exists( 'boniPS_Email' ) ) :
	class boniPS_Email extends boniPS_Object {
		...
		function __construct( $notice_id = NULL ) {
			...
			if ( bonips_get_post_type( $notice_id ) != BONIPS_EMAIL_KEY ) return;
		...
		protected function populate( $notice_id = NULL ) {
			...
			$this->post          = bonips_get_post( $this->post_id );

			$this->point_types   = (array) bonips_get_post_meta( $this->post_id, 'bonips_email_ctype', true );
			if ( empty( $this->point_types ) ) $this->point_types = array( BONIPS_DEFAULT_TYPE_KEY );

			$this->emailnotices  = bonips_get_addon_settings( 'emailnotices' );
			...
			), (array) bonips_get_post_meta( $this->post_id, 'bonips_email_settings', true ) );
			...
			$this->settings = apply_filters( 'bonips_email_notice_settings', $settings, $this->post_id, $this );

			$this->trigger  = bonips_get_post_meta( $this->post_id, 'bonips_email_instance', true );
			$this->last_run = bonips_get_post_meta( $this->post_id, 'bonips_email_last_run', true );
		...
		public function save_settings( $setup = array() ) {
			...
			$saved = bonips_update_post_meta( $this->post_id, 'bonips_email_settings', $setup );

			$saved = apply_filters( 'bonips_email_save_settings', $saved, $this->post_id, $setup, $this );
		...
		public function get_trigger() {
			...
			return apply_filters( 'bonips_get_emails_trigger', $this->trigger, $this );
		...
		public function set_trigger( $instance = '' ) {
			...
			$trigger  = apply_filters( 'bonips_set_email_trigger', $new, $instance, $this );

			if ( $trigger !== false ) {

				bonips_update_post_meta( $this->post_id, 'bonips_email_instance', $trigger );
		...
		public function schedule( $event = array(), $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			$schedule = bonips_add_user_meta( $user_id, 'bonips_scheduled_email_notices', '', array(
			...
			return apply_filters( 'bonips_schedule_email', $schedule, $event, $balance, $point_type, $this );
		...
		public function send( $event = array(), $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			add_filter( 'wp_mail_content_type', 'bonips_get_email_content_type' );

			$result     = wp_mail( $recipients, $subject, $message, $headers );

			remove_filter( 'wp_mail_content_type', 'bonips_get_email_content_type' );

			$this->update_last_run();
            //added  bonips object here for error undefine variable $bonips
            $bonips  = bonips( $point_type );
			return apply_filters( 'bonips_email_send', $result, $event, $bonips, $this );
		...
		public function update_last_run( $timestamp = NULL ) {
			...
			$timestamp = apply_filters( 'bonips_email_update_last_run', absint( $timestamp ), $this );

			if ( $timestamp > 0 ) {

				$result         = bonips_update_post_meta( $this->post_id, 'bonips_email_last_run', $timestamp );
		...
		public function get_email_styling() {
			...
			$style = bonips_get_post_meta( $this->post_id, 'bonips_email_styling', true );

			// Defaults
			if ( empty( $style ) )
				$style = $this->emailnotices['styling'];

			return apply_filters( 'bonips_email_notice_get_styling', $style, $this );
		...
		public function get_recipients( $user = '', $admin = '' ) {
			...
			return apply_filters( 'bonips_email_notice_get_recipients', $emails, $user, $admin, $this );
		...
		public function get_headers() {
			...
			return apply_filters( 'bonips_email_notice_get_headers', $headers, $this );
		...
		public function get_subject( $event = array(), $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			if ( ! empty( $subject ) ) {
				...
				if ( $this->emailnotices['filter']['subject'] === true )
					$subject = bonips_get_the_title( $this->post );

				$subject = $bonips->template_tags_amount( $subject, $event['amount'] );
				$subject = $bonips->template_tags_user( $subject, $event['user_id'] );
				...
				if ( array_key_exists( 'data', $event ) && is_array($event['data']) && ! empty( $event['data'] ) && array_key_exists( 'ref_type', $event['data'] ) && $event['data']['ref_type'] == 'post' )
					$subject = $bonips->template_tags_post( $subject, $event['ref_id'] );

				$subject = str_replace( '%amount%', $event['amount'], $subject );

			}

			return apply_filters( 'bonips_email_notice_get_subject', $subject, $this );
		...
		public function get_body( $event = array(), $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			if ( ! empty( $content ) ) {
				...
				$content = $bonips->template_tags_amount( $content, $event['amount'] );
				// to display correct user names in transfer email
				if( $event['ref']==='transfer' ){
					$content = $bonips->template_tags_user( $content, $event['ref_id'] );
					$content = bonips_transfer_render_message( $content, $event['data'] );
				}elseif( $event['ref']==='woocommerce_payment' ){
					$content = str_replace( '%order_id%', $event['ref_id'], $content );
				}else{
					$content = $bonips->template_tags_user( $content, $event['user_id'] );
				}

				if ( array_key_exists( 'data', $event ) && is_array($event['data']) && ! empty( $event['data'] ) && array_key_exists( 'ref_type', $event['data'] ) && $event['data']['ref_type'] == 'post' )
					$content = $bonips->template_tags_post( $content, $event['ref_id'] );

				$content = str_replace( '%amount%',        $bonips->format_creds( $event['amount'] ), $content );
				$content = str_replace( '%new_balance%',   $bonips->format_creds( $event['new'] ), $content );
				$content = str_replace( '%old_balance%',   $bonips->format_creds( $event['old'] ), $content );
			...
			return apply_filters( 'bonips_email_notice_get_body', $content, $this );
		...
		public function get_message( $event = array(), $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			// Backwards comp.
			$message = apply_filters( 'bonips_email_content_body', $message, $event, $this );

			return apply_filters( 'bonips_email_notice_get_message', $message, $this );
