 ...
 * @version 1.3
 */
if ( ! class_exists( 'boniPRESS_Payment_Gateway' ) ) :
	abstract class boniPRESS_Payment_Gateway {
		...
		public function __construct( $args = array(), $gateway_prefs = NULL ) {
			...
			$this->core             = bonipress();
			$this->buycred          = bonipress_get_buycred_settings();
			...
			// Decode Log Entries
			add_filter( 'bonipress_prep_template_tags',                          array( $this, 'decode_log_entries' ), 10, 2 );
			add_filter( 'bonipress_parse_log_entry_buy_creds_with_' . $this->id, array( $this, 'log_entry' ), 10, 2 );
		...
		public function valid_request() {
			...
			if ( ! isset( $_REQUEST['token'] ) || ! wp_verify_nonce( $_REQUEST['token'], 'bonipress-buy-creds' ) ) return false;
		...
		public function send_json( $content = '' ) {
			...
			$content = apply_filters( 'bonipress_buycred_send_json', $content, $this );
		...
		public function exceeds_limit() {
			...
			$exceeds   = false;
			$remaining = bonipress_user_can_buycred( $this->buyer_id, $this->point_type );
			...
			return apply_filters( 'bonipress_exceeds_buycred_limit', $exceeds, $remaining, $this );
		...
		public function checkout_header() {
			...
			return apply_filters( 'bonipress_buycred_checkout_header', $content, $this );
		...
		public function checkout_footer( $button_label = '' ) {
			...
			if ( ! empty( $this->redirect_fields ) ) {
				...
				$fields = apply_filters( 'bonipress_buycred_redirect_fields', $this->redirect_fields, $this );
			...
			return apply_filters( 'bonipress_buycred_checkout_footer', $content, $this );
		...
		public function checkout_logo( $title = '' ) {
			...
			return apply_filters( 'bonipress_buycred_checkout_logo', $content, $this );
...
<table class="table" cellspacing="0" cellpadding="0">
	...
	<tbody>
		' . implode( '', $table_rows ) . '
			...
			return apply_filters( 'bonipress_buycred_checkout_order', $content, $this );
...
<table>
	<tr>
		<td style="min-width: 100px;"><div class="form-control-static">1 ' . esc_html( $bonipress->singular() ) . '</div></td>
		...
		$content .= '</td>
			...
			echo apply_filters( 'bonipress_buycred_exchange_rate_field', $content, $default, $this );
		...
		public function add_pending_payment( $data ) {
			...
			$check = bonipress_get_page_by_title( $post_title, ARRAY_A, 'buycred_payment' );
			if ( $check === NULL || ( isset( $check['post_status'] ) && $check['post_status'] == 'trash' ) ) {
				...
				if ( $post_id !== NULL && ! is_wp_error( $post_id ) ) {
					...
					bonipress_add_post_meta( $post_id, 'to',         $to, true );
					bonipress_add_post_meta( $post_id, 'from',       $from, true );
					bonipress_add_post_meta( $post_id, 'amount',     $amount, true );
					bonipress_add_post_meta( $post_id, 'cost',       $cost, true );
					bonipress_add_post_meta( $post_id, 'currency',   $currency, true );
					bonipress_add_post_meta( $post_id, 'point_type', $point_type, true);
					bonipress_add_post_meta( $post_id, 'gateway',    $this->id, true );

					bonipress_delete_user_meta( $from, 'buycred_pending_payments' );
			...
			return apply_filters( 'bonipress_add_pending_payment', $post_id, $data );
		...
		public function get_pending_payment( $post_id = NULL ) {
			...
			return apply_filters( 'bonipress_get_pending_payment', $pending_payment, $post_id );
		...
		public function get_recipient_id() {
			...
			return apply_filters( 'bonipress_get_buycred_recipient_id', $recipient_id, $this );
		...
		public function get_amount() {
			...
			$settings           = bonipress_get_buycred_sale_setup( $this->point_type );
			$amount             = false;
			...
			return apply_filters( 'bonipress_get_buycred_amount', $amount, $this );
		...
		public function get_point_type() {
			...
			if ( isset( $_REQUEST['ctype'] ) ) {
				...
				if ( $type_id != '' && bonipress_point_type_exists( $type_id ) )
					$point_type = $type_id;
		...
		public function get_cost( $amount = 0, $point_type = BONIPRESS_DEFAULT_TYPE_KEY, $raw = false, $custom_rate = 0 ) {
			...
			return apply_filters( 'bonipress_buycred_get_cost', $cost, $amount, $point_type, $this->prefs, $setup );
		...
		public function get_thankyou() {
			...
			if ( $this->buycred['thankyou']['use'] == 'page' ) {
				...
				if ( ! empty( $this->buycred['thankyou']['page'] ) )
					$url = bonipress_get_permalink( $this->buycred['thankyou']['page'] );
			...
			$profile_url = bonipress_get_users_profile_url( $this->buyer_id );
			$url         = str_replace( '%profile%', $profile_url, $url );

			return apply_filters( 'bonipress_buycred_thankyou_url', $url, $this );
		...
		public function get_log_entry( $from = 0, $to = 0 ) {
			...
			return apply_filters( 'bonipress_buycred_get_log_entry', $entry, $from, $to, $this );
		...
		public function field_name( $field = '' ) {
			...
			return 'bonipress_pref_buycreds[gateway_prefs][' . $this->id . ']' . $field;
		...
		public function field_id( $field = '' ) {
			...
			return 'bonipress-gateway-prefs-' . str_replace( '_', '-', $this->id ) . '-' . $field;
		...
		public function callback_url() {
			...
			$url = add_query_arg( 'bonipress_call', $this->id, home_url( '/' ) );

			return apply_filters( 'bonipress_buycred_callback_url', $url, $this );
		...
		public function save_log_entry( $id = '', $outcome = '' ) {
			...
			update_option( 'bonipress_buycred_last_call', array(
				'gateway' => $this->id,
				'date'    => time(),
				'outcome' => $outcome,
				'id'      => $id,
...
<?php
			...
			do_action( 'bonipress_buycred_page_top', $title, $reload, $this->id );
		...
		public function get_page_footer() {
			...
			do_action( 'bonipress_buycred_page_footer', $this->id );
...
<?php
		...
		public function get_billing_address_form( $country_dropdown = false ) {
			...
			// Let others play
			$user_details = apply_filters( 'bonipress_buycred_user_details', $user_details, $this );

			// Required Fields
			$required_fields = apply_filters( 'bonipress_buycred_req_fields', array( 'first_name', 'last_name', 'address1', 'city', 'zip', 'state', 'country' ), $this );
...
<?php
			...
			do_action( 'bonipress_buycred_after_billing_details', $user_details, $this );
...
<?php
		...
		public function get_page_redirect( $hidden_fields = array(), $location = '' ) {
			...
			// Logo
			if ( empty( $logo_url ) )
				$logo_url = plugins_url( 'images/cred-icon32.png', boniPRESS_THIS );

			// Hidden Fields
			$hidden_fields = apply_filters( "bonipress_{$id}_purchase_fields", $hidden_fields, $this );
...
<script type="text/javascript"><?php if ( $this->sandbox_mode ) echo '//'; ?>setTimeout( "document.bonipress_<?php echo $id; ?>_request.submit()",2000 );</script>
<?php
		...
		public function transaction_id_is_unique( $transaction_id = '' ) {
			...
			$check = $wpdb->get_results( $wpdb->prepare( $sql, $gateway_id, "%:\"" . $transaction_id . "\";%", $this->bonipress_type ) );
		...
		public function create_unique_transaction_id() {
			...
			do {
				...
				$query = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bonipress_log_table} WHERE ref LIKE %s AND data LIKE %s;", 'buy_creds_with_%', "%:\"" . $id . "\";%" ) );
		...
		public function create_token( $user_id = NULL ) {
			...
			return wp_create_nonce( 'bonipress-buy-' . $this->id );
		...
		public function verify_token( $user_id, $nonce ) {
			...
			if ( substr( wp_hash( $i . 'bonipress-buy-' . $this->id . $uid, 'nonce' ), -12, 10 ) == $nonce )
				return true;
			if ( substr( wp_hash( ( $i - 1 ) . 'bonipress-buy-' . $this->id . $uid, 'nonce' ), -12, 10 ) === $nonce )
				return true;
		...
		public function currencies_dropdown( $name = '', $js = '' ) {
			...
			$currencies = apply_filters( 'bonipress_dropdown_currencies', $currencies, $this->id );
		...
		public function list_option_countries( $selected = '' ) {
			...
			$countries = apply_filters( 'bonipress_list_option_countries', $countries );
		...
		public function IPN_has_required_fields( $required_fields = array(), $method = 'REQUEST' ) {
			...
			$result = apply_filters( 'bonipress_buycred_IPN_missing', $result, $required_fields, $this->id );
		...
		public function IPN_is_valid_sale( $sales_data_key = '', $cost_key = '', $transactionid_key = '', $method = '' ) {
			...
			$result = apply_filters( 'bonipress_buycred_valid_sale', $result, $sales_data_key, $cost_key, $transactionid_key, $method, $this );
		...
		public function complete_payment( $pending_payment = NULL, $transaction_id = '' ) {
			...
			$reply      = false;
			$bonipress     = bonipress( $pending_payment->point_type );
			...
			if ( ! $bonipress->has_entry( $reference, $pending_payment->buyer_id, $pending_payment->recipient_id, $data, $pending_payment->point_type ) ) {

				add_filter( 'bonipress_get_email_events', array( $this, 'email_notice' ), 10, 2 );
				$reply = $bonipress->add_creds(
					$reference,
				...
				remove_filter( 'bonipress_get_email_events', array( $this, 'email_notice' ), 10, 2 );

			}

			return apply_filters( 'bonipress_buycred_complete_payment', $reply, $transaction_id, $this );
