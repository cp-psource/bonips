...
/**
 * boniPS_Payment_Gateway class
 * @since 0.1
 * @version 1.3
 */
if ( ! class_exists( 'boniPS_Payment_Gateway' ) ) :
	abstract class boniPS_Payment_Gateway {
		...
		public function __construct( $args = array(), $gateway_prefs = NULL ) {
			...
			$this->core             = bonips();
			$this->buycred          = bonips_get_buycred_settings();
			...
			// Decode Log Entries
			add_filter( 'bonips_prep_template_tags',                          array( $this, 'decode_log_entries' ), 10, 2 );
			add_filter( 'bonips_parse_log_entry_buy_creds_with_' . $this->id, array( $this, 'log_entry' ), 10, 2 );
		...
		public function valid_request() {
			...
			if ( ! isset( $_REQUEST['token'] ) || ! wp_verify_nonce( $_REQUEST['token'], 'bonips-buy-creds' ) ) return false;

			$valid                = true;

			$this->point_type     = $this->get_point_type();
			if ( $this->point_type != BONIPS_DEFAULT_TYPE_KEY )
				$this->core = bonips( $this->point_type );

			$this->buycred_limit  = bonips_get_buycred_sale_setup( $this->point_type );
			...
			if ( $this->core->exclude_user( $this->buyer_id ) ){
				...
				$this->errors[] = __( 'Käufer sind von diesem Punktetyp ausgeschlossen.', 'bonips' );
			}

			elseif ( $this->core->exclude_user( $this->recipient_id ) ) {
				$valid          = false;
				$this->errors[] = __( 'Der Empfänger ist von diesem Punkttyp ausgeschlossen. ', 'bonips' );
			}

			elseif ( $this->amount === false || $this->amount == 0 ){
				$valid = false;
				$this->errors[] = __( 'Ein Betragswert ist erforderlich.', 'bonips' );
			}

			elseif ( ! empty( $this->bonips_limit['max'] ) && $this->amount > floatval( $this->bonips_limit['max'] ) ){
				$valid = false;
				$this->errors[] = sprintf( __( 'Der Betrag muss kleiner als %d sein.', 'bonips' ), $this->bonips_limit['max'] );
			}

			elseif ( $this->exceeds_limit() ){
				$valid = false;
				$this->errors[] = __( 'Du hast das Limit überschritten.', 'bonips' );
			}
			...
			return apply_filters( 'bonips_valid_buycred_request', $valid, $this );
		...
		public function send_json( $content = '' ) {
			...
			$content = apply_filters( 'bonips_buycred_send_json', $content, $this );
		...
		public function exceeds_limit() {
			...
			$exceeds   = false;
			$remaining = bonips_user_can_buycred( $this->buyer_id, $this->point_type );
			...
			return apply_filters( 'bonips_exceeds_buycred_limit', $exceeds, $remaining, $this );
		...
		public function checkout_header() {
			...
			if ( $this->sandbox_mode )
				$content .= '<div class="checkout-header"><div class="warning">' . esc_js( esc_attr( __( 'Test Modus', 'bonips' ) ) )  . '</div></div>';

			$content .= '<div class="checkout-body padded' . ( ( ! $this->sandbox_mode ) ? ' no-header' : '' ) . '">';

			return apply_filters( 'bonips_buycred_checkout_header', $content, $this );
		...
		public function checkout_footer( $button_label = '' ) {
			...
			if ( $button_label == '' )
				$button_label = __( 'Weiter', 'bonips' );

			$content  = '';
			if ( ! empty( $this->redirect_fields ) ) {

				$fields = apply_filters( 'bonips_buycred_redirect_fields', $this->redirect_fields, $this );
			...
			$button   = apply_filters( 'bonips_buycred_checkout_button', $button, $this );
			...
			return apply_filters( 'bonips_buycred_checkout_footer', $content, $this );
		...
		public function checkout_logo( $title = '' ) {
			...
			return apply_filters( 'bonips_buycred_checkout_logo', $content, $this );
		...
		public function checkout_order() {
			...
			if ( $this->gifting )
				$table_rows[] = '<tr><td colspan="2"><strong>' . esc_js( esc_attr( __( 'Empfänger', 'bonips' ) ) ) . ':</strong> ' . esc_html( get_userdata( $this->recipient_id )->display_name ) . '</td></tr>';

			$table_rows[] = '<tr class="total"><td class="item right">' . esc_js( esc_attr( __( 'Kosten', 'bonips' ) ) ) . '</td><td class="cost right">' . sprintf( '%s %s', $this->cost, $this->prefs['currency'] ) . '</td></tr>';

			$table_rows   = apply_filters( 'bonips_buycred_order_table_rows', $table_rows, $this );
...
<table class="table" cellspacing="0" cellpadding="0">
	<thead>
		<tr>
			<th class="item">' . esc_js( esc_attr( __( 'Artikel', 'bonips' ) ) ) . '</td>
			<th class="cost right">' . esc_js( esc_attr( __( 'Betrag', 'bonips' ) ) ) . '</td>
		</tr>
	</thead>
	<tbody>
		' . implode( '', $table_rows ) . '
			...
			return apply_filters( 'bonips_buycred_checkout_order', $content, $this );
		...
		public function checkout_transaction_id() {
			...
			$content = '<h2><span class="text-mutted">' . esc_js( esc_attr( __( 'Transaktions-ID', 'bonips' ) ) ) . '</span>' . esc_attr( $this->transaction_id ) . '</h2>';

			return apply_filters( 'bonips_buycred_checkout_txtid', $content, $this );
		...
		public function checkout_cancel() {
			...
			$content = '<hr /><div class="cancel"><a href="' . $this->get_cancelled( $this->transaction_id ) . '">' . esc_js( esc_attr( __( 'Kauf abbrechen', 'bonips' ) ) )  . '</a></div>';

			return apply_filters( 'bonips_buycred_checkout_cancel', $content, $this );
...
<table>
	<tr>
		<td style="min-width: 100px;"><div class="form-control-static">1 ' . esc_html( $bonips->singular() ) . '</div></td>
		...
		if ( isset( $this->prefs['currency'] ) )
			$content .= '<span class="bonips-gateway-' . $this->id . '-currency">' . ( ( $this->prefs['currency'] == '' ) ? __( 'Währung wählen', 'bonips' ) : esc_attr( $this->prefs['currency'] ) ) . '</span>';
		...
		$content .= '</td>
			...
			echo apply_filters( 'bonips_buycred_exchange_rate_field', $content, $default, $this );
		...
		public function add_pending_payment( $data ) {
			...
			$check = bonips_get_page_by_title( $post_title, ARRAY_A, 'buycred_payment' );
			if ( $check === NULL || ( isset( $check['post_status'] ) && $check['post_status'] == 'trash' ) ) {
				...
				if ( $post_id !== NULL && ! is_wp_error( $post_id ) ) {
					...
					bonips_add_post_meta( $post_id, 'to',         $to, true );
					bonips_add_post_meta( $post_id, 'from',       $from, true );
					bonips_add_post_meta( $post_id, 'amount',     $amount, true );
					bonips_add_post_meta( $post_id, 'cost',       $cost, true );
					bonips_add_post_meta( $post_id, 'currency',   $currency, true );
					bonips_add_post_meta( $post_id, 'point_type', $point_type, true);
					bonips_add_post_meta( $post_id, 'gateway',    $this->id, true );

					bonips_delete_user_meta( $from, 'buycred_pending_payments' );

					$bonips    = bonips( $point_type );

					$log_entry = $this->first_comment( sprintf( _x( 'Neue Anfrage zum Kauf von %s erhalten.', '%s wird durch die Punktzahl und den Namen ersetzt.', 'bonips' ), $bonips->format_number( $amount ) . ' ' . $bonips->plural() ) );
					$log_entry = apply_filters( 'bonips_new_buycred_request_comment_' . $this->id, $log_entry, $data );
			...
			return apply_filters( 'bonips_add_pending_payment', $post_id, $data );
		...
		public function get_pending_payment( $post_id = NULL ) {
			...
			return apply_filters( 'bonips_get_pending_payment', $pending_payment, $post_id );
		...
		public function get_recipient_id() {
			...
			return apply_filters( 'bonips_get_buycred_recipient_id', $recipient_id, $this );
		...
		public function get_amount() {
			...
			$settings           = bonips_get_buycred_sale_setup( $this->point_type );
			$amount             = false;
			...
			return apply_filters( 'bonips_get_buycred_amount', $amount, $this );
		...
		public function get_point_type() {
			...
			if ( isset( $_REQUEST['ctype'] ) ) {
				...
				if ( $type_id != '' && bonips_point_type_exists( $type_id ) )
					$point_type = $type_id;
		...
		public function get_cost( $amount = 0, $point_type = BONIPS_DEFAULT_TYPE_KEY, $raw = false, $custom_rate = 0 ) {
			...
			if ( array_key_exists( $point_type, $this->prefs['exchange'] ) ) {
				...
				$override = bonips_get_user_meta( $this->current_user_id, 'bonips_buycred_rates_' . $point_type, '', true );
			...
			return apply_filters( 'bonips_buycred_get_cost', $cost, $amount, $point_type, $this->prefs, $setup );
		...
		public function get_thankyou() {
			...
			if ( $this->buycred['thankyou']['use'] == 'page' ) {
				...
				if ( ! empty( $this->buycred['thankyou']['page'] ) )
					$url = bonips_get_permalink( $this->buycred['thankyou']['page'] );
			...
			$profile_url = bonips_get_users_profile_url( $this->buyer_id );
			$url         = str_replace( '%profile%', $profile_url, $url );

			return apply_filters( 'bonips_buycred_thankyou_url', $url, $this );
		...
		public function get_log_entry( $from = 0, $to = 0 ) {
			...
			return apply_filters( 'bonips_buycred_get_log_entry', $entry, $from, $to, $this );
		...
		public function field_name( $field = '' ) {
			...
			return 'bonips_pref_buycreds[gateway_prefs][' . $this->id . ']' . $field;
		...
		public function field_id( $field = '' ) {
			...
			return 'bonips-gateway-prefs-' . str_replace( '_', '-', $this->id ) . '-' . $field;
		...
		public function callback_url() {
			...
			$url = add_query_arg( 'bonips_call', $this->id, home_url( '/' ) );

			return apply_filters( 'bonips_buycred_callback_url', $url, $this );
		...
		public function save_log_entry( $id = '', $outcome = '' ) {
			...
			update_option( 'bonips_buycred_last_call', array(
				'gateway' => $this->id,
				'date'    => time(),
				'outcome' => $outcome,
				'id'      => $id,
		...
		public function get_page_header( $site_title = '', $reload = false ) {
			...
			// Set Title
			if ( $this->sandbox_mode )
				$title = __( 'Testzahlung', 'bonips' );

			elseif ( isset( $this->label ) )
				$title = $this->label;

			else
				$title = __( 'Zahlung', 'bonips' );
...
<head>
	...
	<link rel="stylesheet" href="<?php echo plugins_url( 'assets/css/bootstrap-grid.css', boniPS_THIS ); ?>" type="text/css" media="all" />
	<link rel="stylesheet" href="<?php echo plugins_url( 'assets/css/bonips-forms.css', boniPS_THIS ); ?>" type="text/css" media="all" />
	<?php do_action( 'bonips_buycred_page_header', $title, $reload, $this->id ); ?>

</head>
<body class="bonips-metabox">
	<div class="row">
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 text-right">
			...
			<p><a href="<?php echo $this->get_cancelled( $this->transaction_id ); ?>" id="return-where-we-came-from"><?php _e( 'Abbrechen', 'bonips' ); ?></a></p>
		</div>
	</div>
<?php

			do_action( 'bonips_buycred_page_top', $title, $reload, $this->id );
		...
		public function get_page_footer() {
			...
			do_action( 'bonips_buycred_page_footer', $this->id );
...
<?php
		...
		public function get_billing_address_form( $country_dropdown = false ) {
			...
			if ( ! isset( $_POST['billing']['address1'] ) ) {
				...
				// Else grab PSeCommerce User Fields
				elseif ( class_exists( 'PSeCommerce' ) ) {
			...
			// Let others play
			$user_details = apply_filters( 'bonips_buycred_user_details', $user_details, $this );

			// Required Fields
			$required_fields = apply_filters( 'bonips_buycred_req_fields', array( 'first_name', 'last_name', 'address1', 'city', 'zip', 'state', 'country' ), $this );
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-first-name"><?php _e( 'Vorname', 'bonips' ); ?></label>
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-last-name"><?php _e( 'Nachname', 'bonips' ); ?></label>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-address1"><?php _e( 'Adresszeile 1', 'bonips' ); ?></label>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-address2"><?php _e( 'Adresszeile 2', 'bonips' ); ?></label>
...
<div class="row">
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-city"><?php _e( 'Stadt', 'bonips' ); ?></label>
	...
	<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-zip"><?php _e( 'Postleitzahl', 'bonips' ); ?></label>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-state"><?php _e( 'Bundesland', 'bonips' ); ?></label>
...
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="form-group">
			<label for="billing-country"><?php _e( 'Land', 'bonips' ); ?></label>

			<?php if ( $country_dropdown !== false ) : ?>

			<select name="billing[country]" id="billing-country" class="form-control">
				<option value=""><?php _e( 'Land auswählen', 'bonips' ); ?></option>
...
<?php
			...
			do_action( 'bonips_buycred_after_billing_details', $user_details, $this );
...
<?php
		...
		public function get_page_redirect( $hidden_fields = array(), $location = '' ) {
			...
			// Logo
			if ( empty( $logo_url ) )
				$logo_url = plugins_url( 'images/cred-icon32.png', boniPS_THIS );

			// Hidden Fields
			$hidden_fields = apply_filters( "bonips_{$id}_purchase_fields", $hidden_fields, $this );

?>
<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<form name="bonips_<?php echo $id; ?>_request" class="form text-center" action="<?php echo $location; ?>" method="post" id="redirect-form">
			...
			<noscript><input type="submit" name="submit-form" value="<?php printf( __( 'Weiter zu %s', 'bonips' ), $this->label ); ?>" /></noscript>
			<p id="manual-continue"><a href="javascript:void(0);" onclick="document.bonips_<?php echo $id; ?>_request.submit();return false;"><?php _e( 'Klicke hier, wenn Du nicht automatisch weitergeleitet wirst', 'bonips' ); ?></a></p>
		</form>
	</div>
</div>
<script type="text/javascript"><?php if ( $this->sandbox_mode ) echo '//'; ?>setTimeout( "document.bonips_<?php echo $id; ?>_request.submit()",2000 );</script>
<?php
		...
		public function transaction_id_is_unique( $transaction_id = '' ) {
			...
			$check = $wpdb->get_results( $wpdb->prepare( $sql, $gateway_id, "%:\"" . $transaction_id . "\";%", $this->bonips_type ) );
		...
		public function create_unique_transaction_id() {
			...
			do {
				...
				$query = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bonips_log_table} WHERE ref LIKE %s AND data LIKE %s;", 'buy_creds_with_%', "%:\"" . $id . "\";%" ) );
		...
		public function create_token( $user_id = NULL ) {
			...
			return wp_create_nonce( 'bonips-buy-' . $this->id );
		...
		public function verify_token( $user_id, $nonce ) {
			...
			if ( substr( wp_hash( $i . 'bonips-buy-' . $this->id . $uid, 'nonce' ), -12, 10 ) == $nonce )
				return true;
			if ( substr( wp_hash( ( $i - 1 ) . 'bonips-buy-' . $this->id . $uid, 'nonce' ), -12, 10 ) === $nonce )
				return true;
		...
		public function currencies_dropdown( $name = '', $js = '' ) {
			...
			$currencies = apply_filters( 'bonips_dropdown_currencies', $currencies, $this->id );
			...
			echo '<option value="">' . __( 'Wählen', 'bonips' ) . '</option>';
		...
		public function item_types_dropdown( $name = '' ) {
			...
			$types = apply_filters( 'bonips_dropdown_item_types', $types );
		...
		public function list_option_countries( $selected = '' ) {
			...
			$countries = apply_filters( 'bonips_list_option_countries', $countries );
		...
		public function list_option_us_states( $selected = '', $non_us = false ) {
			...
			$states = apply_filters( 'bonips_list_option_us', $states );
		...
		public function list_option_months( $selected = '' ) {
			...
			$months = array (
				...
				"07"  =>  __( 'Juli', 'bonips' ),
				"08"  =>  __( 'August', 'bonips' ),
				"09"  =>  __( 'September', 'bonips' ),
				"10"  =>  __( 'Oktober', 'bonips' ),
				"11"  =>  __( 'November', 'bonips' ),
				"12"  =>  __( 'Dezember', 'bonips' )
			);
		...
		public function IPN_has_required_fields( $required_fields = array(), $method = 'REQUEST' ) {
			...
			$result = apply_filters( 'bonips_buycred_IPN_missing', $result, $required_fields, $this->id );
		...
		public function IPN_is_valid_sale( $sales_data_key = '', $cost_key = '', $transactionid_key = '', $method = '' ) {
			...
			$result = apply_filters( 'bonips_buycred_valid_sale', $result, $sales_data_key, $cost_key, $transactionid_key, $method, $this );
		...
		public function complete_payment( $pending_payment = NULL, $transaction_id = '' ) {
			...
			$reply      = false;
			$bonips     = bonips( $pending_payment->point_type );
			...
			if ( ! $bonips->has_entry( $reference, $pending_payment->buyer_id, $pending_payment->recipient_id, $data, $pending_payment->point_type ) ) {

				add_filter( 'bonips_get_email_events', array( $this, 'email_notice' ), 10, 2 );
				$reply = $bonips->add_creds(
					$reference,
				...
				remove_filter( 'bonips_get_email_events', array( $this, 'email_notice' ), 10, 2 );

			}

			return apply_filters( 'bonips_buycred_complete_payment', $reply, $transaction_id, $this );
