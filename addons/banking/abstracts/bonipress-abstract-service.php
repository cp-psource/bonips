...
/**
 * boniPRESS_Service class
 * @see http://codex.bonipress.me/classes/bonipress_service/
 * @since 1.2
 * @version 1.1
 */
if ( ! class_exists( 'boniPRESS_Service' ) ) :
	abstract class boniPRESS_Service {
		 ...
		 */
		public function run() {
			wp_die( 'function boniPRESS_Service::run() must be over-ridden in a sub-class.' );
		}
		...
		public function get_days_in_year() {
			...
			return apply_filters( 'bonipress_banking_days_in_year', $days, $this );
		...
		public function is_large_site() {
			...
			if ( is_multisite() && wp_is_large_network() )
				...
				if ( $users['total_users'] > 3000 )
					...
					$total = $wpdb->get_var( "SELECT COUNT(*) FROM {$bonipress_log_table};" );
		...
		public function get_excluded_user_ids() {
			...
			if ( $this->prefs['min_balance'] != '' && $this->core->number( $this->prefs['min_balance'] ) > $this->core->zero() ) {
				...
				$user_ids = $wpdb->get_col( $wpdb->prepare( "SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value < {$format};", $this->bonipress_type, $this->prefs['min_balance'] ) );
				if ( ! empty( $user_ids ) ) {
					foreach ( $user_ids as $user_id ) {
						if ( in_array( $user_id, $exclude_ids ) ) continue;
						$exclude_ids[] = absint( $user_id );
		...
		public function date_to_timestamp( $string = '' ) {
			...
			return bonipress_date_to_gmt_timestamp( $string );
		...
		public function timestamp_to_date( $string = '' ) {
			...
			return bonipress_gmt_timestamp_to_local( $string );
		...
		public function time_select( $name = '', $id = '', $date = NULL ) {
			...
			$options = bonipress_banking_get_time_options();
