 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_banking_get_next_payout' ) ) :
	function bonipress_banking_get_next_payout( $prefs = '', $time = 0 ) {
		...
		return apply_filters( 'bonipress_banking_get_next_payout', $time, $prefs );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_banking_get_timeframes' ) ) :
	function bonipress_banking_get_timeframes() {

		$timeframes = array(
			'hourly'    => array(
				'label'       => __( 'Stündlich', 'bonipress' ),
				'single'      => __( 'Stunde', 'bonipress' ),
				'date_format' => 'G'
			),
			'daily'     => array(
				'label'       => __( 'Täglich', 'bonipress' ),
				'single'      => __( 'Tag', 'bonipress' ),
				'date_format' => 'z'
			),
			'weekly'    => array(
				'label'       => __( 'Wöchentlich', 'bonipress' ),
				'single'      => __( 'Woche', 'bonipress' ),
				'date_format' => 'W'
			),
			'monthly'   => array(
				'label'       => __( 'Monatlich', 'bonipress' ),
				'single'      => __( 'Monat', 'bonipress' ),
				'date_format' => 'M'
			),
			'quarterly'  => array(
				'label'       => __( 'Vierteljährlich', 'bonipress' ),
				'single'      => __( 'Quartal', 'bonipress' ),
				'date_format' => 'Y'
			),
			'semiannually'  => array(
				'label'       => __( 'Halbjährlich', 'bonipress' ),
				'single'      => __( 'Halbjährlich', 'bonipress' ),
				'date_format' => 'Y'
			),
			'annually'  => array(
				'label'       => __( 'Jährlich', 'bonipress' ),
				'single'      => __( 'Jährlich', 'bonipress' ),
				'date_format' => 'Y'
			)
		);

		return apply_filters( 'bonipress_banking_timeframes', $timeframes );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_banking_get_time_options' ) ) :
	function bonipress_banking_get_time_options( $twelve_hour = false ) {
		...
		return apply_filters( 'bonipress_banking_time_options', $options );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_get_recurring_payout_defaults' ) ) :
	function bonipress_get_recurring_payout_defaults() {
...
if ( ! function_exists( 'bonipress_add_new_recurring_payout' ) ) :
	function bonipress_add_new_recurring_payout( $new_id = '', $schedule_setup = array(), $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		// Save
		bonipress_update_option( 'bonipress-recurring-schedule-' . $point_type, $schedules );

		// Add Schedule to CRON
		wp_schedule_single_event( $last_run, 'bonipress-recurring-' . $new_id, array( 'id' => $new_id ) );
...
if ( ! function_exists( 'bonipress_delete_recurring_payout' ) ) :
	function bonipress_delete_recurring_payout( $id_to_remove = NULL, $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
		...
		if ( ! empty( $schedules ) ) {
			...
			// Save
			bonipress_update_option( 'bonipress-recurring-schedule-' . $point_type, $new_schedule );
