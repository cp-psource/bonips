...
/**
 * boniPS Bank Service - Interest
 * @since 1.2
 * @version 1.2
 */
if ( ! class_exists( 'boniPS_Banking_Service_Interest' ) ) :
	class boniPS_Banking_Service_Interest extends boniPS_Service {
		...
		function __construct( $service_prefs, $type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			parent::__construct( array(
				...
				'defaults' => array(
					...
					'log'           => __( '%plural% Zinszahlung', 'bonips' ),
					'min_balance'   => 1,
					'exclude_ids'   => '',
					'exclude_roles' => array()
				)
			...
			$this->cron_compound_key = 'bonips_bank_interest_comp' . $type;
			$this->cron_payout_key   = 'bonips_bank_interest_pay' . $type;
			$this->compound_meta_key = $type . '_comp';
			$this->log_reference     = apply_filters( 'bonips_bank_interest_reference', 'interest', $this );
		...
		public function run() {
			...
			add_action( 'bonips_bank_interest_comp' . $this->bonips_type, array( $this, 'do_compounding' ), 10 );
			add_action( 'bonips_bank_interest_pay' . $this->bonips_type,  array( $this, 'do_interest_payout' ), 10 );
			...
			if ( $payout === false ) {
				...
				$now += bonips_banking_get_next_payout( $this->prefs['rate']['pay_out'] );
		...
		public function do_compounding() {
			...
			$number     = absint( apply_filters( 'bonips_compound_max_limit', 1500, $this ) );
			...
			if ( $this->is_large_site() ) {
				...
				$transient_key = 'bonips-compoun-' . $this->bonips_type;
				...
				$original_start = bonips_get_option( 'bonips_todays_compound_started', false );
				if ( $original_start === false )
					update_option( 'bonips_todays_compound_started', $this->now );
			...
			$wheres        = array();
			$wheres[]      = $wpdb->prepare( "meta_key = %s", $this->bonips_type );
			...
			$period        = apply_filters( 'bonips_compound_period', 1, $this );
			if ( $period > 0 ) $period = ( $period / $days_in_year );

			// Get the balance key
			$balance_key   = bonips_get_meta_key( $this->bonips_type );
			...
			if ( ! empty( $user_balances ) ) {
				...
				foreach ( $user_balances as $entry ) {
					...
					if ( $balance <= $this->core->zero() && apply_filters( 'bonips_compound_negative_interest', false, $this ) === false ) continue;

					// Get the interest we have earned up until now
					$saved_interest = bonips_get_user_meta( $entry->user_id, $this->compound_meta_key, '', true );
					...
					if ( $interest_rate > 0 ) {
						...
						bonips_update_user_meta( $entry->user_id, $this->compound_meta_key, '', $interest );
			...
			if ( $limit != '' ) {
				...
				if ( count( $user_balances ) < $number || $offset > $total_rows ) {
					...
					$original_start = (int) bonips_get_option( 'bonips_todays_compound_started', $this->now );
					delete_option( 'bonips_todays_compound_started' );
		...
		public function do_interest_payout() {
			...
			// Default number of balances we will be running through for now
			$number        = absint( apply_filters( 'bonips_compound_pay_max_limit', 1500, $this ) );
			...
			if ( $total_rows > $number ) {
				...
				$original_start = bonips_get_option( 'bonips_todays_compound_payout_started', false );
				if ( $original_start === false )
					update_option( 'bonips_todays_compound_payout_started', $now );
			...
			if ( ! empty( $pending_interests ) ) {
				...
				foreach ( $pending_interests as $entry ) {
					...
					if ( ! $this->core->has_entry( $this->log_reference, $entry->umeta_id, $entry->user_id, '', $this->bonips_type ) ) {
						...
						if ( $payout ) {
							$total_rows--;
							bonips_delete_user_meta( $entry->user_id, $this->compound_meta_key );
						}
			...
			if ( $total_rows <= 0 ) {
				...
				$original_start = (int) bonips_get_option( 'bonips_todays_compound_payout_started', $now );
				if ( $original_start != $now )
					delete_option( 'bonips_todays_compound_payout_started' );

				// Schedule next run
				$next = $original_start + bonips_banking_get_next_payout( $this->prefs['rate']['pay_out'] );
		...
		public function get_users_interest_rate( $user_id = 0, $default = 0 ) {
			...
			$rate  = $default;
			$saved = bonips_get_user_meta( $user_id, 'bonips_banking_rate_' . $this->bonips_type, '', true );
			if ( strlen( $saved ) > 0 )
				$rate = $saved;

			return apply_filters( 'bonips_get_users_interest_rate', $rate, $user_id, $default, $this );
...
<div class="row">
	<div class="col-sm-6">
		...
		<table class="widefat fixed striped" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					<th style="width: 100%;" colspan="4"><?php _e( 'Täglicher zusammengesetzter Zeitplan', 'bonips' ); ?></th>
				</tr>
				<tr>
					<th style="width: 30%;"><?php _e( 'Jahr', 'bonips' ); ?></th>
					<th style="width: 20%;"><?php _e( 'Monat', 'bonips' ); ?></th>
					<th style="width: 20%;"><?php _e( 'Tag', 'bonips' ); ?></th>
					<th style="width: 30%;"><?php _e( 'Zeit', 'bonips' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<tr>
	...
	<div class="col-sm-6">
		...
		<table class="widefat fixed striped" cellpadding="0" cellspacing="0">
			<thead>
				<tr>
					<th style="width: 100%;" colspan="4"><?php _e( 'Auszahlungsplan', 'bonips' ); ?></th>
				</tr>
				<tr>
					<th style="width: 30%;"><?php _e( 'Jahr', 'bonips' ); ?></th>
					<th style="width: 20%;"><?php _e( 'Monat', 'bonips' ); ?></th>
...
<div class="row">
	<div class="col-sm-6">
		<h3><?php _e( 'Einrichtung', 'bonips' ); ?></h3>
		<div class="row">
			<div class="col-sm-3 col-xs-10">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'rate' => 'amount' ) ); ?>"><?php _e( 'Zinssatz', 'bonips' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( array( 'rate' => 'amount' ) ); ?>" id="<?php echo $this->field_id( array( 'rate' => 'amount' ) ); ?>" class="form-control" placeholder="<?php _e( 'Benötigt', 'bonips' ); ?>" value="<?php echo esc_attr( $this->prefs['rate']['amount'] ); ?>" />
				</div>
			</div>
			<div class="col-xs-1">
				<div class="form-group">
			...
			<div class="col-sm-4 col-xs-6">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'name' => 'pay_out' ) ); ?>"><?php _e( 'Auszahlungshäufigkeit', 'bonips' ); ?></label>
					<?php $this->timeframe_dropdown( array( 'rate' => 'pay_out' ), false, false ); ?>
				</div>
			</div>
			<div class="col-sm-4 col-xs-6">
				<div class="form-group">
					<label for="<?php echo $this->field_id( 'min_balance' ); ?>"><?php _e( 'Mindestguthaben', 'bonips' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( 'min_balance' ); ?>" id="<?php echo $this->field_id( 'min_balance' ); ?>" class="form-control" placeholder="<?php _e( 'Benötigt', 'bonips' ); ?>" value="<?php echo $this->core->number( $this->prefs['min_balance'] ); ?>" />
				</div>
			</div>
		</div>
	</div>
	<div class="col-sm-6">
		<h3><?php _e( 'Auszahlungsprotokolltabelle', 'bonips' ); ?></h3>
		<div class="row">
			<div class="col-xs-12">
				<div class="form-group">
...
<div class="row">
	<div class="col-xs-12">
		<p><span class="description"><?php _e( 'Das Ändern des Auszahlungszeitraums nach Aktivierung des Dienstes wird erst wirksam, wenn die aktuell geplante Auszahlung ausgeführt wird. Um dies zu ändern, musst Du auch den obigen Auszahlungsplan anpassen.', 'bonips' ); ?></span></p>
	</div>
</div>
<div class="row">
	<div class="col-sm-6">
		<h3><?php _e( 'Nach ID ausschließen', 'bonips' ); ?></h3>
		<div class="row">
			<div class="col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( 'exclude_ids' ); ?>"><?php _e( 'Durch Kommas getrennte Liste der Benutzer-IDs', 'bonips' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( 'exclude_ids' ); ?>" id="<?php echo $this->field_id( 'exclude_ids' ); ?>" class="form-control" placeholder="<?php _e( 'Durch Kommas getrennte Liste der Benutzer-IDs', 'bonips' ); ?>" value="<?php echo esc_attr( $this->prefs['exclude_ids'] ); ?>" />
				</div>
			</div>
		</div>
	</div>
	<div class="col-sm-6">
		<h3><?php _e( 'Nach Rolle ausschließen', 'bonips' ); ?></h3>
		<div class="row">
			<div class="col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( 'exclude_roles' ); ?>"><?php _e( 'Auszuschließende Rollen', 'bonips' ); ?></label>
				</div>
				<div class="row">
<?php
...
<?php
			...
			do_action( 'bonips_banking_compound_interest', $this );
		...
		public function sanitise_preferences( $post ) {
			...
			return apply_filters( 'bonips_banking_save_interest', $new_settings, $this, $rescheduled );
		...
		public function user_screen( $user ) {
			...
			$users_rate = bonips_get_user_meta( $user->ID, 'bonips_banking_rate_' . $this->bonips_type, '', true );

?>
<table class="form-table">
	<tr>
		<th><label for="compoun-interest-rate-<?php echo $this->bonips_type; ?>"><?php _e( 'Zinssatz', 'bonips' ); echo ' (' . $this->core->plural() . ')'; ?></label></th>
		<td>
			<input type="text" size="8" name="bonips_interest_rate[<?php echo $this->bonips_type; ?>][rate]" id="compoun-interest-rate-<?php echo $this->bonips_type; ?>" placeholder="<?php echo esc_attr( $this->prefs['rate']['amount'] . ' %' ); ?>" value="<?php echo esc_attr( $users_rate ); ?>" /> %
			<p><span class="description"><?php _e( 'Leer lassen, um die Standardrate zu bezahlen.', 'bonips' ); ?></span></p>
		</td>
	</tr>
</table>
<?php
		...
		function save_custom_rate( $user_id ) {
			...
			if ( isset( $_POST['bonips_interest_rate'] ) && array_key_exists( $this->bonips_type, $_POST['bonips_interest_rate'] ) ) {

				$rate = sanitize_text_field( $_POST['bonips_interest_rate'][ $this->bonips_type ]['rate'] );
				if ( strlen( $rate ) > 0 ) {

					$rate = str_replace( ',', '.', $rate );
					if ( $rate != $this->prefs['rate']['amount'] )
						bonips_update_user_meta( $user_id, 'bonips_banking_rate_' . $this->bonips_type, '', $rate );
					else
						bonips_delete_user_meta( $user_id, 'bonips_banking_rate_' . $this->bonips_type );
