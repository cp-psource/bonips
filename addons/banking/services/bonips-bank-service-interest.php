...
if ( ! class_exists( 'boniPS_Banking_Service_Interest' ) ) {
	class boniPS_Banking_Service_Interest extends boniPS_Service {
		...
		public function run() {
			...
			add_action( 'bonips_banking_interest_compound', array( $this, 'do_compound' ) );
			add_action( 'bonips_banking_do_compound_batch', array( $this, 'do_compound_batch' ) );
			
			add_action( 'bonips_banking_interest_payout',   array( $this, 'do_payouts' ) );
			add_action( 'bonips_banking_interest_do_batch', array( $this, 'do_interest_batch' ) );
		}
		...
		public function deactivate() {
			...
			$timestamp = wp_next_scheduled( 'bonips_banking_interest_compound' );
			if ( $timestamp !== false )
				wp_clear_scheduled_hook( $timestamp, 'bonips_banking_interest_compound' );

			// Unschedule payouts
			$timestamp = wp_next_scheduled( 'bonips_banking_interest_payout' );
			if ( $timestamp !== false )
				wp_clear_scheduled_hook( $timestamp, 'bonips_banking_interest_payout' );
		}
		...
		public function process() {
			...
			if ( ! wp_next_scheduled( 'bonips_banking_interest_compound' ) ) {
				wp_schedule_event( time(), 'daily', 'bonips_banking_interest_compound' );
			}
			...
			if ( $this->time_to_run( $this->prefs['rate']['pay_out'], $last_payout ) ) {
				...
				// Schedule Payouts
				if ( wp_next_scheduled( 'bonips_banking_interest_payout' ) === false )
					wp_schedule_single_event( time(), 'bonips_banking_interest_payout' );
			}
		}
		...
		public function do_compound() {
			...
			if ( (int) $total > $threshold ) {
				...
				foreach ( $batches as $batch_id => $batch ) {
					...
					if ( wp_next_scheduled( $run_time, 'bonips_banking_do_compound_batch', array( $batch ) ) === false )
						wp_schedule_single_event( $run_time, 'bonips_banking_do_compound_batch', array( $batch ) );
				}
			}
			else {
		...
		public function do_compound_batch( $batch ) {
			if ( !empty( $batch ) && is_array( $batch ) ) {
				...
				foreach ( $batch as $user_id ) {
					...
					// Get past interest
					$past_interest = bonips_get_user_meta( $user_id, $this->core->get_cred_id() . '_comp', '', true );
					...
					// Save interest
					bonips_update_user_meta( $user_id, $this->core->get_cred_id() . '_comp', '', $interest );
				}
			}
		}
		...
		public function do_payouts() {
			...
			if ( (int) $total > $threshold ) {
				...
				foreach ( $batches as $batch_id => $batch ) {
					$set = $set+1;
					$run_time = ( $time + ( 60*$set ) );
					if ( wp_next_scheduled( $run_time, 'bonips_banking_interest_do_batch', array( $batch ) ) === false )
						wp_schedule_single_event( $run_time, 'bonips_banking_interest_do_batch', array( $batch ) );
				}
			}
			else {
		...
		public function do_interest_batch( $batch ) {
			if ( !empty( $batch ) && is_array( $batch ) ) {
				...
				foreach ( $batch as $user_id ) {
					...
					// Reset past interest
					bonips_update_user_meta( $user_id, $this->core->get_cred_id() . '_comp', '', 0 );
				}
			}
		}
		...
		public function save( $id, $now ) {
			...
			// Get Bank settings
			$bank = bonips_get_option( 'bonips_pref_bank' );
			
			// Update settings
			$bank['service_prefs'][$this->id] = $this->prefs;

			// Save new settings
			bonips_update_option( 'bonips_pref_bank', $bank );
		}
		...
		public function preferences() {
			$prefs = $this->prefs; ?>
					...
					<label class="subheader"><?php _e( 'Zinssatz', 'bonips' ); ?></label>
					<ol class="inline">
						<li>
							<label>&nbsp;</label>
						...
						<li>
							<label for="<?php echo $this->field_id( array( 'rate' => 'pay_out' ) ); ?>"><?php _e( 'Bezahlt / Berechnet', 'bonips' ); ?></label><br />
						...
						<li class="block">
							...
							<span class="description"><?php _e( 'Der Zinssatz kann entweder positiv oder negativ sein und wird täglich berechnet.', 'bonips' ); ?></span>
						</li>
					</ol>
					<label class="subheader"><?php _e( 'Mindestguthaben', 'bonips' ); ?></label>
					<ol>
						<li>
							...
							<span class="description"><?php _e( 'Das Minimum-Guthaben, damit Zinsen gelten.', 'bonips' ); ?></span>
						</li>
					</ol>
					<label class="subheader"><?php _e( 'Protokollvorlage', 'bonips' ); ?></label>
					<ol>
						<li>
...
<?php
		...
		function sanitise_preferences( $post ) {
			...
			return apply_filters( 'bonips_banking_save_interest', $new_settings, $this->prefs );
		}
	}
}
