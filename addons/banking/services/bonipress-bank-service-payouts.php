...
if ( !class_exists( 'boniPRESS_Banking_Service_Payouts' ) ) {
	class boniPRESS_Banking_Service_Payouts extends boniPRESS_Service {
		...
		function __construct( $service_prefs ) {
			parent::__construct( array(
				...
				'defaults' => array(
					'amount'     => 10,
					'rate'       => 'daily',
					'log'        => __( 'Tägliche %_plural%', 'bonipress' ),
					'excludes'   => '',
					'cycles'     => 0,
					'last_run'   => '',
					'run_time'   => 60
		...
		public function run() {
			...
			add_action( 'bonipress_banking_recurring_payout', array( $this, 'do_payouts' ) );
			add_action( 'bonipress_banking_do_batch',         array( $this, 'do_payout_batch' ), 10, 3 );
		}
		...
		public function process() {
			...
			if ( $this->time_to_run( $this->prefs['rate'], $last_run ) ) {
				...
				// Schedule payouts
				if ( wp_next_scheduled( 'bonipress_banking_recurring_payout', array( $cycles ) ) === false )
					wp_schedule_single_event( time(), 'bonipress_banking_recurring_payout', array( $cycles ) );
			}
		}
		...
		public function do_payouts( $cycle = NULL ) {
			...
			if ( (int) $total > $threshold ) {
				...
				foreach ( $batches as $batch_id => $batch ) {
					...
					if ( wp_next_scheduled( $run_time, 'bonipress_banking_do_batch', array( $batch, $set, $cycle ) ) === false )
						wp_schedule_single_event( $run_time, 'bonipress_banking_do_batch', array( $batch, $set, $cycle ) );
				}
				set_transient( 'bonipress_banking_num_payout_batches', $set, HOUR_IN_SECONDS );
			}
		...
		public function do_payout_batch( $batch, $set = NULL, $cycle = NULL ) {
			if ( !empty( $batch ) && is_array( $batch ) ) {
				...
				if ( $set !== NULL ) {
					$total = get_transient( 'bonipress_banking_num_payout_batches' );
					if ( $total !== false && $set == $total ) {
						delete_transient( 'bonipress_banking_num_payout_batches' );
		...
		public function save( $now = 0, $cycles = 0, $deactivate = false ) {
			...
			// Get Bank settings
			$bank = get_option( 'bonipress_pref_bank' );
			...
			// Save new settings
			update_option( 'bonipress_pref_bank', $bank );
		}
		...
		public function preferences() {
			...
			if ( empty( $last_run ) )
				$last_run = __( 'Noch nicht ausgeführt', 'bonipress' );
			else
				$last_run = date_i18n( get_option( 'date_format' ) . ' : ' . get_option( 'time_format' ), $last_run ); ?>

					
					<label class="subheader"><?php _e( 'Benutzer bezahlen', 'bonipress' ); ?></label>
					<ol class="inline">
						<li>
							...
							<span class="description"><?php _e( 'Kann nicht Null sein.', 'bonipress' ); ?></span>
							<input type="hidden" name="<?php echo $this->field_name( 'last_run' ); ?>" value="<?php echo $prefs['last_run']; ?>" />
						</li>
						<li>
							<label for="<?php echo $this->field_id( 'rate' ); ?>"><?php _e( 'Intervall', 'bonipress' ); ?></label><br />
							<?php $this->timeframe_dropdown( 'rate', false ); ?>

						</li>
						<li>
							<label><?php _e( 'Zyklen', 'bonipress' ); ?></label>
							...
							<span class="description"><?php _e( 'Für unbegrenzt auf -1 setzen', 'bonipress' ); ?></span>
						</li>
						<li>
							<label><?php _e( 'Letzter Lauf / Aktiviert', 'bonipress' ); ?></label><br />
							<div class="h2"><?php echo $last_run; ?></div>
						</li>
						<li class="block"><strong><?php _e( 'Intervall', 'bonipress' ); ?></strong><br /><?php echo $this->core->template_tags_general( __( 'Wähle aus, wie oft Du %_plural% vergeben möchtest. Beachte, dass bei Aktivierung dieses Dienstes die erste Auszahlung zu Beginn des nächsten Zeitraums erfolgt. Bei einem "täglichen" Intervall erfolgt die erste Auszahlung also als erstes am Morgen.', 'bonipress' ) ); ?></li>
						<li class="block"><strong><?php _e( 'Cycles', 'bonipress' ); ?></strong><br /><?php _e( 'In Zyklen kannst Du auswählen, wie viele Intervalle dieser Dienst ausgeführt werden soll. Jedes Mal, wenn ein Zyklus ausgeführt wird, verringert sich der Wert, bis er Null erreicht. In diesem Fall wird dieser Dienst selbst deaktiviert. Verwende -1, um unbegrenzt oft zu laufen.', 'bonipress' ); ?></li>
						<li class="block"><strong><?php _e( 'Important', 'bonipress' ); ?></strong><br /><?php _e( 'Du kannst Auszahlungen jederzeit stoppen, indem Du diesen Dienst deaktivierst. Denke daran, dass dieser Dienst fortgesetzt wird, wenn Du ihn deaktivierst, während noch Zyklen übrig sind, wenn er erneut aktiviert wird. Setze die Zyklen zum Zurücksetzen auf Null.', 'bonipress' ); ?></li>
					</ol>
					<label class="subheader" for="<?php echo $this->field_id( 'excludes' ); ?>"><?php _e( 'Ausgeschlossen', 'bonipress' ); ?></label>
					<ol>
						<li>
							...
							<span class="description"><?php _e( 'Durch Kommas getrennte Liste von Benutzer-IDs, die von diesem Dienst ausgeschlossen werden sollen. Keine Leerzeichen erlaubt!', 'bonipress' ); ?></span>
						</li>
					</ol>
					<label class="subheader" for="<?php echo $this->field_id( 'log' ); ?>"><?php _e( 'Protokollvorlage', 'bonipress' ); ?></label>
					<ol>
						<li>
...
<?php
		...
		function sanitise_preferences( $post ) {
			...
			return apply_filters( 'bonipress_banking_save_recurring', $new_settings, $this->prefs );
		}
	}
}
?>