...
if ( ! class_exists( 'boniPRESS_Banking_Service_Interest' ) ) {
	class boniPRESS_Banking_Service_Interest extends boniPRESS_Service {
		...
		public function run() {
			...
			add_action( 'bonipress_banking_interest_compound', array( $this, 'do_compound' ) );
			add_action( 'bonipress_banking_do_compound_batch', array( $this, 'do_compound_batch' ) );
			
			add_action( 'bonipress_banking_interest_payout',   array( $this, 'do_payouts' ) );
			add_action( 'bonipress_banking_interest_do_batch', array( $this, 'do_interest_batch' ) );
		}
		...
		public function deactivate() {
			...
			$timestamp = wp_next_scheduled( 'bonipress_banking_interest_compound' );
			if ( $timestamp !== false )
				wp_clear_scheduled_hook( $timestamp, 'bonipress_banking_interest_compound' );

			// Unschedule payouts
			$timestamp = wp_next_scheduled( 'bonipress_banking_interest_payout' );
			if ( $timestamp !== false )
				wp_clear_scheduled_hook( $timestamp, 'bonipress_banking_interest_payout' );
		}
		...
		public function process() {
			...
			if ( ! wp_next_scheduled( 'bonipress_banking_interest_compound' ) ) {
				wp_schedule_event( time(), 'daily', 'bonipress_banking_interest_compound' );
			}
			...
			if ( $this->time_to_run( $this->prefs['rate']['pay_out'], $last_payout ) ) {
				...
				// Schedule Payouts
				if ( wp_next_scheduled( 'bonipress_banking_interest_payout' ) === false )
					wp_schedule_single_event( time(), 'bonipress_banking_interest_payout' );
			}
		}
		...
		public function do_compound() {
			...
			$total = count( $users );
			$threshold = (int) apply_filters( 'bonipress_do_banking_limit', 2000 );
			...
			if ( (int) $total > $threshold ) {
				...
				foreach ( $batches as $batch_id => $batch ) {
					...
					if ( wp_next_scheduled( $run_time, 'bonipress_banking_do_compound_batch', array( $batch ) ) === false )
						wp_schedule_single_event( $run_time, 'bonipress_banking_do_compound_batch', array( $batch ) );
				}
			}
			else {
		...
		public function do_compound_batch( $batch ) {
			if ( !empty( $batch ) && is_array( $batch ) ) {
				...
				foreach ( $batch as $user_id ) {
					...
					// Get past interest
					$past_interest = bonipress_get_user_meta( $user_id, $this->core->get_cred_id() . '_comp', '', true );
					...
					// Save interest
					bonipress_update_user_meta( $user_id, $this->core->get_cred_id() . '_comp', '', $interest );
				}
			}
		}
		...
		public function do_payouts() {
			...
			if ( (int) $total > $threshold ) {
				...
				foreach ( $batches as $batch_id => $batch ) {
					$set = $set+1;
					$run_time = ( $time + ( 60*$set ) );
					if ( wp_next_scheduled( $run_time, 'bonipress_banking_interest_do_batch', array( $batch ) ) === false )
						wp_schedule_single_event( $run_time, 'bonipress_banking_interest_do_batch', array( $batch ) );
				}
			}
			else {
		...
		public function do_interest_batch( $batch ) {
			if ( !empty( $batch ) && is_array( $batch ) ) {
				...
				foreach ( $batch as $user_id ) {
					...
					// Reset past interest
					bonipress_update_user_meta( $user_id, $this->core->get_cred_id() . '_comp', '', 0 );
				}
			}
		}
		...
		public function save( $id, $now ) {
			...
			// Get Bank settings
			$bank = bonipress_get_option( 'bonipress_pref_bank' );
			
			// Update settings
			$bank['service_prefs'][$this->id] = $this->prefs;

			// Save new settings
			bonipress_update_option( 'bonipress_pref_bank', $bank );
		}
		...
		public function preferences() {
			$prefs = $this->prefs; ?>
					...
					<label class="subheader"><?php _e( 'Zinssatz', 'bonipress' ); ?></label>
					<ol class="inline">
						<li>
							<label>&nbsp;</label>
						...
						<li>
							<label for="<?php echo $this->field_id( array( 'rate' => 'pay_out' ) ); ?>"><?php _e( 'Bezahlt / Berechnet', 'bonipress' ); ?></label><br />
						...
						<li class="block">
							...
							<span class="description"><?php _e( 'Der Zinssatz kann entweder positiv oder negativ sein und wird täglich berechnet.', 'bonipress' ); ?></span>
						</li>
					</ol>
					<label class="subheader"><?php _e( 'Mindestguthaben', 'bonipress' ); ?></label>
					<ol>
						<li>
							...
							<span class="description"><?php _e( 'Das Minimum-Guthaben, damit Zinsen gelten.', 'bonipress' ); ?></span>
						</li>
					</ol>
					<label class="subheader"><?php _e( 'Protokollvorlage', 'bonipress' ); ?></label>
					<ol>
						<li>
					...
					<label class="subheader" for="<?php echo $this->field_id( 'run_time' ); ?>"><?php _e( 'Laufzeit', 'bonipress' ); ?></label>
					<ol>
						<li>
...
<?php
		...
		function sanitise_preferences( $post ) {
			...
			return apply_filters( 'bonipress_banking_save_interest', $new_settings, $this->prefs );
		}
	}
}
