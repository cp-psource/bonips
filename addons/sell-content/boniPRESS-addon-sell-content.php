...
if ( ! class_exists( 'boniPRESS_Sell_Content_Module' ) ) :
	class boniPRESS_Sell_Content_Module extends boniPRESS_Module {
		...
		function __construct() {
			...
			parent::__construct( 'boniPRESS_Sell_Content_Module', array(
		...
		public function module_init() {
			...
			$this->priority        = apply_filters( 'bonipress_sell_content_priority', 25, $this );

			// Email add-on support
			add_filter( 'bonipress_get_email_events',         array( $this, 'email_notice_instance' ), 10, 2 );
			add_filter( 'bonipress_email_before_send',        array( $this, 'email_notices' ), 40, 2 );
		...
		public function module_admin_init() {
			...
			// User Override
			add_action( 'bonipress_user_edit_after_balances', array( $this, 'sell_content_user_screen' ), 50 );
		...
		public function register_assets() {
			...
			wp_register_script(
				'bonipress-sell-this',
				plugins_url( 'assets/js/buy-content.js', boniPRESS_SELL ),
				array( 'jquery' ),
				'2.0.1',
				true
			);
		...
		public function enqueue_footer() {
			...
			global $bonipress_sell_this;

			// Only enqueue our script if it's needed
			if ( $bonipress_sell_this === true ) {
				...
				wp_localize_script(
					...
					'boniPRESSBuyContent',
					array(
						'ajaxurl'    => esc_url( ( isset( $post->ID ) ) ? bonipress_get_permalink( $post->ID ) : home_url( '/' ) ),
						'token'      => wp_create_nonce( 'bonipress-buy-this-content' ),
						'working'    => esc_js( $this->sell_content['working'] ),
						'reload'     => $this->sell_content['reload'],
						'sweeterror' => __( 'Error', 'bonipress' )
					)
				);

				wp_enqueue_script( 'bonipress-sell-this' );
		...
		public function template_redirect() {
			...
			global $bonipress_partial_content_sale;

			$bonipress_partial_content_sale = false;
		...
		public function maybe_buy_content() {
			...
			if ( is_user_logged_in() && ! bonipress_is_admin() ) {

				if ( isset( $_POST['action'] ) && $_POST['action'] == 'bonipress-buy-content' && isset( $_POST['postid'] ) && isset( $_POST['token'] ) && wp_verify_nonce( $_POST['token'], 'bonipress-buy-this-content' ) ) {
					...
					if ( ! array_key_exists( $point_type, $bonipress_types ) || bonipress_force_singular_session( $this->current_user_id, 'bonipress-last-content-purchase' ) )
						wp_send_json( 'ERROR' );

					// If the content is for sale and we have not paid for it
					if ( bonipress_post_is_for_sale( $post_id ) && ! bonipress_user_paid_for_content( $this->current_user_id, $post_id ) ) {

						$content  = '';
						$post     = bonipress_get_post( $post_id );
						$purchase = bonipress_sell_content_new_purchase( $post, $this->current_user_id, $point_type );

						// Successfull purchase
						if ( $purchase === true ) {

							preg_match('/\[bonipress_sell_this[^\]]*](.*)\[\/bonipress_sell_this[^\]]*]/uis', $post->post_content , $match );

							$content = $post->post_content;
							if ( is_array( $match ) && array_key_exists( 1, $match ) )
								$content = $match[1];

							do_action( 'bonipress_sell_before_content_render' );
						...
						// Let others play
						$content = apply_filters( 'bonipress_content_purchase_ajax', $content, $purchase );
		...
		public function the_content( $content ) {
			...
			if ( bonipress_post_is_for_sale( $post_id ) ) {
				...
				if ( $bonipress_partial_content_sale === true )
					return $_content;
				...
				if ( is_user_logged_in() ) {
					...
					if ( ! bonipress_is_admin() && $post->post_author != $this->current_user_id ) {

						// In case we have not paid
						if ( ! bonipress_user_paid_for_content( $this->current_user_id, $post_id ) ) {

							// Get Payment Options
							$payment_options = bonipress_sell_content_payment_buttons( $this->current_user_id, $post_id );
		...
		public function psf_content() {
			...
			if ( bonipress_post_is_for_sale( $post_id ) && ( psf_is_single_forum() || psf_is_single_topic() || psf_is_single_reply() ) ) {
				...
				// Partial Content Sale - We have already done the work in the shortcode
				if ( $bonipress_partial_content_sale === true ) return;
		...
		public function sell_content_user_screen( $user ) {
			...
			foreach ( $bonipress_types as $point_type_key => $label ) {
				...
				if ( ! empty( $this->sell_content['type'] ) && in_array( $point_type_key, $this->sell_content['type'] ) ) {
					...
					if ( ! $bonipress->exclude_user( $user->ID ) ) {

						$setup['excluded'] = false;

						$settings          = bonipress_get_option( 'bonipress_sell_this_' . $point_type_key );

						$setup['default']  = $settings['profit_share'];

						$users_share = bonipress_get_user_meta( $user->ID, 'bonipress_sell_content_share_' . $point_type_key, '', true );
...
?>
<p class="bonipress-p"><?php _e( 'Users profit share when their content is purchased.', 'bonipress' ); ?></p>
<table class="form-table bonipress-inline-table">
	<tr>
		<th scope="row"><?php _e( 'Profit Share', 'bonipress' ); ?></th>
		<td>
			<fieldset id="bonipress-badge-list" class="badge-list">
				<legend class="screen-reader-text"><span><?php _e( 'Profit Share', 'bonipress' ); ?></span></legend>
<?php
...
?>
				<div class="bonipress-wrapper buycred-wrapper disabled-option color-option">
					<div><?php printf( _x( '%s Profit Share', 'Points Name', 'bonipress' ), $data['name'] ); ?></div>
					<div class="balance-row">
						<div class="balance-view"><?php _e( 'Disabled', 'bonipress' ); ?></div>
						<div class="balance-desc"><em><?php _e( 'Not accepted as payment.', 'bonipress' ); ?></em></div>
					</div>
				</div>
<?php
...
?>
				<div class="bonipress-wrapper buycred-wrapper disabled-option color-option">
					...
					<div class="balance-row">
						...
						<div class="balance-desc"><em><?php printf( _x( 'User can not pay using %s', 'Points Name', 'bonipress' ), $data['name'] ); ?></em></div>
					</div>
				</div>
<?php
...
?>
				<div class="bonipress-wrapper buycred-wrapper color-option selected">
					<div><?php printf( _x( '%s Profit Share', 'Buying Points', 'bonipress' ), $data['name'] ); ?></div>
					<div class="balance-row">
						<div class="balance-view"><input type="text" size="8" name="bonipress_sell_this[<?php echo $point_type; ?>]" class="half" placeholder="<?php echo esc_attr( $data['default'] ); ?>" value="<?php if ( $data['override'] ) echo esc_attr( $data['custom'] ); ?>" /> %</div>
						<div class="balance-desc"><em><?php _e( 'Leave empty to use the default.', 'bonipress' ); ?></em></div>
					</div>
				</div>
<?php
...
<?php
		...
		function save_manual_profit_share( $user_id ) {
			...
			if ( isset( $_POST['bonipress_sell_this'] ) && ! empty( $_POST['bonipress_sell_this'] ) ) {

				foreach ( $_POST['bonipress_sell_this'] as $point_type => $share ) {

					$share = sanitize_text_field( $share );

					bonipress_delete_user_meta( $user_id, 'bonipress_sell_content_share_' . $point_type );
					if ( $share != '' && is_numeric( $share ) )
						bonipress_update_user_meta( $user_id, 'bonipress_sell_content_share_' . $point_type, '', $share );
		...
		protected function enabled_options( $selected = '' ) {
			...
			$options = array(
				'disabled' => __( 'Disabled', 'bonipress' ),
				'enabled'  => __( 'Enabled', 'bonipress' )
			);
		...
		public function after_general_settings( $bonipress = NULL ) {
			...
			$post_types     = bonipress_sell_content_post_types();
			$selected_types = explode( ',', $this->sell_content['post_types'] );

			$point_types    = bonipress_get_types( true );

?>
<h4><span class="dashicons dashicons-admin-plugins static"></span><?php _e( 'Sell Content', 'bonipress' ); ?></h4>
<div class="body" style="display:none;">

	<h3><?php _e( 'Post Types', 'bonipress' ); ?></h3>
	<p><?php _e( 'Which post type(s) content field do you want to sell access to?', 'bonipress' ); ?></p>
	<div id="bonipress-sell-this-post-type-filter">
<?php
...
?>
	<div class="row">
		<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
			<div class="checkbox">
				<label for="<?php echo $this->field_id( array( 'post_types' => $post_type ) ); ?>"><input type="checkbox" name="<?php echo $this->field_name( array( 'post_types' => $post_type ) ); ?>"<?php echo $selected; ?> id="<?php echo $this->field_id( array( 'post_types' => $post_type ) ); ?>" class="bonipress-check-count" data-type="<?php echo $post_type; ?>" value="<?php echo $post_type; ?>" /> <?php echo esc_attr( $post_type_label ); ?></label>
			</div>
		</div>
...
<?php
					...
					$options = bonipress_get_post_type_options( $post_type );
					if ( ! empty( $options ) ) {
...
?>
	...
	<h3><?php _e( 'Point Types', 'bonipress' ); ?></h3>
	<p><?php _e( 'Which point type(s) can be used as payment for accessing content?', 'bonipress' ); ?></p>
	<div class="row">
...
?>
			<div class="form-group">
				<label for="<?php echo $this->field_id( array( 'type' => $point_type ) ); ?>"><input type="checkbox" name="<?php echo $this->field_name( array( 'type' => $point_type ) ); ?>"<?php echo $selected; ?> id="<?php echo $this->field_id( array( 'type' => $point_type ) ); ?>" class="bonipress-check-count" data-type="<?php echo $point_type; ?>" value="<?php echo $point_type; ?>" /> <?php echo esc_attr( $point_type_label ); ?></label>
			</div>
<?php
...
<?php
			...
			if ( ! empty( $point_types ) ) {
				foreach ( $point_types as $point_type => $point_type_label ) {
					...
					$type_setup = bonipress_get_option( 'bonipress_sell_this_' . $point_type );
					...
					$expiration_label = apply_filters( 'bonipress_sell_exp_title', __( 'Hour(s)', 'bonipress' ), $point_type );

?>
	<div id="bonipress-sell-<?php echo $point_type; ?>-wrap" style="display: <?php echo $selected; ?>;">
		<h3><?php printf( __( '%s Setup', 'bonipress' ), $point_type_label ); ?></h3>
		<div class="row">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-status' ) ); ?>"><?php _e( 'Default Status', 'bonipress' ); ?></label>
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-price' ) ); ?>"><?php _e( 'Default Price', 'bonipress' ); ?></label>
			...
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-expire' ) ); ?>"><?php _e( 'Expiration', 'bonipress' ); ?></label>
					...
					<p><span class="description"><?php printf( __( 'Option to automatically expire purchases after certain number of %s. Use zero to disable.', 'bonipress' ), $expiration_label ); ?></span></p>
				</div>
			</div>
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-profit-share' ) ); ?>"><?php _e( 'Profit Share', 'bonipress' ); ?></label>
					...
					<p><span class="description"><?php printf( __( 'Option to pay a percentage of each sale with the content author.', 'bonipress' ), $expiration_label ); ?></span></p>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-button' ) ); ?>"><?php _e( 'Button Label', 'bonipress' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( array( 'post_type_setup' => $point_type ) ); ?>[button_label]" id="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-button' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'bonipress' ); ?>" value="<?php echo esc_attr( $type_setup['button_label'] ); ?>" />
					<p><span class="description"><?php echo $this->core->available_template_tags( array(), '%price%' ); ?></span></p>
				</div>
			</div>
			<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-css' ) ); ?>"><?php _e( 'Button CSS Classes', 'bonipress' ); ?></label>
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-log-pay' ) ); ?>"><?php _e( 'Payment log entry template', 'bonipress' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( array( 'post_type_setup' => $point_type ) ); ?>[log_payment]" id="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-log-pay' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'bonipress' ); ?>" value="<?php echo esc_attr( $type_setup['log_payment'] ); ?>" />
					<p><span class="description"><?php echo $this->core->available_template_tags( array( 'general', 'post' ) ); ?></span></p>
				</div>
			</div>
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<div class="form-group">
					<label for="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-log-share' ) ); ?>"><?php _e( 'Profit Share payout log entry template', 'bonipress' ); ?></label>
					<input type="text" name="<?php echo $this->field_name( array( 'post_type_setup' => $point_type ) ); ?>[log_sale]" id="<?php echo $this->field_id( array( 'post_type_setup' => $point_type . '-log-share' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'bonipress' ); ?>" value="<?php echo esc_attr( $type_setup['log_sale'] ); ?>" />
					<p><span class="description"><?php echo $this->core->available_template_tags( array( 'general', 'post' ) ); ?></span></p>
				</div>
			</div>
		</div>
...
?>
	...
	<h3><?php _e( 'Transactions', 'bonipress' ); ?></h3>
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="checkbox">
				<label for="<?php echo $this->field_id( 'reload' ); ?>"><input type="checkbox" name="<?php echo $this->field_name( 'reload' ); ?>" id="<?php echo $this->field_id( 'reload' ); ?>" <?php checked( $this->sell_content['reload'], 1 ); ?> value="1" /> <?php _e( 'Reload page after successful payments.', 'bonipress' ); ?></label>
			</div>
		</div>
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'working' ); ?>"><?php _e( 'Button Label', 'bonipress' ); ?></label>
				<input type="text" name="<?php echo $this->field_name( 'working' ); ?>" id="<?php echo $this->field_id( 'working' ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'bonipress' ); ?>" value="<?php echo esc_attr( $this->sell_content['working'] ); ?>" />
				<p><span class="description"><?php _e( 'Option to show a custom button label while the payment is being processed. HTML is allowed.', 'bonipress' ); ?></span></p>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<h3><?php _e( 'Purchase Template', 'bonipress' ); ?></h3>
			<p><span class="description"><?php _e( 'The content will be replaced with this template when viewed by a user that has not paid for the content but can afford to pay.', 'bonipress' ); ?></span></p>
<?php
...
?>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<h3><?php _e( 'Insufficient Funds Template', 'bonipress' ); ?></h3>
			<p><span class="description"><?php _e( 'The content will be replaced with this template when viewed by a user that has not paid for the content and can not afford to pay.', 'bonipress' ); ?></span></p>
<?php
...
?>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<p><span class="description"><?php _e( 'The content will be replaced with this template when viewed by someone who is not logged in on your website.', 'bonipress' ); ?></span></p>
<?php
...
(function($) {
	...
	$( '#boniPRESS-wrap .bonipress-check-count' ).click(function(){

		if ( $(this).is( ':checked' ) ) {

			selectedposttypes++;
			$( '#bonipress-sell-content-post-type-warning' ).hide();
			...
			if ( selectedposttypes <= 0 )
				$( '#bonipress-sell-content-post-type-warning' ).show();
			else
				$( '#bonipress-sell-content-post-type-warning' ).hide();
	...
	$( '#boniPRESS-wrap .bonipress-check-count' ).click(function(){
		...
		if ( $(this).is( ':checked' ) ) {
			...
			selectedpointtypes++;
			$( '#bonipress-sell-content-point-type-warning' ).hide();

			$( '#bonipress-sell-' + $(this).data( 'type' ) + '-wrap' ).show();

		}
		else {
			selectedpointtypes--;
			if ( selectedpointtypes <= 0 )
				$( '#bonipress-sell-content-point-type-warning' ).show();
			else
				$( '#bonipress-sell-content-point-type-warning' ).hide();

			$( '#bonipress-sell-' + $(this).data( 'type' ) + '-wrap' ).hide();
		}

	});

	$( '#bonipress-sell-this-post-type-filter' ).on( 'change', 'select.toggle-filter-menu', function(){
...
<?php
		...
		public function sanitize_extra_settings( $new_data, $data, $general ) {
			...
			if ( array_key_exists( 'post_type_setup', $settings ) ) {
				foreach ( $settings['post_type_setup'] as $point_type => $setup ) {
					...
					bonipress_update_option( 'bonipress_sell_this_' . $point_type, $new );
			...
			update_option( 'bonipress_sell_content_one_seven_updated', time() );
		...
		public function add_metabox( $post ) {
			...
			add_meta_box(
				'bonipress-sell-content-setup',
				apply_filters( 'bonipress_sell_this_label', __( 'Sell Content', 'bonipress' ), $this ),
				array( $this, 'metabox' ),
				$post->post_type,
				'side',
				'high'
			...
			add_filter( 'postbox_classes_' . $post->post_type . '_bonipress-sell-content-setup',  array( $this, 'metabox_classes' ) );
...
?>
<style type="text/css">
#bonipress-sell-content-setup .inside { padding: 0 !important; }
#bonipress-sell-content-setup .inside .row { margin-bottom: 0; }
#bonipress-sell-content-setup .inside .container-fluid { padding-left: 0; padding-right: 0; }
#bonipress-sell-content-setup .inside .row .col-lg-12 .form-group { padding: 12px 12px 10px 12px; background-color: white; border-bottom: 1px solid #ddd; }
#bonipress-sell-content-types .point-type-setup .cover { border-bottom: 1px solid #ddd; }
#bonipress-sell-content-types .point-type-setup .cover > .row { padding-top: 6px; padding-bottom: 12px; }
#bonipress-sell-content-types .point-type-setup:last-child .cover { border-bottom: none; }
</style>
<div id="bonipress-sell-content-types" class="container-fluid">
	<input type="hidden" name="bonipress-sell-this-setup-token" value="<?php echo wp_create_nonce( 'bonipress-sell-this-content' ); ?>" />
<?php
			...
			if ( ! empty( $settings['type'] ) ) {
				foreach ( $settings['type'] as $point_type ) {
					...
					$expiration_description = __( 'Never expires', 'bonipress' );
		...
		<div class="row row-narrow">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div class="form-group slim">
					<label for="bonipress-sell-this-<?php echo $point_type; ?>-status" class="slim"><input type="checkbox" name="bonipress_sell_this[<?php echo $point_type; ?>][status]" id="bonipress-sell-this-<?php echo $point_type; ?>-status"<?php if ( $sale_setup['status'] === 'enabled' ) echo ' checked="checked"'; ?> value="enabled" class="toggle-setup" data-type="<?php echo $point_type; ?>" /> <?php printf( __( 'Sell using %s', 'Point types name', 'bonipress' ), $bonipress->plural() ); ?></label>
				</div>
			</div>
		</div>
		<div class="cover">
			<div class="row row-narrow padded-row bonipress-sell-setup-container" id="bonipress-sell-content-<?php echo $point_type; ?>-wrap" style="display: <?php if ( $sale_setup['status'] === 'enabled' ) echo 'block'; else echo 'none'; ?>;">
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
					<div class="form-group slim">
						<label for="bonipress-sell-this-<?php echo $point_type; ?>-price"><?php _e( 'Price', 'bonipress' ); ?></label>
						<input type="text" name="bonipress_sell_this[<?php echo $point_type; ?>][price]" id="bonipress-sell-this-<?php echo $point_type; ?>-price" class="form-control" value="<?php echo esc_attr( $sale_setup['price'] ); ?>" />
					</div>
				</div>
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
					<div class="form-group slim">
						<label for="bonipress-sell-this-<?php echo $point_type; ?>-expire"><?php _e( 'Expiration', 'bonipress' ); ?></label>
						<input type="text" name="bonipress_sell_this[<?php echo $point_type; ?>][expire]" id="bonipress-sell-this-<?php echo $point_type; ?>-expire" class="form-control" value="<?php echo absint( $sale_setup['expire'] ); ?>" />
					</div>
				</div>
			</div>
		</div>
...
(function($) {
	...
	$( '#bonipress-sell-content-types .toggle-setup' ).click(function(){

		if ( $(this).is( ':checked' ) ) {
			$( '#bonipress-sell-content-types #bonipress-sell-content-' + $(this).data( 'type' ) + '-wrap' ).slideDown();
		}
		else {
			$( '#bonipress-sell-content-types #bonipress-sell-content-' + $(this).data( 'type' ) + '-wrap' ).slideUp();
		}
...
<?php
		...
		public function save_metabox( $post_id ) {
			...
			// Minimum requirement
			if ( ! isset( $_POST['bonipress_sell_this'] ) || ! is_array( $_POST['bonipress_sell_this'] ) || empty( $_POST['bonipress_sell_this'] ) ) return;

			// Verify nonce
			if ( isset( $_POST['bonipress-sell-this-setup-token'] ) && wp_verify_nonce( $_POST['bonipress-sell-this-setup-token'], 'bonipress-sell-this-content' ) ) {
				...
				if ( ! empty( $settings['type'] ) ) {
					foreach ( $settings['type'] as $point_type ) {
						...
						if ( ! array_key_exists( $point_type, $_POST['bonipress_sell_this'] ) ) continue;
						...
						$submission = shortcode_atts( array(
							...
							'price'  => 0,
							'expire' => 0
						), $_POST['bonipress_sell_this'][ $point_type ] );
						...
						if ( strlen( $submission['price'] ) > 0 )
							$new_setup['price'] = $bonipress->number( sanitize_text_field( $submission['price'] ) );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonipress_load_sell_content_addon' ) ) :
	function bonipress_load_sell_content_addon( $modules, $point_types ) {

		$modules['solo']['content'] = new boniPRESS_Sell_Content_Module();
...
add_filter( 'bonipress_load_modules', 'bonipress_load_sell_content_addon', 90, 2 );
