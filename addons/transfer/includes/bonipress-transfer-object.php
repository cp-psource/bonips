 ...
 * @version 1.0
 */
if ( ! class_exists( 'boniPRESS_Transfer' ) ) :
	class boniPRESS_Transfer extends boniPRESS_Object {
		...
		protected function populate() {
			...
			$this->settings = apply_filters( 'bonipress_get_transfer_settings', $settings, $this );
		...
		public function generate_new_transfer_id( $sender_id = 0, $recipient_id = 0 ) {
			...
			return apply_filters( 'bonipress_new_transfer_id', $transfer_id, $sender_id, $recipient_id, $this );
		...
		public function get_transfer( $transfer_id = false ) {
			...
			elseif ( $transfer_id !== false ) {
				...
				$log_entries  = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$bonipress_log_table} WHERE data LIKE %s ORDER BY creds ASC LIMIT 2;", $transfer_sql ) );
			...
			if ( ! empty( $log_entries ) && count( $log_entries ) == 2 ) {
				...
				$transfer->point_type      = new boniPRESS_Point_Type( $log_entries[0]->ctype );
			...
			return apply_filters( 'bonipress_get_transfer', $transfer, $transfer_id, $this );
		...
		public function new_instance( $request = array() ) {
			...
			$recipient_id             = bonipress_get_user_id( $recipient );
			...
			$usable_types             = bonipress_get_usable_types( $sender_id );
			...
			if ( ! empty( $usable_types ) ) {
				...
				$balances = ( bonipress_is_current_account( $this->sender_id ) ) ? $bonipress_current_account->balance : false;
				...
				foreach ( $requested_types as $type_id ) {
					...
					if ( ! bonipress_point_type_exists( $type_id ) || ! in_array( $type_id, $usable_types ) || ! in_array( $type_id, $this->settings['types'] ) ) continue;

					// The current users balances are already populated in the $bonipress_current_account object, if this is the case, we do not need to ask for them again.
					$balance                    = ( $balances !== false && $balances[ $type_id ] !== false ) ? $balances[ $type_id ] : new boniPRESS_Balance( $this->sender_id, $type_id );
					...
					$balance->lowest            = apply_filters( 'bonipress_transfer_acc_limit', $balance->point_type->number( 0 ), $type_id, $this->sender_id, $this->reference );
		...
		public function get_transfer_limit_desc( $point_type = BONIPRESS_DEFAULT_TYPE_KEY ) {
			...
			if ( ! empty( $this->limits ) && array_key_exists( $point_type, $this->limits ) ) {
				...
				//$description = sprintf( _x( 'Maximum %s / %s', 'Transfer limit / period', 'bonipress' ), $type_object->format( $limit->amount ), $limit->label );
		...
		public function is_valid_transfer_request( $request = array(), $posted = array() ) {
			...
			$this->request         = shortcode_atts( apply_filters( 'bonipress_new_transfer_request', array(
				'token'        => NULL,
				'recipient_id' => NULL,
				'user_id'      => 'current',
			...
			// Security
			if ( ! wp_verify_nonce( $this->request['token'], 'bonipress-new-transfer-' . $this->request['reference'] ) )
				return 'error_1';
			...
			if ( ! bonipress_point_type_exists( $this->request['ctype'] ) || ! in_array( $this->request['ctype'], $this->settings['types'] ) )
				return 'error_10';
			...
			// Transfer recipient
			$recipient_id          = bonipress_get_transfer_recipient( sanitize_text_field( $this->request['recipient_id'] ) );
			...
			if ( $bonipress->exclude_user( $this->sender_id ) )
				return 'error_4';

			// The recipient is excluded from the point type
			if ( $bonipress->exclude_user( $this->recipient_id ) )
				return 'error_4';

			// Amount can not be zero
			$amount                = $bonipress->number( abs( $this->request['amount'] ) );
			if ( $amount < $bonipress->get_lowest_value() )
				return 'error_5';

			$balance               = $bonipress->get_users_balance( $this->sender_id );
			$lowest_balance        = apply_filters( 'bonipress_transfer_acc_limit', $bonipress->zero(), $this->point_type, $this->sender_id, $this->reference );
			...
			if ( $transfer_limit !== false ) {
				...
				// Format the limit amount
				$transfer_limit->amount = $bonipress->number( $transfer_limit->amount );
			...
			$this->data            = apply_filters( 'bonipress_transfer_data', array(
				'ref_type' => 'user',
			...
			if ( $bonipress->has_entry( $this->reference, $this->recipient_id, $this->sender_id, $this->data, $this->point_type ) )
				return 'error_9';

			$this->balances[ $this->point_type ] = $balance;

			$this->valid_request = apply_filters( 'bonipress_is_valid_transfer_request', true, $this );
		...
		public function total_sent( $reference = 'transfer', $user_id = false, $point_type = '', $period_start = 0, $period_end = 0 ) {
			...
			return apply_filters( 'bonipress_get_total_transfered', $total, $reference, $user_id, $point_type, $period_start, $period_end );
		...
		public function new_transfer() {
			...
			$result                = false;
			$bonipress                = bonipress( $this->point_type );
			$this->transfer_charge = apply_filters( 'bonipress_transfer_charge', $this->transfer_amount, $this );
			...
			do_action( 'bonipress_transfer_ready', $this->transfer_id, $request, $this->settings );
			...
			if ( $bonipress->add_creds(
				...
				if ( ! $bonipress->has_entry( $this->reference, $this->sender_id, $this->recipient_id, $this->data, $this->point_type ) ) {

					$this->transfer_payout = apply_filters( 'bonipress_transfer_payout', $this->transfer_amount, $this );
					...
					do_action( 'bonipress_transfer_completed', $this->transfer_id, $request, $this->settings, $this );

					// Return the good news
					$result        = array(
						'amount'      => $this->transfer_charge,
						'css'         => '.bonipress-balance-' . $this->point_type,
						'balance'     => $bonipress->format_creds( $attempt_check ),
						'zero'        => ( ( $attempt_check <= $bonipress->zero() ) ? true : false )
					);
			...
			return apply_filters( 'bonipress_new_transfer', $result, $this->request, $attempt_check, $this );
...
?>
<div class="bonipress-transfer-cred-wrapper"<?php if ( $this->reference != '' ) echo ' id="transfer-form-' . esc_attr( $this->reference ) . '"'; ?>>
	<form class="form bonipress-transfer bonipress-transfer-form" id="bonipress-transfer-form-<?php echo esc_attr( $this->reference ); ?>" method="post" data-ref="<?php echo esc_attr( $this->reference ); ?>" action="">

		<?php do_action( 'bonipress_transfer_form_start', $this->args, $this->settings ); ?>
		...
		<div class="row">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<input type="hidden" name="bonipress_new_transfer[token]" value="<?php echo wp_create_nonce( 'bonipress-new-transfer-' . $this->reference ); ?>" />
				<input type="hidden" name="bonipress_new_transfer[reference]" value="<?php echo esc_attr( $this->reference ); ?>" />
				<input type="submit" class="bonipress-submit-transfer<?php echo ' ' . esc_attr( $this->args['button_class'] ); ?>" value="<?php echo esc_attr( $this->args['button'] ); ?>" />
			</div>

		</div>

		<?php do_action( 'bonipress_transfer_form_end', $this->args, $this->settings ); ?>
...
<?php
			...
			return do_shortcode( apply_filters( 'bonipress_transfer_render', $output, $this->args, $this ) );
		...
		public function get_transfer_recipient_field( $return = false ) {
			...
			if ( count( $recipients ) < 1 ) {
				$field .= '<input type="text" name="bonipress_new_transfer[recipient_id]" value="" aria-required="true" class="bonipress-autofill form-control" data-form="' . esc_attr( $this->reference ) . '" placeholder="' . $placeholder . '" />';
			}
			...
			elseif ( count( $recipients ) == 1 ) {
				...
				$field     .= '<p class="form-control-static">' . $value . '</p><input type="hidden" name="bonipress_new_transfer[recipient_id]" value="' . esc_attr( $fist_user->ID ) . '" />';
			...
			elseif ( count( $recipients ) > 1 ) {
				...
				$field .= '<select name="bonipress_new_transfer[recipient_id]" class="form-control">';
			...
			$field  = apply_filters( 'bonipress_transfer_to_field', $field, $this->settings, $this->args );
		...
		public function get_transfer_amount_field( $return = false ) {
			...
			if ( ! is_array( $this->transfer_amount ) && $this->transfer_amount == 0 )
				$field .= '<input type="text" name="bonipress_new_transfer[amount]" placeholder="' . esc_attr( $balance->minimum ) . '" class="form-control" value="' . esc_attr( $balance->minimum ) . '" />';

			// Multiple amounts to pick from
			elseif ( is_array( $this->transfer_amount ) && count( $this->transfer_amount ) > 1 ) {

				$field .= '<select name="bonipress_new_transfer[amount]" class="form-control">';
			...
			// An amount has been nominated
				...
				$field .= '<input type="hidden" name="bonipress_new_transfer[amount]" value="' . esc_attr( $this->transfer_amount ) . '" />';
			...
			$field  = apply_filters( 'bonipress_transfer_form_amount', $field, $this->args, $this->settings );
		...
		public function get_transfer_point_type_field( $return = false ) {
			...
			$field = '<input type="hidden" name="bonipress_new_transfer[ctype]" value="' . esc_attr( $this->transferable_types[0] ) . '" />';
			...
			if ( count( $this->transferable_types ) > 1 ) {
				...
				$field .= '<select name="bonipress_new_transfer[ctype]" class="form-control">';
		...
		public function get_transfer_message_field( $return = false ) {
			...
			$field = apply_filters( 'bonipress_transfer_form_message', $field, $this->args, $this->settings );
		...
		public function get_transfer_extra_fields( $return = false ) {
			...
			$field = apply_filters( 'bonipress_transfer_form_extra', $field, $this->args, $this->settings );
		...
		public function get_error_message( $return = false ) {
			...
			$content = apply_filters( 'bonipress_transfer_form_errors', $content, $this );
