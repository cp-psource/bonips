...
/**
 * Shortcode: bonipress_my_badges
 * Allows you to show the current users earned badges.
 * @since 1.5
 * @version 1.2.1
 */
if ( ! function_exists( 'bonipress_render_my_badges' ) ) :
	function bonipress_render_my_badges( $atts, $content = '' ) {
		...
		echo '<div class="row" id="bonipress-users-badges"><div class="col-xs-12">';

		// Show only badges that we have earned
		if ( $show == 'earned' ) {

			bonipress_display_users_badges( $user_id, $width, $height );
		...
		return apply_filters( 'bonipress_my_badges', $output, $user_id );

	}
endif;

/**
 * Shortcode: bonipress_badges
 * Allows you to show all published badges
 * @since 1.5
 * @version 1.1.1
 */
if ( ! function_exists( 'bonipress_render_badges' ) ) :
	function bonipress_render_badges( $atts, $template = '' ) {
		...
		$output = '<div id="bonipress-all-badges">';

		if ( ! empty( $all_badges ) ) {

			foreach ( $all_badges as $badge_id ) {

				$badge               = bonipress_get_badge( $badge_id, 0 );
				$badge->image_width  = $width;
				$badge->image_height = $height;

				$row = $template;
				$row = str_replace( '%badge_title%',   $badge->title,                                  $row );
				$row = str_replace( '%requirements%',  bonipress_display_badge_requirements( $badge_id ), $row );
				$row = str_replace( '%count%',         $badge->earnedby,                               $row );
				$row = str_replace( '%default_image%', $badge->main_image,                             $row );
				
				if( bonipress_user_has_badge( get_current_user_id(), $badge_id) ) {
				...
				$output .= apply_filters( 'bonipress_badges_badge', $row, $badge );
		...
		return apply_filters( 'bonipress_badges', $output );
