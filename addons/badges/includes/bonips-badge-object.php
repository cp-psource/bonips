...
/**
 * boniPS_Badge class
 * @see http://codex.bonips.me/classes/bonips_badge/
 * @since 1.7
 * @version 1.1
 */
if ( ! class_exists( 'boniPS_Badge' ) ) :
	class boniPS_Badge extends boniPS_Object {
		...
		function __construct( $object = NULL, $level_id = NULL ) {
			...
			elseif ( is_numeric( $object ) && bonips_get_post_type( $object ) == BONIPS_BADGE_KEY )
		...
		protected function populate( $object = NULL, $level_id = NULL ) {
			...
			$this->title        = ( isset( $object->post_title ) ) ? $object->post_title : bonips_get_the_title( $this->post_id );
			$this->earnedby     = ( isset( $object->earnedby ) ) ? $object->earnedby : $this->get_user_count( $level_id );
			$this->levels       = ( isset( $object->levels ) ) ? $object->levels : bonips_get_badge_levels( $this->post_id );
			...
			if ( absint( bonips_get_post_meta( $this->post_id, 'manual_badge', true ) ) === 1 )
				$this->manual = true;
		...
		public function get_user_count( $level_id = NULL ) {
			...
			$count = bonips_get_post_meta( $this->post_id, 'total-users-with-badge', true );
			if ( $count == '' || $level_id !== NULL ) {
				...
				$count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( DISTINCT user_id ) FROM {$wpdb->usermeta} WHERE meta_key = %s {$level_filter};", bonips_get_meta_key( $this->user_meta_key ) ) );
				if ( $count === NULL ) $count = 0;

				if ( $level_id === NULL )
					bonips_update_post_meta( $this->post_id, 'total-users-with-badge', $count, true );

			}

			return apply_filters( 'bonips_count_users_with_badge', absint( $count ), $this->post_id );
		...
		public function user_has_badge( $user_id = false, $level_id = 0 ) {
			...
			return apply_filters( 'bonips_user_has_badge', $has_badge, $user_id, $this->post_id, $level_id, $this->current_level );
		...
		public function get_users_current_level( $user_id = false ) {
			...
			if ( bonips_is_current_account( $user_id ) && isset( $bonips_current_account->badge_ids ) && ! empty( $bonips_current_account->badge_ids ) && array_key_exists( $this->post_id, $bonips_current_account->badge_ids ) )
				return absint( $bonips_current_account->badge_ids[ $this->post_id ] );

			$current_level = bonips_get_user_meta( $user_id, $this->user_meta_key, '', true );
		...
		public function query_users_level( $user_id = false ) {
			...
			if ( ! empty( $base_requirements ) ) {
				foreach ( $base_requirements as $requirement_id => $requirement ) {
					...
					$query  = $wpdb->get_var( $wpdb->prepare( "SELECT {$having} FROM {$bonips_log_table} WHERE ctype = %s AND ref = %s AND user_id = %d;", $requirement['type'], $requirement['reference'], $user_id ) );
					
					$query  = apply_filters( 'bonips_badge_requirement', $query, $requirement_id, $requirement, $having, $user_id );
			...
			do_action( 'bonips_badge_level_reached', $user_id, $this->post_id, $level_reached );
		...
		public function assign( $user_id = false, $level_id = 0 ) {
			...
			$execute = apply_filters( 'bonips_badge_assign', true, $user_id, $new_level, $this );

			// Need to update counter with new assignments
			if ( $execute ) {

				$new_level = apply_filters( 'bonips_badge_user_value', $new_level, $user_id, $this->post_id );
				bonips_update_user_meta( $user_id, $this->user_meta_key, '', $new_level );
				...
				if ( $new_level == 0 ) {
					...
					bonips_update_post_meta( $this->post_id, 'total-users-with-badge', $this->earnedby );

				}

				$this->payout_reward( $previous_level, $new_level );

				do_action( 'bonips_after_badge_assign', $user_id, $this->post_id, $new_level );
		...
		public function payout_reward( $previous_level = -1, $new_level = 0 ) {
			...
			if ( $previous_level < 0 && $new_level == 0 ) {
				...
				if ( ! $bonips->has_entry( 'badge_reward', $this->post_id, $this->user_id, 0, $reward['type'] ) ) {

					$exec = apply_filters( 'customize_bonips_badge_condition', true, $this->post_id, $this->user_id, $reward['type']);
					...
					if( $exec ) {
						...
						do_action( 'bonips_badge_rewardes', $this->user_id, $previous_level, $new_level, $reward, $this );
			...
			// Earning a new level as well
				...
				for ( $i = $previous_level; $i <= $new_level; $i++ ) {
					...
					// Make sure we only get points once for each level we reach for each badge
					if ( ! $bonips->has_entry( 'badge_reward', $this->post_id, $this->user_id, $i, $reward['type'] ) ) {
		...
		public function assign_all() {
			...
			if ( ! empty( $this->levels[0]['requires'] ) ) {
				...
				if ( count( $this->levels[0]['requires'] ) == 1 ) {
					...
					$results             = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT user_id, {$having} as total FROM {$bonips_log_table} WHERE ctype = %s AND ref = %s GROUP BY user_id HAVING {$having} >= %f;", $requirement['type'], $requirement['reference'], $requirement['amount'] ) );
					...
					foreach ( $this->levels as $level_id => $level_setup ) {
						...
						if ( ! empty( $level_setup['requires'] ) ) {
							foreach ( $level_setup['requires'] as $requirement_id => $requirement ) {
								...
								$level_user_ids[ $requirement_id ] = $wpdb->get_col( $wpdb->prepare( "SELECT DISTINCT user_id FROM {$bonips_log_table} WHERE ctype = %s AND ref = %s GROUP BY user_id HAVING {$having} >= %f;", $requirement['type'], $requirement['reference'], $requirement['amount'] ) );
				...
				if ( ! empty( $results ) ) {
					...
					// Assign each user
					foreach ( $results as $row )
						bonips_update_user_meta( $row->user_id, $this->user_meta_key, '', $row->level_id );
		...
		public function divest( $user_id = false ) {
			...
			bonips_delete_user_meta( $user_id, $this->user_meta_key );
		...
		public function divest_all() {
			...
			$count = $wpdb->delete(
				$wpdb->usermeta,
				array( 'meta_key' => bonips_get_meta_key( $this->user_meta_key ) ),
				array( '%s' )
			);
		...
		public function delete( $delete_post = false ) {
			...
			if ( ! empty( $this->point_types ) ) {
				...
				foreach ( $this->point_types as $point_type )
					bonips_delete_option( 'bonips-badge-references-' . $point_type );

			}

			if ( $delete_post )
				bonips_delete_post( $this->post_id, true );
		...
		public function get_image( $image = NULL ) {
			...
			$level = 'none';
			if ( $image === 'main' )
				$image_identification = bonips_get_post_meta( $this->post_id, 'main_image', true );
			...
			if ( is_numeric( $image_identification ) &&  strpos( '://', (string) $image_identification ) === false )
				$image_url = bonips_get_attachment_url( $image_identification );
			...
			return apply_filters( 'bonips_badge_image', $html, $image, $this );
