 ...
 * @version 1.6
 */
if ( ! function_exists( 'bonips_have_ranks' ) ) :
	function bonips_have_ranks( $point_type = '' ) {
		...
		return apply_filters( 'bonips_have_ranks', $have_ranks, $point_type, $count );
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_get_published_ranks_count' ) ) :
	function bonips_get_published_ranks_count( $point_type = NULL ) {
		...
		if ( $point_type === NULL ) {
			...
			if ( $count === false ) {
				...
				if ( $point_type !== NULL && bonips_point_type_exists( sanitize_key( $point_type ) ) ) {
		...
		return apply_filters( 'bonips_get_published_ranks_count', $count, $point_type );
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_get_rank_object_id' ) ) :
	function bonips_get_rank_object_id( $identifier = NULL ) {

		if ( $identifier === NULL ) return false;

		$rank_id = false;

		if ( absint( $identifier ) !== 0 && bonips_get_post_type( absint( $identifier ) ) == BONIPS_RANK_KEY )
			$rank_id = absint( $identifier );

		else {

			$rank = bonips_get_page_by_title( $identifier, OBJECT, BONIPS_RANK_KEY );
 ...
 * @version 1.3
 */
if ( ! function_exists( 'bonips_get_rank' ) ) :
	function bonips_get_rank( $rank_identifier = NULL ) {

		global $bonips_rank;

		$rank_id     = bonips_get_rank_object_id( $rank_identifier );
		if ( $rank_id === false ) return false;

		if ( isset( $bonips_rank )
			&& ( $bonips_rank instanceof boniPS_Rank )
			&& ( $rank_id === $bonips_rank->post_id )
		) {
			return $bonips_rank;
		}

		$bonips_rank = new boniPS_Rank( $rank_id );

		do_action( 'bonips_get_rank' );
 ...
 * @version 1.5
 */
if ( ! function_exists( 'bonips_rank_has_logo' ) ) :
	function bonips_rank_has_logo( $rank_identifier = NULL ) {
		...
		return apply_filters( 'bonips_rank_has_logo', $return, $rank_id );
 ...
 * @version 1.4
 */
if ( ! function_exists( 'bonips_get_rank_logo' ) ) :
	function bonips_get_rank_logo( $rank_identifier = NULL, $size = 'post-thumbnail', $attr = NULL ) {
		...
		return apply_filters( 'bonips_get_rank_logo', $logo, $rank_id, $size, $attr );
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_count_users_with_rank' ) ) :
	function bonips_count_users_with_rank( $rank_identifier = NULL ) {

		$rank_id    = bonips_get_rank_object_id( $rank_identifier );
		if ( $rank_id === false ) return 0;

		$user_count = bonips_get_post_meta( $rank_id, 'bonips_rank_users', true );

		if ( $user_count == '' ) {

			$point_type = bonips_get_post_meta( $rank_id, 'ctype', true );
			if ( $point_type == '' ) return 0;

			global $wpdb;

			$user_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( user_id ) FROM {$wpdb->usermeta} WHERE meta_key = %s AND meta_value = %d;", bonips_get_meta_key( BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ) ), $rank_id ) );

			if ( $user_count === NULL ) $user_count = 0;

			bonips_update_post_meta( $rank_id, 'bonips_rank_users', $user_count );
 ...
 * @version 1.0.1
 */
if ( ! function_exists( 'bonips_get_users_rank_id' ) ) :
	function bonips_get_users_rank_id( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		$rank_id        = bonips_get_user_meta( $user_id, BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ), true );

		if ( $rank_id == '' ) {

			$rank = bonips_find_users_rank( $user_id, $point_type );

			// Found a rank, save it
			if ( $rank !== false ) {
				bonips_save_users_rank( $user_id, $rank->rank_id, $point_type );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_save_users_rank' ) ) :
	function bonips_save_users_rank( $user_id = NULL, $rank_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		bonips_update_user_meta( $user_id, BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ), $rank_id );

		if ( isset( $bonips_current_account->ranks ) && $bonips_current_account->balance[ $point_type ] !== false )
			$bonips_current_account->balance[ $point_type ]->rank = new boniPS_Rank( $rank_id );
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_get_my_rank' ) ) :
	function bonips_get_my_rank( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		if ( ! is_user_logged_in() ) return;

		$account = bonips_get_current_account();
		if ( $account !== false )
			return $account->balance[ $point_type ]->rank;

		return bonips_get_users_rank( get_current_user_id(), $point_type );
...
if ( ! function_exists( 'bonips_get_users_rank' ) ) :
	function bonips_get_users_rank( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		// Get Rank object
		if ( $rank_id != '' )
			$rank = bonips_get_rank( $rank_id );

		return apply_filters( 'bonips_get_users_rank', $rank, $user_id, $rank_id, $point_type );
 ...
 * @uses bonips_user_got_demoted if user got demoted to a lower rank.
 * @uses bonips_user_got_promoted if user got promoted to a higher rank.
 * @since 1.1
 * @version 1.7
 */
if ( ! function_exists( 'bonips_find_users_rank' ) ) :
	function bonips_find_users_rank( $user_id = NULL, $point_type = BONIPS_DEFAULT_TYPE_KEY, $act = true ) {

		if ( bonips_manual_ranks() ) return false;
		...
		if ( isset( $account_object->ranks ) ) {
			...
			$current_rank_id = bonips_get_user_meta( $user_id, BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ), true );
			$current_rank    = ( $current_rank_id != '' ) ? bonips_get_rank( $current_rank_id ) : false;
		}

		$balance        = $balance_object->current;
		if ( bonips_rank_based_on_total( $point_type ) )
		...
		$results        = $wpdb->get_row( $wpdb->prepare( "
			...
			FROM {$posts} ranks 
				...
				INNER JOIN {$postmeta} min ON ( ranks.ID = min.post_id AND min.meta_key = 'bonips_rank_min' )
				INNER JOIN {$postmeta} max ON ( ranks.ID = max.post_id AND max.meta_key = 'bonips_rank_max' ) 
		...
		if ( $act === true && isset( $results->rank_id ) ) {
			...
			if ( $results->current_id !== false && $current_rank !== false && $current_rank->maximum > $results->maximum )
				do_action( 'bonips_user_got_demoted', $user_id, $results, $current_rank, $point_type );

			// Promotions
			else {

				do_action( 'bonips_user_got_promoted', $user_id, $results->rank_id, $results, $point_type );

			}

			// Reset counters
			if ( $current_rank !== false )
				bonips_delete_post_meta( $current_rank->post_id, 'bonips_rank_users' );

			bonips_delete_post_meta( $results->rank_id, 'bonips_rank_users' );
		...
		return apply_filters( 'bonips_find_users_rank', $results, $user_id, $point_type );
 ...
 * @version 1.7
 */
if ( ! function_exists( 'bonips_assign_ranks' ) ) :
	function bonips_assign_ranks( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		if ( bonips_manual_ranks() ) return 0;

		global $wpdb;

		$type_object    = new boniPS_Point_Type( $point_type );
		$rank_key       = bonips_get_meta_key( BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ) );

		$balance_key    = bonips_get_meta_key( $point_type );
		$bonips      = bonips( $point_type );
		if ( isset( $bonips->rank['base'] ) && $bonips->rank['base'] == 'total' )
			$balance_key = bonips_get_meta_key( $point_type, '_total' );

		$ranks          = bonips_get_ranks( 'publish', '-1', 'ASC', $point_type );
		$balance_format = ( isset( $type_object->sql_format ) ) ? $type_object->sql_format : '%d';

		do_action( 'bonips_assign_ranks_start' );
		...
		if ( ! empty( $ranks ) ) {
			foreach ( $ranks as $rank ) {
				...
				bonips_delete_post_meta( $rank->post_id, 'bonips_rank_users' );

			}
		}

		do_action( 'bonips_assign_ranks_end' );
 ...
 * @version 1.6
 */
if ( ! function_exists( 'bonips_get_ranks' ) ) :
	function bonips_get_ranks( $status = 'publish', $number = '-1', $order = 'DESC', $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
		...
		if ( $ranks === false ) {
			...
			$rank_ids  = $wpdb->get_col( $wpdb->prepare( "
				...
				FROM {$posts} ranks
					...
					LEFT JOIN {$postmeta} min ON ( ranks.ID = min.post_id AND min.meta_key = 'bonips_rank_min' ) 
				WHERE ranks.post_type = %s 
			...
			if ( ! empty( $rank_ids ) ) {
				...
				foreach ( $rank_ids as $rank_id )
					$results[] = bonips_get_rank( $rank_id );
		...
		return apply_filters( 'bonips_get_ranks', $results, $status, $number, $order );
 ...
 * @version 1.6
 */
if ( ! function_exists( 'bonips_get_users_of_rank' ) ) :
	function bonips_get_users_of_rank( $rank_identifier = NULL, $number = 25, $order = 'DESC', $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$rank_id       = bonips_get_rank_object_id( $rank_identifier );
		if ( $rank_id === false || ! bonips_point_type_exists( $point_type ) ) return false;
		...
		if ( $users === false ) {
			...
			$rank_meta_key = bonips_get_meta_key( BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ) );
			$balance_key   = bonips_get_meta_key( $point_type );
		...
		return apply_filters( 'bonips_get_users_of_rank', $users, $rank_id, $number, $order, $point_type );
 ...
 * @version 1.0
 */
if ( ! function_exists( 'bonips_manual_ranks' ) ) :
	function bonips_manual_ranks( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {

		$prefs  = bonips_get_addon_settings( 'rank', $point_type );
 ...
 * @version 1.1
 */
if ( ! function_exists( 'bonips_rank_based_on_total' ) ) :
	function bonips_rank_based_on_total( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_show_rank_in_buddypress' ) ) :
	function bonips_show_rank_in_buddypress( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
 ...
 * @version 1.2
 */
if ( ! function_exists( 'bonips_show_rank_in_psforum' ) ) :
	function bonips_show_rank_in_psforum( $point_type = BONIPS_DEFAULT_TYPE_KEY ) {
