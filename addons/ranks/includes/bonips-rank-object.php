...
/**
 * boniPS_Rank class
 * @see http://codex.bonips.me/classes/bonips_rank/
 * @since 1.7
 * @version 1.0
 */
if ( ! class_exists( 'boniPS_Rank' ) ) :
	class boniPS_Rank extends boniPS_Object {
		...
		public function __construct( $rank_id = NULL ) {
			...
			if ( bonips_get_post_type( $rank_id ) != BONIPS_RANK_KEY ) return;
		...
		protected function populate( $rank_id = NULL ) {
			...
			$this->title      = bonips_get_the_title( $this->post );
			$this->minimum    = bonips_get_post_meta( $this->post_id, 'bonips_rank_min', true );
			$this->maximum    = bonips_get_post_meta( $this->post_id, 'bonips_rank_max', true );
			$this->count      = bonips_count_users_with_rank( $this->post_id );

			$this->has_logo   = bonips_rank_has_logo( $this->post_id );
			$this->logo_id    = get_post_thumbnail_id( $this->post );
			$this->logo_url   = wp_get_attachment_url( $this->logo_id );

			$point_type       = bonips_get_post_meta( $this->post_id, 'ctype', true );
			if ( ! bonips_point_type_exists( $point_type ) )
				$point_type = BONIPS_DEFAULT_TYPE_KEY;

			$this->point_type = new boniPS_Point_Type( $point_type );

			$this->is_manual  = bonips_manual_ranks( $point_type );
		...
		public function user_has_rank( $user_id = false ) {
			...
			$users_rank = bonips_get_user_meta( $user_id, BONIPS_RANK_KEY, ( ( $post_type != BONIPS_DEFAULT_TYPE_KEY ) ? $post_type : '' ), true );

			$has_rank   = false;
			if ( $users_rank != '' && absint( $users_rank ) === $this->post_id )
				$has_rank = true;

			return apply_filters( 'bonips_user_has_rank', $has_rank, $user_id, $this );
		...
		public function assign( $user_id = false ) {
			...
			$value      = apply_filters( 'bonips_rank_user_value', $this->post_id, $user_id, $this );

			return bonips_update_user_meta( $user_id, BONIPS_RANK_KEY, ( ( $point_type != BONIPS_DEFAULT_TYPE_KEY ) ? $point_type : '' ), $value );
		...
		public function assign_all() {
			...
			do_action( 'bonips_assign_rank_start', $this );
			...
			do_action( 'bonips_assign_rank_end', $this );

			$this->count    = ( $count === NULL ) ? 0 : absint( $count );

			bonips_update_post_meta( $this->post_id, 'bonips_rank_users', $this->count );
		...
		public function divest( $user_id = false ) {
			...
			if ( $users_rank != '' ) {
				...
				$results = bonips_delete_user_meta( $user_id, BONIPS_RANK_KEY, ( ( $post_type != BONIPS_DEFAULT_TYPE_KEY ) ? $post_type : '' ) );
		...
		public function divest_all() {
			...
			$wpdb->delete(
				$wpdb->usermeta,
				array( 'meta_key' => bonips_get_meta_key( BONIPS_RANK_KEY, ( ( $post_type != BONIPS_DEFAULT_TYPE_KEY ) ? $post_type : '' ) ), 'meta_value' => $this->post_id ),
				array( '%s', '%d' )
			);

			$this->count = 0;

			bonips_update_post_meta( $this->post_id, 'bonips_rank_users', 0 );
