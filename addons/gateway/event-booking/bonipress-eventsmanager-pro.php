 ...
 * @version 1.1
 */
if ( ! class_exists( 'EM_Gateway_boniPRESS' ) ) {
	class EM_Gateway_boniPRESS extends EM_Gateway {

		public $gateway = 'bonipress';
		public $title = '';
		public $status = 1;
		public $status_txt = '';
		public $bonipress_type = 'bonipress_default';
		...
		function __construct() {
			...
			// Settings
			$settings = get_option( 'bonipress_eventsmanager_gateway_prefs' );
...
<?php
		...
		function booking_form_button() {
			...
			if ( $this->core->get_users_cred( $user_id, $this->bonipress_type ) <= $this->core->format_number( 0 ) ) return;
...
<?php		}
			...
			if ( $result ) {
				...
				if ( $EM_Booking->get_price() > 0 ) {
					...
					if ( $this->core->exclude_user( $EM_Booking->person->ID ) ) {
						$EM_Booking->add_error( __( 'You can not pay using this gateway.', 'bonipress' ) );
						$ok = false;
					}
		...
		public function can_pay( $EM_Booking ) {
			...
			$balance = $this->core->get_users_cred( $EM_Booking->person->ID, $this->bonipress_type );
		...
		function mysettings() {
			...
			do_action( 'bonipress_em_before_settings', $this ); ?>
...
<table class="form-table">
	...
	<tr>
		...
		<td>
			<input name="bonipress_gateway[refund]" type="text" id="bonipress-gateway-log-refund" value="<?php echo $this->prefs['refund']; ?>" size="5" /> %<br />
			<span class="description"><?php _e( 'The percentage of the paid amount to refund if a user cancels their booking. Use zero for no refunds. No refunds are given to "Rejected" bookings!', 'bonipress' ); ?></span>
		</td>
	</tr>
	<tr>
		<th scope="row"><?php _e( 'Profit Sharing', 'bonipress' ); ?></th>
		<td>
			<input name="bonipress_gateway[share]" type="text" id="bonipress-gateway-profit-sharing" value="<?php echo $this->prefs['share']; ?>" size="5" /> %<br />
...
<table class="form-table" id="bonipress-exchange-rate" style="<?php echo $box; ?>">
	<tr>
		...
		<td>
			<input name="bonipress_gateway[rate]" type="text" id="bonipress-gateway-rate" size="6" value="<?php echo $this->prefs['rate']; ?>" /><br />
...
<table class="form-table">
	<tr>
		...
		<td>
			<input name="bonipress_gateway[log][purchase]" type="text" id="bonipress-gateway-log-purchase" style="width: 95%;" value="<?php echo $this->prefs['log']['purchase']; ?>" size="45" /><br />
	...
	<tr>
		<th scope="row"><?php _e( 'Refunds', 'bonipress' ); ?></th>
		<td>
			<input name="bonipress_gateway[log][refund]" type="text" id="bonipress-gateway-log-refund" style="width: 95%;" value="<?php echo $this->prefs['log']['refund']; ?>" size="45" /><br />
			<span class="description"><?php echo $this->core->available_template_tags( array( 'general', 'post' ) ); ?></span>
		</td>
	</tr>
</table>
...
<table class="form-table">
	<tr valign="top">
		...
		<td>
			<input name="bonipress_gateway[labels][link]" type="text" id="bonipress-gateway-labels-link" style="width: 95%" value="<?php echo $this->prefs['labels']['link']; ?>" size="45" /><br />
	...
	<tr valign="top">
		...
		<td>
			<input name="bonipress_gateway[labels][header]" type="text" id="bonipress-gateway-labels-header" style="width: 95%" value="<?php echo $this->prefs['labels']['header']; ?>" size="45" /><br />
	...
	<tr valign="top">
		...
		<td>
			<input name="bonipress_gateway[labels][button]" type="text" id="bonipress-gateway-labels-button" style="width: 95%" value="<?php echo $this->prefs['labels']['button']; ?>" size="45" /><br />
...
<table class='form-table'>
	<tr valign="top">
		...
		<td>
			<input type="text" name="bonipress_gateway[messages][success]" id="bonipress-gateway-messages-success" style="width: 95%;" value="<?php echo stripslashes( $this->prefs['messages']['success'] ); ?>" /><br />
	...
	<tr valign="top">
		...
		<td>
			<input type="text" name="bonipress_gateway[messages][error]" id="bonipress-gateway-messages-error" style="width: 95%;" value="<?php echo stripslashes( $this->prefs['messages']['error'] ); ?>" /><br />
			<span class="description"><?php _e( 'No HTML allowed!', 'bonipress' ); ?><br /><?php echo $this->core->available_template_tags( array( 'general' ) ); ?></span>
		</td>
	</tr>
</table>
