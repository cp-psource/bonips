 ...
 * @version 1.0
 */
if ( ! class_exists( 'boniPRESS_Stats_Widget_Circulation' ) ) :
	class boniPRESS_Stats_Widget_Circulation extends boniPRESS_Statistics_Widget {
		...
		function __construct( $widget_id = '', $args = array() ) {
			...
			$this->dates = bonipress_get_stat_dates( 'today_this' );
		}
		...
		function get_data() {
			...
			foreach ( $point_types as $type_id => $label ) {
				...
				$totals[] = '<strong>' . sprintf( __( 'Total %s:', 'bonipress' ), $this->ctypes[ $type_id ] ) . '</strong> <span style="color:' . $this->colors[ $type_id ]['positive'] . '">' . $bonipress->format_creds( $total ) . '</span>';
				$all = $all + $total;
		...
		function front_widget() {
			...
			if ( $this->args['ctypes'] == 'all' )
				$label = __( 'Total amount in circulation', 'bonipress' );
			else
				$label = sprintf( __( 'Total amount of %s in circulation', 'bonipress' ), $this->ctypes[ $this->args['ctypes'] ] );

			$circulation = $this->get_data();
			$gains_loses = $this->gains_and_loses();

?>
<div id="bonipress-stats-overview" class="row">
	<div id="bonipress-stats-<?php echo $this->id; ?>" class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
...
<?php
				...
				$bonipress = bonipress( $this->args['ctypes'] );
				...
				$circulation['series'][] = "{ value: {$gains_p}, color: '" . $color . "', highlight: '" . $color . "', label: '" . esc_attr__( 'Total gains (%)', 'bonipress' ) . "' }";

				$color = $lose_color = $this->colors[ $this->args['ctypes'] ]['positive'];
				$circulation['series'][] = "{ value: {$gains_l}, color: '" . $color . "', highlight: '" . $color . "', label: '" . esc_attr__( 'Total loses (%)', 'bonipress' ) . "' }";

// 
?>
		<ul id="total-per-point-type"><li><?php printf( __( 'Total Gained: %s', 'bonipress' ), '<span style="color:' . $gain_color . '">' . $bonipress->format_creds( $gains ) . '</span>' ); ?></li><li><?php printf( __( 'Total Spent: %s', 'bonipress' ), '<span style="color:' . $lose_color . '">' . $bonipress->format_creds( $loses ) . '</span>' ); ?></li></ul>
<?php

			endif;

?>
		<div class="table-responsive bonipress-stats-table">
			<table class="table table-condensed" cellpadding="0" cellspacing="0">
				<thead>
					<tr>
						<th class="rowtitle"></th>
						<th class="doublecell" colspan="2"><?php _e( 'Today', 'bonipress' ); ?></th>
						<th class="doublecell" colspan="2"><?php _e( 'This Week', 'bonipress' ); ?></th>
						<th class="doublecell" colspan="2"><?php _e( 'This Month', 'bonipress' ); ?></th>
						<th class="doublecell" colspan="2"><?php _e( 'This Year', 'bonipress' ); ?></th>
					</tr>
					<tr class="subheader">
						<th class="rowtitle"></th>
						<th class="cell"><?php _e( 'Gained', 'bonipress' ); ?></th>
						<th class="cell"><?php _e( 'Lost', 'bonipress' ); ?></th>
						<th class="cell"><?php _e( 'Gained', 'bonipress' ); ?></th>
						<th class="cell"><?php _e( 'Lost', 'bonipress' ); ?></th>
						<th class="cell"><?php _e( 'Gained', 'bonipress' ); ?></th>
						<th class="cell"><?php _e( 'Lost', 'bonipress' ); ?></th>
						<th class="cell"><?php _e( 'Gained', 'bonipress' ); ?></th>
						<th class="cell last"><?php _e( 'Lost', 'bonipress' ); ?></th>
					</tr>
				</thead>
				<tbody>
<?php
...
<?php
				...
				foreach ( $dates as $key => $item ) {
					echo '<td class="cell">' . $bonipress->format_number( $item['gains'] ) . '</td>';
					echo '<td class="cell">' . $bonipress->format_number( $item['loses'] ) . '</td>';
				}
...
?>
<div id="bonipress-stats-overview" class="clear clearfix">
	<div id="bonipress-stats-<?php echo $this->id; ?>" class="left-column">
		<canvas id="total-circulation-<?php echo $this->id; ?>-chart"></canvas>
	</div>
	<h1><?php echo $label; ?>: <?php echo $this->format_number( $circulation['total'] ); ?></h1>
<?php
...
<?php
				...
				foreach ( $dates as $key => $item ) {
					...
					echo '<td class="cell"><a href="' . esc_url( $url ) . '">' . $bonipress->format_number( $item['gains'] ) . '</a></td>';
					$url = add_query_arg( array( 'show' => $key, 'num' => 0, 'compare' => urlencode( '<' ) ), $base_url );
					echo '<td class="cell"><a href="' . esc_url( $url ) . '">' . $bonipress->format_number( $item['loses'] ) . '</a></td>';
				}
