 ...
 * @version 1.0
 */
if ( ! class_exists( 'boniPRESS_Stats_Widget_Daily_Loses' ) ) :
	class boniPRESS_Stats_Widget_Daily_Loses extends boniPRESS_Statistics_Widget {
		...
		function __construct( $widget_id = '', $args = array() ) {
			...
			$this->dates = bonipress_get_stat_dates( 'x_dates', $this->args['span'] );
		}
		...
		function get_spending() {
			...
			foreach ( $ctypes as $count => $type_id ) {
				...
				$series[] = "{ label : '" . esc_js( $bonipress->plural() ) . "', fillColor : '" . str_replace( ',1)', ',0.3)', $this->colors[ $type_id ]['negative'] ) . "', strokeColor : '" . $this->colors[ $type_id ]['negative'] . "', pointColor : '" . $this->colors[ $type_id ]['negative'] . "', data : [" . implode( ', ', $values ) . "] }";
		...
		function widget() {
			...
			$lose_label = __( 'Most common ways your users have lost or spent points during this period.', 'bonipress' );
			if ( $this->args['ctypes'] != 'all' )
				$lose_label = $this->core->template_tags_general( __( 'Most common ways your users have lost or spent %_plural% during this period.', 'bonipress' ) );
...
<div id="" class="clear clearfix">
	<h1><?php _e( 'Loses in the last 10 days', 'bonipress' ); ?></h1>
	<p><span class="description"><?php echo $lose_label; ?></span></p>
<?php

			if ( ! empty( $ten_day_lose ) ) {

				echo '<div class="bonipress-popular-items"><ol>';
...
<li>
	...
	<span class="view"><a href="<?php echo esc_url( $url ); ?>"><?php _e( 'View', 'bonipress' ); ?></a></span>
	<ul>
...
<?php
				...
				echo '</ol></div><div class="last-ten-days-chart"><canvas id="daily-loses-' . $this->id . '-chart"></canvas><div id="' . $this->id . '-legend" class="bonipress-chart-legend clear clearfix"></div></div>';

			}
			else {
				echo '<div class="bonipress-empty-widget"><p>' . __( 'No data found', 'bonipress' ) . '</p></div>';
			}
