 ...
 * @version 1.0
 */
if ( ! class_exists( 'boniPS_Stats_Widget_Daily_Gains' ) ) :
	class boniPS_Stats_Widget_Daily_Gains extends boniPS_Statistics_Widget {
		...
		function get_gains() {
			...
			foreach ( $ctypes as $count => $type_id ) {
				...
				$series[] = "{ label : '" . esc_js( $bonips->plural() ) . "', fillColor : '" . str_replace( ',1)', ',0.3)', $this->colors[ $type_id ]['positive'] ) . "', strokeColor : '" . $this->colors[ $type_id ]['positive'] . "', pointColor : '" . $this->colors[ $type_id ]['positive'] . "', data : [" . implode( ', ', $values ) . "] }";
		...
		function widget() {
			...
			$label = __( 'Most common ways your users have gained points during this period.', 'bonips' );
			if ( $this->args['ctypes'] != 'all' )
				$label = $this->core->template_tags_general( __( 'Most common ways your users have gained %_plural% during this period.', 'bonips' ) );
...
<div id="" class="clear clearfix">
	<h1><?php _e( 'Gains in the last 10 days', 'bonips' ); ?></h1>
...
<?php
				...
				echo '</ol></div><div class="last-ten-days-chart"><canvas id="daily-gain-' . $this->id . '-chart"></canvas><div id="' . $this->id . '-legend" class="bonips-chart-legend clear clearfix"></div></div>';
