...
/**
 * boniPS_Chart class
 * @see http://codex.bonips.me/objects/bonips_chart/
 * @since 1.8
 * @version 1.0
 */
if ( ! class_exists( 'boniPS_Chart' ) ) :
	class boniPS_Chart extends boniPS_Object {
		...
		protected function populate() {
			...
			foreach ( bonips_get_types() as $type_id => $label ) {
				...
				$type_object = new boniPS_Point_Type( $type_id );
		...
		public function apply_defaults( $args = array() ) {
			...
			return apply_filters( 'bonips_chart_args', wp_parse_args( $args, $defaults ), $args, $this );
		...
		public function parse_args( $args = array() ) {
			...
			if ( empty( $args['color'] ) ) {
				...
				$color = array();
				foreach ( bonips_get_color_sets() as $set )
					$color[] = $set['positive'];
			...
			return apply_filters( 'bonips_chart_parse_args', $args, $original, $this );
		...
		public function get_chart_options() {
			...
			return apply_filters( 'bonips_chart_get_options', $options, $this );
		...
		public function generate_element_id() {
			...
			return apply_filters( 'bonips_chart_generate_element_id', BONIPS_SLUG . '-chart' . count( $bonips_charts ), $this );
		...
		public function get_canvas_styling() {
			...
			return apply_filters( 'bonips_chart_get_canvas_styling', $css, $this );
		...
		public function get_reference_label( $reference = '' ) {
			...
			return apply_filters( 'bonips_chart_get_reference_label', $label, $reference, $this );
		...
		public function generate_canvas( $type = '', $input = array() ) {
			...
			if ( ! empty( $input ) ) {
				...
				foreach ( $input as $set_id => $input_set ) {
					...
					foreach ( $input_set as $row_id => $row ) {
						...
						elseif ( $row->type == 'amount' ) {
							$label = ( $this->multi_type ) ? __( 'Amount', 'bonips' ) : $this->point_types[ BONIPS_DEFAULT_TYPE_KEY ]->color['positive'];
						}
						...
						$dataset->backgroundColor[] = ( ( $type != 'radar' ) ? $color : ( ( BONIPS_STATS_COLOR_TYPE == 'hex' ) ? bonips_hex_to_rgb( $color, true, '0.6' ) : $color ) );
				...
				$bonips_charts[ $element_id ] = $chart;

			}

			$output = apply_filters( 'bonips_chart_generate_canvas_' . $type, $output, $input, $this );

			return apply_filters( 'bonips_chart_generate_canvas', $output, $input, $this );
