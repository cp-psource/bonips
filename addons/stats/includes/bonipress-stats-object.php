...
/**
 * boniPRESS_Chart class
 * @see http://codex.bonipress.me/objects/bonipress_chart/
 * @since 1.8
 * @version 1.0
 */
if ( ! class_exists( 'boniPRESS_Chart' ) ) :
	class boniPRESS_Chart extends boniPRESS_Object {
		...
		protected function populate() {
			...
			foreach ( bonipress_get_types() as $type_id => $label ) {
				...
				$type_object = new boniPRESS_Point_Type( $type_id );
		...
		public function apply_defaults( $args = array() ) {
			...
			return apply_filters( 'bonipress_chart_args', wp_parse_args( $args, $defaults ), $args, $this );
		...
		public function parse_args( $args = array() ) {
			...
			if ( empty( $args['color'] ) ) {
				...
				$color = array();
				foreach ( bonipress_get_color_sets() as $set )
					$color[] = $set['positive'];
			...
			return apply_filters( 'bonipress_chart_parse_args', $args, $original, $this );
		...
		public function get_chart_options() {
			...
			return apply_filters( 'bonipress_chart_get_options', $options, $this );
		...
		public function get_canvas_styling() {
			...
			return apply_filters( 'bonipress_chart_get_canvas_styling', $css, $this );
		...
		public function get_reference_label( $reference = '' ) {
			...
			return apply_filters( 'bonipress_chart_get_reference_label', $label, $reference, $this );
		...
		public function generate_canvas( $type = '', $input = array() ) {
			...
			$output = apply_filters( 'bonipress_chart_generate_canvas_' . $type, $output, $input, $this );

			return apply_filters( 'bonipress_chart_generate_canvas', $output, $input, $this );
