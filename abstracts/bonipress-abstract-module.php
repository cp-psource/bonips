...
/**
 * boniPRESS_Module class
 * @see https://n3rds.work/docs/bonipress_module/
 * @since 0.1
 * @version 1.3.5
 */
if ( ! class_exists( 'boniPRESS_Module' ) ) :
	abstract class boniPRESS_Module {
		...
		public function __construct( $module_id = '', $args = array(), $type = BONIPRESS_DEFAULT_TYPE_KEY ) {
			...
			if ( ! $this->is_main_type )
				$this->option_id .= '_' . $this->bonipress_type;

			$this->settings_name = 'boniPRESS-' . $this->module_name;
			if ( ! $this->is_main_type )
				$this->settings_name .= '-' . $this->bonipress_type;
			...
			if ( ! $this->is_main_type && ! empty( $this->screen_id ) )
				$this->screen_id = 'bonipress_' . $this->bonipress_type . substr( $this->screen_id, 6 );
				...
				if ( ! empty( $this->option_id ) ) {
					...
					if ( is_array( $this->option_id ) ) {
						...
						foreach ( $this->option_id as $option_id => $option_name ) {
							...
							$settings = bonipress_get_option( $option_name, false );
					...
					// String = one
						...
						// General settings needs not to be loaded
						if ( str_replace( 'bonipress_pref_core', '', $this->option_id ) == '' )
							$this->$module = $this->core;

						// Grab the requested option
						else {

							$this->$module = bonipress_get_option( $this->option_id, false );
		...
		public function load() {
			...
			if ( $this->register === true && ! empty( $this->option_id ) )
				add_action( 'bonipress_admin_init',       array( $this, 'register_settings' ), $this->menu_pos );
			...
			add_action( 'bonipress_admin_init',           array( $this, 'module_admin_init' ), $this->menu_pos+1 );
			add_action( 'bonipress_widgets_init',         array( $this, 'module_widgets_init' ) );
			add_action( 'bonipress_admin_enqueue',        array( $this, 'scripts_and_styles' ), $this->menu_pos );
		...
		public function is_active( $key = '' ) {
			...
			return apply_filters( 'bonipress_module_is_active', $is_active, $module, $key, $this );
		...
		public function add_menu() {
			...
			if ( $this->module_id != 'boniPRESS_Log_Module' && bonipress_override_settings() && ! bonipress_is_main_site() ) return;
		...
		public function set_entries_per_page() {
			...
			if ( $_REQUEST['wp_screen_options']['option'] == $settings_key ) {
				...
				bonipress_update_user_meta( get_current_user_id(), $settings_key, '', $value );
			}
...
<?php
		...
		public function update_notice( $get = 'settings-updated', $class = 'updated', $message = '' ) {
			...
			if ( empty( $message ) )
				$message = __( 'Einstellungen aktualisiert', 'bonipress' );
		...
		public function field_name( $name = '' ) {
			...
			return 'bonipress_pref_core' . $name;
