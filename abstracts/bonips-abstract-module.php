 ...
 * @version 1.3.5
 */
if ( ! class_exists( 'boniPS_Module' ) ) :
	abstract class boniPS_Module {
		...
		public function __construct( $module_id = '', $args = array(), $type = BONIPS_DEFAULT_TYPE_KEY ) {
			...
			if ( empty( $module_id ) ) wp_die( 'boniPS_Module() Fehler. Eine Modul-ID ist erforderlich!' );
			...
			if ( ! $this->is_main_type )
				$this->option_id .= '_' . $this->bonips_type;

			$this->settings_name = 'boniPS-' . $this->module_name;
			if ( ! $this->is_main_type )
				$this->settings_name .= '-' . $this->bonips_type;
			...
			if ( ! $this->is_main_type && ! empty( $this->screen_id ) )
				$this->screen_id = 'bonips_' . $this->bonips_type . substr( $this->screen_id, 6 );
				...
				if ( ! empty( $this->option_id ) ) {
					...
					if ( is_array( $this->option_id ) ) {
						...
						foreach ( $this->option_id as $option_id => $option_name ) {
							...
							$settings = bonips_get_option( $option_name, false );
					...
					// String = one
						...
						// General settings needs not to be loaded
						if ( str_replace( 'bonips_pref_core', '', $this->option_id ) == '' )
							$this->$module = $this->core;

						// Grab the requested option
						else {

							$this->$module = bonips_get_option( $this->option_id, false );
		...
		public function load() {
			...
			if ( $this->register === true && ! empty( $this->option_id ) )
				add_action( 'bonips_admin_init',       array( $this, 'register_settings' ), $this->menu_pos );
			...
			add_action( 'bonips_admin_init',           array( $this, 'module_admin_init' ), $this->menu_pos+1 );
			add_action( 'bonips_widgets_init',         array( $this, 'module_widgets_init' ) );
			add_action( 'bonips_admin_enqueue',        array( $this, 'scripts_and_styles' ), $this->menu_pos );
		...
		public function is_active( $key = '' ) {
			...
			return apply_filters( 'bonips_module_is_active', $is_active, $module, $key, $this );
		...
		public function add_menu() {
			...
			if ( $this->module_id != 'boniPS_Log_Module' && bonips_override_settings() && ! bonips_is_main_site() ) return;

			// Minimum requirements
			if ( ! empty( $this->labels ) && ! empty( $this->screen_id ) ) {

				// Menu Slug
				$menu_slug  = ( ! $this->is_main_type ) ? BONIPS_SLUG . '_' . $this->bonips_type : BONIPS_SLUG;
		...
		public function set_entries_per_page() {
			...
			if ( $_REQUEST['wp_screen_options']['option'] == $settings_key ) {
				...
				bonips_update_user_meta( get_current_user_id(), $settings_key, '', $value );
			}
...
<?php
		...
		public function update_notice( $get = 'settings-updated', $class = 'updated', $message = '' ) {
			...
			if ( empty( $message ) )
				$message = __( 'Einstellungen aktualisiert', 'bonips' );
