...
/**
 * boniPRESS_Hook class
 * @see https://n3rds.work/docs/bonipress_hook/
 * @since 0.1
 * @version 1.3.2
 */
if ( ! class_exists( 'boniPRESS_Hook' ) ) :
	abstract class boniPRESS_Hook {
		...
		public function __construct( $args = array(), $hook_prefs = NULL, $type = BONIPRESS_DEFAULT_TYPE_KEY ) {
			...
			if ( ! empty( $this->defaults ) )
				$this->prefs = bonipress_apply_defaults( $this->defaults, $this->prefs );
		...
		public function run() {
			...
			wp_die( 'function boniPRESS_Hook::run() muss in einer Unterklasse überschrieben werden.' );
		...
		public function preferences() {
			...
			echo '<p>' . __( 'Dieser Hook hat keine Einstellungen', 'bonipress' ) . '</p>';
		...
		public function field_id( $field = '' ) {
			...
			// $bonipress_field_id - This little trick is used when widgets are not in a sidebar
			...
			return $option_id . '-' . str_replace( '_', '-', $this->id ) . '-' . $field . $bonipress_field_id;
		...
		public function over_hook_limit( $instance = '', $reference = '', $user_id = NULL, $ref_id = NULL ) {
			...
			return apply_filters( 'bonipress_over_hook_limit', $over_limit, $instance, $reference, $user_id, $ref_id, $this );
		...
		public function get_limit_types() {
			...
			return apply_filters( 'bonipress_hook_limits', array(
				'x' => __( 'Unlimitiert', 'bonipress' ),
				'd' => __( '/ Tag', 'bonipress' ),
				'w' => __( '/ Woche', 'bonipress' ),
				'm' => __( '/ Monat', 'bonipress' ),
				't' => __( 'Insgesamt', 'bonipress' )
			), $this );
		...
		public function hook_limit_setting( $name = '', $id = '', $selected = '' ) {
			...
			$name    = apply_filters( 'bonipress_hook_limit_name_by', $name, $this );

			// Adjust the field id
			$id      = str_replace( 'limit', 'limit-by', $id );
			$id      = str_replace( 'alimit', 'alimit-by', $id );
			$id      = apply_filters( 'bonipress_hook_limit_id_by', $id, $this );
		...
		public function impose_limits_dropdown( $pref_id = '', $use_select = true ) {
			...
			$settings = '';
			$limits   = array(
				''           => __( 'Unlimitiert', 'bonipress' ),
				'twentyfour' => __( 'Einmal alle 24 Stunden', 'bonipress' ),
				'sevendays'  => __( 'Einmal alle 7 Tage', 'bonipress' ),
				'daily'      => __( 'Einmal pro Tag (um Mitternacht zurückgesetzt)', 'bonipress' )
			);
			$limits   = apply_filters( 'bonipress_hook_impose_limits', $limits, $this );

			echo '<select name="' . $this->field_name( $pref_id ) . '" id="' . $this->field_id( $pref_id ) . '" class="form-control">';

			if ( $use_select )
				echo '<option value="">' . __( 'Auswählen', 'bonipress' ) . '</option>';
		...
		public function is_over_daily_limit( $ref = '', $user_id = 0, $max = 0, $ref_id = NULL ) {
			...
			return apply_filters( 'bonipress_hook_over_daily_limit', $reply, $ref, $user_id, $max );
		...
		public function include_post_type( $post_type ) {
			...
			if ( in_array( $post_type, apply_filters( 'bonipress_post_type_excludes', $excludes ) ) ) return false;
		...
		/**
		 * Limit Query
		 * Queries the boniPRESS log for the number of occurances of the specified
		...
		public function limit_query( $ref = '', $user_id = 0, $start = 0, $end = 0, $ref_id = NULL ) {
			...
			// DB Query
			$total = $wpdb->get_var( "SELECT COUNT(*) FROM {$bonipress_log_table} WHERE {$wheres};" );
			if ( $total === NULL ) $total = 0;

			return apply_filters( 'bonipress_hook_limit_query', $total, $ref, $user_id, $ref_id, $start, $end );
