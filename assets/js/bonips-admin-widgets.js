...
wpWidgets = {
	...
	init : function() {
		...
		$('#widgets-right .sidebar-name').on('click', function() {
			var $this = $(this),
		...
		$('#widgets-right .widgets-holder-wrap').each(function(index, element) {
			var $element = $(element),
				...
				li = $('<li tabindex="0">').text(name.trim()); // Modern
	...
	saveOrder : function( sidebarId ) {
		var data = {
			action: 'bonips-hook-order',
			savewidgets: $('#_wpnonce_widgets').val(),
			ctype: boniPSHooks.type,
			sidebars: []
		};
	...
	removeInactiveWidgets : function() {
		...
		$.post( ajaxurl, data, function() {
			console.log( 'boniPS delete inactive Hook' );
